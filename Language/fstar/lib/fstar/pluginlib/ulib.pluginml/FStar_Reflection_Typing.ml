...
let rec ln' (e : FStarC_Reflection_Types.term) (n : Prims.int) : Prims.bool=
  ...
  | FStarC_Reflection_V2_Data.Tv_Let (recf, attrs, b, def, body) ->
      let uu___ =
        if (ln'_terms attrs n) && (ln'_binder b n)
        then (if recf then ln' def (n + Prims.int_one) else ln' def n)
        else false in
      if uu___ then ln' body (n + Prims.int_one) else false
  | FStarC_Reflection_V2_Data.Tv_Match (scr, ret, brs) ->
      let uu___ =
        if ln' scr n
        then
      ...
      if uu___ then ln'_branches brs n else false
  | FStarC_Reflection_V2_Data.Tv_AscribedT (e1, t, tac, b) ->
      if (ln' e1 n) && (ln' t n)
      then
        (match tac with
