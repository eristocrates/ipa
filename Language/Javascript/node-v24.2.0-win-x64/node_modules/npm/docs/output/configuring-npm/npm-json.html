...
<p>A name can be optionally prefixed by a scope, e.g. <code>@npm/example</code>. See
...
<p>This ensures your package <code>@npm/tea-latte</code> can be installed <em>along</em> with the
second major version of the host package <code>@npm/tea</code> only. <code>npm install tea-latte</code> could possibly yield the following dependency graph:</p>
...
This file contains the dependencies <code>@npm/renderized</code> and <code>@npm/super-streams</code> which
...
<code>@npm/foo</code> to always be <code>1.0.0</code> while also making <code>@npm/bar</code> at any depth beyond <code>@npm/foo</code>
...
<p>Keys can be nested to any arbitrary length. To override <code>@npm/foo</code> only when it's a
child of <code>@npm/bar</code> and only when <code>@npm/bar</code> is a child of <code>@npm/baz</code>:</p>
<pre><code class="language-json">{
  "overrides": {
    "@npm/baz": {
      "@npm/bar": {
...
<h3 id="libc">libc</h3>
<p>If your code only runs or builds in certain versions of libc, you can
specify which ones.  This field only applies if <code>os</code> is <code>linux</code>.</p>
<pre><code class="language-json">{
  "os": "linux",
  "libc": "glibc"
}
