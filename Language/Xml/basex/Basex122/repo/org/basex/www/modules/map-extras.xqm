(:~
 ...
 :
 : @author Leo Wörteler <leo@woerteler.de>
 : @version 0.0.2
 :)
module namespace map-extras='http://www.basex.org/modules/map-extras';
...
(:~
 ...
 :)
declare function map-extras:insert-with(
  $f as function(item()*, item()*) as item()*,
  $key as item(),
  $value as item()*,
  $map as map(*)
) as map(*) {
  map:merge(($map, map:entry($key,
...
(:~
 ...
 :)
declare %public function map-extras:insert(
  $key as item(),
  $value as item()*,
  $map as map(*)
) as map(*) {
  map:merge(($map, map:entry($key, $value)))
};
...
(:~
 ...
 :)
declare %public function map-extras:fold(
...
(:~
 ...
 :)
declare %public function map-extras:values(
  $map as map(*)
) as item()* {
  for-each($map, map:keys($map))
};
...
(:~
 ...
 :)
declare %public function map-extras:map(
...
(:~
 ...
 :)
declare %public function map-extras:map-with-key(
  $f as function(item(), item()*) as item()*,
  $map as map(*)
) as map(*) {
  map:merge(
