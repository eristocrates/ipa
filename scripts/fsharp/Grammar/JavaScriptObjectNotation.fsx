...
module JavaScriptObjectNotation =
    ...
    let abnfTerminal (input: string) =
        let result =
            runParserOnString AugmentedBackusNaurForm.Terminal.Value.inhabitant () "" input
    ...
    let nullLexeme = abnfTerminal "%x6e.75.6c.6c"

    let falseLexeme = abnfTerminal "%x66.61.6c.73.65"

    let trueLexeme = abnfTerminal "%x74.72.75.65"
