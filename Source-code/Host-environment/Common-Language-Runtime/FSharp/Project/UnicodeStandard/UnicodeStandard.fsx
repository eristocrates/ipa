...
[<Struct>]
type Grapheme_Cluster = { as_string: string }
...
let unicodepoints'from'string (input: string) =
...
let string'from'unicodepoint (unicodepoint: Unicodepoint) = Rune(unicodepoint.value).ToString()
