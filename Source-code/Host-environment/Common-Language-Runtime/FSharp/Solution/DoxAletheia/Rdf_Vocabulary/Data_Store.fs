...
type String_Index =
    ...
    member this.as_bytes = BitConverter.GetBytes this.unwrap

    static member wrap_bytes(bytes: byte array) =
        bytes |> BitConverter.ToUInt64 |> StringIndex

    static member wrap_mdb(mdb_string_index: MDBValue) =
        String_Index.wrap_bytes mdb_string_index.as_bytes

    static member some_mdb(mdb_string_index: MDBValue) =
        Some(String_Index.wrap_mdb mdb_string_index)
...
module Transaction =
    let read () =
        environment.BeginTransaction(TransactionBeginFlags.ReadOnly)
...
type Memory_Map<'InputType, 'OutputType> =
    ...
    member this.Get
        ...
        use transaction = Transaction.read ()

        match transaction.Get(this.database, key_as_bytes) with
        | struct (MDBResultCode.Success, _, mdb_value) -> some_continuation mdb_value

        | struct (MDBResultCode.NotFound, _, _) -> None

        | struct (result_code, _, _) -> failwithf "%s.Get failed with MDBResultCode: %A" this.name result_code
    ...
    member this.Put (key_as_bytes: byte array) (value_as_bytes: byte array) (transaction: LightningTransaction) =
        transaction.Put(this.database, key_as_bytes, value_as_bytes)
        |> this.MDBContinuation transaction

    member this.Delete (key_as_bytes: byte array) (transaction: LightningTransaction) =
        transaction.Delete(this.database, key_as_bytes)
...
type Index_Message = EnsureStringIndex of Blake3.Hash * string * AsyncReplyChannel<String_Index>
...
type Data_Index
    (
        string_index_to_string: Memory_Map<String_Index, string>,
        string_hash_to_string_index: Memory_Map<Hash, String_Index>
    ) =

    let load_string_array () =

        let string_array = ResizeArray<string>()

        use transaction = Transaction.read ()

        use cursor = transaction.CreateCursor(string_index_to_string.database)

        let mutable cursor_result = cursor.First()

        let mutable continue_loading = true

        while continue_loading do

            match cursor_result with
            | struct (MDBResultCode.Success, mdb_string_index, mdb_string) ->

                let persisted_string_index = String_Index.wrap_mdb mdb_string_index

                let expected_string_index = uint64 string_array.Count

                if persisted_string_index.unwrap
                   <> expected_string_index then
                    failwithf
                        "Expected String_Index %i, but LMDB contained String_Index %i."
                        expected_string_index
                        persisted_string_index.unwrap

                let string_value = mdb_string.as_bytes |> Encoding.UTF8.GetString

                string_array.Add string_value

                cursor_result <- cursor.Next()

            | struct (MDBResultCode.NotFound, _, _) -> continue_loading <- false

            | struct (result_code, _, _) ->
                failwithf "Initial String_Index_to_String load failed with MDBResultCode: %A" result_code

        string_array


    // Evaluated exactly once while this Data_Index is constructed.
    let string_array = load_string_array ()
        ...
        MailboxProcessor.Start (fun inbox ->
            ...
            let rec loop () =
                async {
                    ...
                    match message with
                    | EnsureStringIndex (string_hash, string_value, reply) ->

                        let string_index = uint64 string_array.Count |> StringIndex

                        use transaction = Transaction.write ()

                        transaction
                        |> string_hash_to_string_index.Put string_hash.as_bytes string_index.as_bytes
                        |> string_index_to_string.Put string_index.as_bytes string_value.as_bytes
                        |> Transaction.commit
    ...
    member _.maybe_string_index_for_hash(string_hash: Hash) : String_Index option =
    ...
    member _.get_string_for_string_index(string_index: String_Index) : string =

        string_index_to_string.Get string_index.as_bytes (fun mdb_string ->
            mdb_string.as_bytes
    ...
    member this.ensure_string_index_for_string(string_value: string) : String_Index =

        let string_hash = blake3_hash string_value

        index_agent.PostAndReply(fun reply -> EnsureStringIndex(string_hash, string_value, reply))


    member this.intern_string(string_value: string) : unit =
...
let data_index =
    Data_Index(Memory_Map.string_index_to_string, Memory_Map.string_hash_to_string_index)
