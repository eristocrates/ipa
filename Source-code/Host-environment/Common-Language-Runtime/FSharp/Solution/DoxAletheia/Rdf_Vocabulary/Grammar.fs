        ...
        parser {
            let! digits =
                choice [
                         ...
                         tuple3 (parse_char '2') (parse_char '5') (any_point_from_interval Unicodepoint.interval_0_5)
                         ...
                         tuple3
                             (parse_char '2')
                             (any_point_from_interval Unicodepoint.interval_0_4)
                         ...
                         |>> (fun struct (left_digit, center_digit, right_digit) ->
                             [| left_digit
                                ...
                                right_digit |])
                         tuple2
                             (any_point_from_interval Unicodepoint.interval_1_9)
                             (any_point_from_interval Unicodepoint.Partition.Ascii_Digits)
    ...
    static member parser: Parser<IPv4_Address, Code_Point, unit, ReadableMemory<Code_Point>> =
        parser {
            ...
            let ipv4 =
                sprintf
                    "%s.%s.%s.%s"
                    outer_left.as_raw_string
                    inner_left.as_raw_string
                    inner_right.as_raw_string
                    outer_right.as_raw_string
...
type Hextet =
    ...
    static member parse(input_string: string) =
        ...
        | Error failure -> failwithf "%A: %s" failure (code_line_message input_string.as_code_line Hextet.metasyntax)
...
type Least_Significant_32_Bits =
    ...
    member this.substrings =
        match this with
        | LeastSignificantHextets (left_hextet, right_hextet) ->
            [| left_hextet.as_raw_string
               right_hextet.as_raw_string |]
...
type IPv6_Address =
    ...
    static member Maybe_One_h16__Four_h16__ls32_parser: Parser<IPv6_Address, Code_Point, unit, ReadableMemory<Code_Point>> =
        parser {
            ...
            let ipv6_string =
                sprintf "%s::%s:%s" maybe_hextet_string hextet_string ls32.as_rendered_string
    ...
    static member Maybe_Four_h16__One_h16__ls32_parser: Parser<IPv6_Address, Code_Point, unit, ReadableMemory<Code_Point>> =
        parser {
            ...
            let ipv6_string =
                sprintf "%s::%s:%s" maybe_hextet_string h16.as_raw_string ls32.as_rendered_string
    ...
    member this.substrings =
        ...
        | Maybe_One_h16__Four_h16__ls32 (raw_string, maybe_hextet, hextets, ls32) ->
            match maybe_hextet with
            | ValueSome hextet ->
                Array.concat [| [| hextet.as_raw_string |]
                                hextets
                                |> Array.map (fun hextet -> hextet.as_raw_string)
                                ls32.substrings |]
            | ValueNone ->
        ...
        | Maybe_Two_h16__Three_h16__ls32 (raw_string, maybe_two_hextets, hextets, ls32) ->
            match maybe_two_hextets with
            | ValueSome h16s ->
                Array.concat [| h16s |> Array.map (fun h16 -> h16.as_raw_string)
                                hextets
                                |> Array.map (fun hextet -> hextet.as_raw_string)
                                ls32.substrings |]
            | ValueNone ->
        ...
        | Maybe_Four_h16__One_h16__ls32 (raw_string, maybe_four_hextets, h16, ls32) ->
            ...
            | ValueSome h16s ->
                Array.concat [| h16s |> Array.map (fun h16 -> h16.as_raw_string)
                                ...
                                ls32.substrings |]
            | ValueNone ->
                Array.concat [| [| h16.as_raw_string |]
                                ls32.substrings |]
        ...
        | Maybe_Seven_h16 (raw_string, maybe_seven_hextets) ->
            match maybe_seven_hextets with
            | ValueSome h16s -> Array.concat [| h16s |> Array.map (fun h16 -> h16.as_raw_string) |]
            | ValueNone -> [||]
...
type IPvFuture =
    ...
    static member parse(input_string: string) =
        ...
        | Ok success -> success
        | Error failure -> failwithf "%A: %s" failure (code_line_message input_string.as_code_line IPvFuture.metasyntax)
...
type Scheme =
    ...
    static member parse(input_string: string) =
        ...
        | Ok success -> success
        | Error failure -> failwithf "%A: %s" failure (code_line_message input_string.as_code_line Scheme.metasyntax)
...
type Fragment =
    ...
    static member parse(input_string: string) =
        ...
        | Ok success -> success
        | Error failure -> failwithf "%A: %s" failure (code_line_message input_string.as_code_line Fragment.metasyntax)
...
type Query =
    ...
    static member parser: Parser<Query, Code_Point, unit, ReadableMemory<Code_Point>> =
        parser {
            ...
            return!
                choice [ iquery.code_line_parser
                         |>> (fun code_line ->
                             code_line
                             |> String_Pool.from_code_line
                             |> InternationalizedQuery)
                         query_fragment.code_line_parser
                         |>> (fun code_line ->
                             code_line
                             |> String_Pool.from_code_line
...
type Userinfo =
    ...
    static member parse(input_string: string) =
        ...
        | Ok success -> success
        | Error failure -> failwithf "%A: %s" failure (code_line_message input_string.as_code_line Userinfo.metasyntax)
...
type Segment =
            ...
            return!
                choice [ many ipchar_encoded.code_line_parser
                         |>> (fun immutable_code_square ->
                             Code_Line.from_immutable_code_square immutable_code_square
                             |> String_Pool.from_code_line
    ...
    static member parse(input_string: string) =
        ...
        | Ok success -> success
        | Error failure -> failwithf "%A: %s" failure (code_line_message input_string.as_code_line Segment.metasyntax)
...
type Nonzero_Noncolon_Segment =
    ...
    static member parse(input_string: string) =
        ...
        | Ok success -> success
        | Error failure ->
            failwithf "%A: %s" failure (code_line_message input_string.as_code_line Nonzero_Noncolon_Segment.metasyntax)
    ...
    static member parser: Parser<Path_Abempty, Code_Point, unit, ReadableMemory<Code_Point>> =
        parser {
            ...
            let path_string =
                "/"
                + (segments
                   |> Array.map (fun segment -> segment.as_raw_string)
                   |> String.concat "/")
...
type Path_Absolute =
    ...
    static member parser: Parser<Path_Absolute, Code_Point, unit, ReadableMemory<Code_Point>> =
        parser {
            ...
            return
                match maybe_path with
                | ValueSome path when path.IsInternationalizedPathRootless ->
                    InternationalizedAbsoluteRootPath(path_string, path)
        ...
        parser {
            ...
            return!
                choice [ many ireg_name.code_point_parser
                         |>> (fun immutable_code_line ->
                             immutable_code_line.AsSpan().ToArray()
                             |> String_Pool.from_code_line
...
type Authority =
            ...
            return
                ...
                | ValueSome userinfo, host, ValueNone ->
                    let authority_string =
                        userinfo.as_rendered_string
                        + host.as_rendered_string
    ...
    static member parse(input_string: string) =
        ...
        | Ok success -> success
        | Error failure -> failwithf "%A: %s" failure (code_line_message input_string.as_code_line Authority.metasyntax)
            ...
            let! maybe_part =
                opt (
                    choice [
                             ...
                             |>> (fun path -> RelativeAbsolutePart(path))
                             Path_Noscheme.parser
                             |>> (fun path -> RelativeNoschemePart(path))
            ...
            let! maybe_part =
                opt (
                    choice [
                             ...
                             Path_Absolute.parser
                             |>> (fun path -> HierarchicalAbsolutePart(path))
                             Path_Rootless.parser
                             |>> (fun path -> HierarchicalRootlessPart(path))
    ...
    static member parser: Parser<Absolute_IRI, Code_Point, unit, ReadableMemory<Code_Point>> =
        parser {
            ...
            return
                ...
                | ValueSome query_ ->
                    let absolute_string =
                        scheme.as_rendered_string
                        + hier_part.as_rendered_string
                        + query_.as_rendered_string
...
and Namespaced_IRI =
    ...
    member this.as_prefixed_name (delimiter: string) (prefix_map: Map<string, string>) =
        try
            ...
            match namespace_name, this.local_name.as_raw_string with
...
and Rdf_Triple =
    ...
    member this.as_raw_strings =
        this.curSubject.as_raw_string, this.curPredicate.as_raw_string, this.curObject.as_raw_string

    member this.as_rendered_strings (prefix_delimiter: string) (prefix_map: Map<string, string>) =
        this.curSubject.as_rendered_string prefix_delimiter prefix_map,
        this.curPredicate.as_rendered_string prefix_delimiter prefix_map,
...
and Vertex =
    ...
    member this.as_rendered_string (prefix_delimiter: string) (prefix_map: Map<string, string>) =
        match this with
        | SubjectVertex rdf_subject -> rdf_subject.as_rendered_string prefix_delimiter prefix_map
        | ObjectVertex rdf_object -> rdf_object.as_rendered_string prefix_delimiter prefix_map
...
and Edge =
    ...
    member this.as_rendered_string (prefix_delimiter: string) (prefix_map: Map<string, string>) =
        match this with
        | PredicateEdge rdf_predicate -> rdf_predicate.as_rendered_string prefix_delimiter prefix_map
        | TripleEdge rdf_triple -> rdf_triple.curPredicate.as_rendered_string prefix_delimiter prefix_map
