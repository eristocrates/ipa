...
module ioto =
    ...
    ///   <para>Generic property that marks the relationship between some entity and some ssn:Property. Dependant on context, it can be sensing, acting or else. Useful to descrie a generic service offered by a complex system that aggregates both sensors and actuators.</para>
    /// labels<para>has relationship with property</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>System implementing a control feedback loop in order to control an actuator thanks to the feedback of some sensors.</para>
    /// labels<para>Controlled system</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Service operation that deals with device management</para>
    /// labels<para>Device management operation</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>This kind of service offers a set of operations regarding device management. That means it's not related to the functionnality of the object itsefl, but rather to its inner working : battery state, connectivity, firmware update...</para>
    /// labels<para>Device management service</para></remarks>
    ...
    ///   <para>Specific message that has no content, its sole transfer achieves its purpose. For example, if an empty message is sent to the "switch light on" service of a light bulb, there is no need for an explicit content of the message asking the light to be turned on.</para>
    /// labels<para>Empty message</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Relationship between an Entity and a service that is accessible through it.</para>
    ...
    ///   <para>Characterizes a unique identifier for a thing</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Relationship between an operation and its impact on a property</para>
    ...
    /// <remarks>
    ///   <para>owl:DatatypeProperty</para>
    ///   <para>Associates an Instant individual with a timestamp</para>
    /// </remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Relationship between a service and an entity that accesses it.</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Indicates that the property is the feature of interest of some entity of an IoT system, that can measure it/act upon it.</para>
    /// labels<para>is feature of interest for</para></remarks>
    ...
    ///   <para>owl:ObjectProperty</para>
    ///
    /// labels<para>is input message of</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Most general thing in an iot ontology, a Thing can either be a physical connected object, an application, an abstracted interface... The class Object of DUL, from whom it inherits, doen't only represent physical objects, and can cover a fonction for instance.</para>
    /// labels<para>IoT-Thing</para><para>Thing</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Service operation that is relevant to the business function of the service (and the object underneath in the case of IoT). This type of functions may be linked to a property the object deals with (monitoring or modification)</para>
    /// labels<para>Business operation</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Service intended to be an interface for the functionnalities the object was designed to perform : measure temperature for a temperature sensor, switch on/off for a switch...</para>
    /// labels<para>Business service</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Relation between a controlled system and the entities that provide it with a feedback about its action, usually sensors.</para>
    /// labels<para>gets feedback from</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Controlled system that requires an operator attention, because it is not functionnig properly. Detected when the difference between the Observations and the Actuation made on the monitored property is out of its operating range.</para>
    /// labels<para>Defectuous System</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>This class describes the impact an operation may have on a property. It makes sense in an IoT environement, where things can manage properties of their environment and modify them.</para>
    /// labels<para>Impact on property</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>A manager is an application that offers management capabilities as described in the OneM2M norm.</para>
    ...
    ///   <para>owl:NamedIndividual</para>
    ///   <para>This individual represents the body of a request containing simple data, such as a numerical value.</para>
    /// labels<para>Simple data content</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Relationship between a Thing and a property of the environment. In a cyber-physical system, devices and software agents may have one to many feature of interest from the environment. This feature can be a property, another device, a person...</para>
    ...
    ///   <para>owl:ObjectProperty</para>
    ///
    /// labels<para>is impact of</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Relationship between an impact and the impacted property</para>
    /// labels<para>on property</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Relationship between an entity that provides informations to a system and this system, that interprets these informations as a feedback of its action.</para>
    /// labels<para>provides feedback to</para></remarks>
    ...
    ///   <para>owl:ObjectProperty</para>
    ///
    /// labels<para>property impacted by</para></remarks>
