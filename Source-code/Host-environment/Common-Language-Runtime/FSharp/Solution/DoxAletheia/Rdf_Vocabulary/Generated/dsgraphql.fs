...
module dsgraphql =
    ...
    ///   <para>rdfs:Class</para>
    ///   <para>shacl:NodeShape</para>
    ///   <para>A test case that can be used to verify that a given JSON object is translated into RDF triples using shape definitions from a given graphql:Service. The service must be in the test case graph.</para>
    /// labels<para>JSON-to-RDF test case</para></remarks>
    ...
    ///
    /// labels<para>GraphQL Schema</para></remarks>
    ...
    ///   <para>shacl:NodeShape</para>
    ///   <para>rdfs:Class</para>
    ///   <para>A test case that can be used to verify that a RDF/SHACL to GraphQL writer works correctly. The expected result must be the complete GraphQL schema string.</para>
    /// labels<para>Writer test case</para></remarks>
    ...
    ///   <para>Can be attached to property shapes to indicate they should be round-tripped as GraphQL type ID.</para>
    /// labels<para>is ID field</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>If set to true for a node shape, then it should become a GraphQL interface.</para>
    /// labels<para>is GraphQL interface</para></remarks>
    ...
    ///
    /// An RDF graph may contain multiple GraphQL schemas, each providing a different API or view on the underlying data. GraphQL services can include each other using owl:imports. To simplify common usage patterns, owl:Ontology has been marked as a subclass, so that all Ontologies are also graphql:Schemas once this graphql namespace is imported.</para>
    /// labels<para>GraphQL Schema</para></remarks>
    ...
    ///   <para>rdfs:Class</para>
    ///   <para>shacl:NodeShape</para>
    ///   <para>Represents an input object type in a GraphQL schema.</para>
    /// labels<para>Input object type</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>The GraphQL query string.</para>
    ...
    ///   <para>rdfs:Class</para>
    ///   <para>shacl:NodeShape</para>
    ///   <para>Represents specific GraphQL queries, stored by their query string and possibly other properties with metadata.</para>
    /// labels<para>GraphQL query</para></remarks>
    /// <seealso href="http://datashapes.org/graphql#Query">http://datashapes.org/graphql#Query</seealso>
    let Query = Prefixed_Name(dsgraphql, "Query") |> PrefixedName
    /// <summary>
    ...
    ///   <para>shacl:NodeShape</para>
    ///   <para>rdfs:Class</para>
    ///   <para>The definition of an input value, consisting of a name, type and an optional default value.</para>
    /// labels<para>Input value</para></remarks>
    /// <seealso href="http://datashapes.org/graphql#InputValue">http://datashapes.org/graphql#InputValue</seealso>
    let InputValue = Prefixed_Name(dsgraphql, "InputValue") |> PrefixedName
    /// <summary>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>Defines a GraphQL name for a given subject. The values need to match the regular expression specified in http://facebook.github.io/graphql/October2016/#Name.</para>
    ...
    ///   <para>Gets the GraphQL field name for a property path at a given node shape.</para>
    ...
    ///   <para>rdfs:Class</para>
    ///
    /// labels<para>List type</para></remarks>
    ...
    /// <remarks>
    ///   <para>shacl:NodeShape</para>
    ///   <para>Defines syntax restrictions on the use of the graphql:name property.</para>
    /// labels<para>Name shape</para></remarks>
    ...
    ///   <para>shacl:NodeShape</para>
    ///
    /// labels<para>Named type</para></remarks>
    ...
    ///   <para>shacl:NodeShape</para>
    ///
    /// labels<para>GraphQL node shape shape</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>The text of the actual query in GraphQL syntax.</para>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>An optional JSON object with variables that shall be passed into the query for execution.</para>
    /// labels<para>variables</para></remarks>
    ...
    ///   <para>rdfs:Class</para>
    ///   <para>shacl:NodeShape</para>
    ///   <para>A test case that runs a given GraphQL query (graphql:query) and compares its output JSON with an expected JSON string. Formatting of the strings is ignored. The TestCase is also a graphql:Schema defining which shapes to publish.</para>
    /// labels<para>GraphQL query test case</para></remarks>
    ...
    ///   <para>rdf:Property</para>
    ///
    /// labels<para>query shape</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>Points at a prefix declaration of the default namespace for the shapes derived by GraphQL types. This property can be used to round-trip .graphql files.</para>
    /// labels<para>default prefix</para></remarks>
    ...
    ///   <para>shacl:NodeShape</para>
    ///
    /// labels<para>GraphQL class shape</para></remarks>
    ...
    ///   <para>shacl:NodeShape</para>
    ///
    /// labels<para>Non null type</para></remarks>
