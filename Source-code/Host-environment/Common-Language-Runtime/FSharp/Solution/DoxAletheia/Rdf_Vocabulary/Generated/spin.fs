...
module spin =
    ...
    ///   <para>A SPIN template that wraps an ASK query.</para>
    /// labels<para>Ask template</para></remarks>
    ...
    ///   <para>Suggested abstract base class for all Templates.</para>
    ...
    ///   <para>Can be used to point from any resource to an Update.</para>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>An optional attribute of ConstraintViolations to point at a violation level (such as Error and Warning). Defaults to spin:Error.</para>
    /// labels<para>violation level</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>An optional attribute of ConstraintViolations to provide a path expression from the root resource to the value that is invalid. If this is a URI then the path represents the predicate of a subject/predicate combination. Otherwise it should be a blank node of type sp:Path.</para>
    /// labels<para>violation path</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>The root resource of the violation (often ?this in the constraint body).</para>
    /// labels<para>violation root</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>An optional attribute of ConstraintViolations to point at a specific RDF node that caused the violation. Should be accessible from the root via the violationPath.</para>
    /// labels<para>violation value</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>An object that can be created by spin:constraints to provide information about a constraint violation.</para>
    /// labels<para>Constraint violation</para></remarks>
    ...
    ///   <para>rdfs:Class</para>
    ///
    /// labels<para>Construct template</para></remarks>
    ...
    /// <remarks>
    ///   <para>spin:ConstructTemplate</para>
    ///   <para>Suggested abstract base class for all ConstructTemplates.</para>
    /// labels<para>Construct templates</para></remarks>
    ...
    ///   <para>An abstract base class for all defined functions. This class mainly serves as a shared root so that the various instances of the Function metaclass are grouped together.</para>
    /// labels<para>Functions</para></remarks>
    ...
    ///   <para>An inferencing rule attached to a class. Rules are expressed as CONSTRUCT queries or INSERT/DELETE operations where the variable ?this will be bound to the current instance of the class. These inferences can be used to derive new values from existing values at the instance.</para>
    ...
    ///
    /// will execute the sp:Select or sp:Ask that ?query points to and pre-bind ?this with the value of owl:Thing for the execution of the query. The first result variable of the query's result set will be bound to ?a, the second to ?b etc. If the nodes on the right are bound (or constants) then it will match with the values from the result set.</para>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>Can be used to link a spin:ConstraintViolation with the query or template call that caused it. This property is typically filled in automatically by the constraint checking engine and does not need to be set manually. However, it can be useful to learn more about the origin of a violation.</para>
    /// labels<para>violation source</para></remarks>
    ...
    /// <remarks>
    ///   <para>spin:AskTemplate</para>
    ///   <para>Suggested abstract base class for all AskTemplates.</para>
    /// labels<para>Ask templates</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>Can be set to true to indicate that this module shall not be instantiated. Abstract modules are only there to organize other modules into hierarchies.</para>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>The metaclass of SPIN templates. Templates are classes that are instances of this class. A template represents a reusable SPARQL query or update request that can be parameterized with arguments. Templates can be instantiated in places where normally a SPARQL query or update request is used, in particular as spin:rules and spin:constraints.</para>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>The type of the supported levels of constraint violations, including spin:Error and spin:Warning.</para>
    /// labels<para>Constraint violation level</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>Provides metadata about a column in the result set of a (SPARQL) query, for example of the body queries of SPIN templates. Columns can define human-readable labels that serve as column titles, using rdfs:label.</para>
    ...
    /// <remarks>
    ///   <para>spin:ConstraintViolationLevel</para>
    ///   <para>Indicates that the associated constraint violation is an error that should be fixed.</para>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>Metaclass for functions that can be used in SPARQL expressions (e.g. FILTER or BIND). The function themselves are classes that are instances of this metaclass. Function calls are instances of the function classes, with property values for the arguments.</para>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>An abstract building block of a SPARQL system. A Module can take Arguments as input and applies them on an input RDF Graph. The Arguments should be declared as spin:constraints.</para>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>An "artificial" parent class for all Functions and Templates.</para>
    /// labels<para>Modules</para></remarks>
    ...
    /// <remarks>
    ///   <para>spin:MagicProperty</para>
    ///   <para>An abstract superclass that can be used to group all spin:MagicProperty instances under a single parent class.</para>
    /// labels<para>Magic properties</para></remarks>
    ...
    /// <remarks>
    ///   <para>spin:ConstraintViolationLevel</para>
    ///   <para>Indicates that the associated constraint violation is a severe error that must be fixed. Constraint checking engines may decide to stop further evaluation once a fatal error has been detected.</para>
    ...
    /// <remarks>
    ///   <para>spin:ConstraintViolationLevel</para>
    ///   <para>Indicates that the associated constraint violation is a not a problem. This level can be used for diagnostic and debugging purposes.</para>
    ...
    ///   <para>rdfs:Class</para>
    ///
    /// labels<para>Magic property</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>The metaclass of spin:rule and its subproperties. spin:RuleProperties can have additional metadata attached to them.</para>
    /// labels<para>Rule property</para></remarks>
    ...
    ///
    /// TableDataProviders can link to definitions of columns via spin:column, and these definitions can inform rendering engines.</para>
    /// labels<para>Table data provider</para></remarks>
    ...
    ///   <para>Suggested abstract base class for all SelectTemplates.</para>
    /// labels<para>Select templates</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>A SPIN template that has an UPDATE command as its body.</para>
    /// labels<para>Update template</para></remarks>
    ...
    /// <remarks>
    ///   <para>spin:ConstraintViolationLevel</para>
    ///   <para>Indicates that the associated constraint violation is a problem that may not have to be fixed but may lead to issues in the future.</para>
    ...
    /// <remarks>
    ///   <para>sp:Variable</para>
    ///   <para>A system variable representing the current context instance in a rule or constraint.</para>
    /// labels<para>_this</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>An "abstract" base property that groups together those system properties that the user will hardly ever need to see in property trees. This property may be dropped in future versions of this ontology - right now it's mainly here for convenience.</para>
    /// labels<para>system property</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>Can be used to attach a "constructor" to a class. A constructor is a SPARQL CONSTRUCT query or INSERT/DELETE Update operation that can add initial values to the current instance. At execution time, the variable ?this is bound to the current instance. Tools can call constructors of a class and its superclasses when an instance of a class has been created. Constructors will also be used to initialize resources that have received a new rdf:type triple as a result of spin:rules firing.</para>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>A SPIN template that wraps a SELECT query.</para>
    /// labels<para>Select template</para></remarks>
    ...
    /// <remarks>
    ///   <para>spin:UpdateTemplate</para>
    ///   <para>Suggested abstract base class for all UpdateTemplates.</para>
    /// labels<para>Update templates</para></remarks>
    ...
    ///
    /// will execute the sp:Ask behind ?query with the variable ?this pre-bound to owl:Thing.</para>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>The preferred width of the associated Column, for display purposes. Values in pixels (rendering engines may multiply the values depending on resolution).</para>
    /// labels<para>column width</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>The index of a column (from left to right) starting at 0.</para>
    /// labels<para>column index</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>Can be used to link a resource with a SPARQL query or update request (sp:Command).</para>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>Can be used to point from any resource to a Query.</para>
    ...
    /// <remarks>
    ///   <para>spin:Function</para>
    ///   <para>Evaluates a given SPIN expression or SELECT or ASK query, and returns its result. The first argument must be the expression in SPIN RDF syntax. All other arguments must come in pairs: first a property name, and then a value. These name/value pairs will be pre-bound variables for the execution of the expression.</para>
    /// labels<para>eval</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>Can be used to link a ConstraintViolation with one or more UPDATE Templates that would help fix the violation.</para>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>A template string for displaying instantiations of a module in human-readable form. The template may contain the argument variable names in curly braces to support substitution. For example, "The number of values of the {?arg1} property."</para>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>Can be used to link two sub-properties of spin:rule (or spin:rule itself) to instruct the SPIN engine to execute one set of rules before another one. The values of the subject property will be executed before those of the object property.</para>
    /// labels<para>next rule property</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>The return type of a Function, e.g. xsd:string.</para>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>Can be attached to spin:rule (or subclasses thereof) to instruct a SPIN rules engine that it shall only execute the rules max times. If no value is specified, then the rules will be executed with no specific limit.</para>
    /// labels<para>rule property max iteration count</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>The symbol of a function, e.g. "=" for the eq function.</para>
    ...
    ///
    /// will execute the sp:Construct ?query using the binding of owl:Thing to ?this. The resulting triples will be bound to ?s, ?p and ?o. If any of ?s ?p ?o are bound, it will only return the matching triples.</para>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>Groups together the kinds of SPARQL commands that can appear as SPIN rules and constructors: CONSTRUCT, DELETE WHERE and DELETE/INSERT. This class is never to be instantiated directly.</para>
    /// labels<para>Rule</para></remarks>
    /// <seealso href="http://spinrdf.org/spin#Rule">http://spinrdf.org/spin#Rule</seealso>
    let Rule = Prefixed_Name(spin, "Rule") |> PrefixedName
    /// <summary>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>The datatype or resource type of a spin:Column. For example this is useful as metadata to inform a rendering engine that numeric columns (e.g. xsd:float) need to be right-aligned.</para>
    /// labels<para>column type</para></remarks>
