...
module dul =
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Any Entity that cannot be located in space-time. E.g. mathematical entities: formal semantics elements, regions within dimensional spaces, etc.</para>
    /// labels<para>Abstract</para><para>Astratto</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Any physical, social, or mental object, or a substance. Following DOLCE Full, objects are always participating in some event (at least their own life), and are spatially located.</para>
    /// labels<para>Oggetto</para><para>Object</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>An Event with at least one Agent that isParticipantIn it, and that executes a Task that typically isDefinedIn a Plan, Workflow, Project, etc.</para>
    /// labels<para>Action</para><para>Azione</para></remarks>
    ...
    ///   <para>A Collection has a unification criterion, provided by a Description; for example, a community of practice can be unified by a shared theory or interest, e.g. the community that makes research on mirror neurons shares some core knowledge about mirror neurons, which can be represented as a Description MirrorNeuronTheory that unifies the community. There can be several unifying descriptions.</para>
    /// labels<para>unifica</para><para>unifies</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between collections and entities, e.g. 'the Night Watch by Rembrandt is in the Rijksmuseum collection'; 'Davide is member of the Pen Club', 'Igor is one the subjects chosen for the experiment'.</para>
    ...
    /// On the other hand, in an antiques context, the individual aspects for a specific piece of furniture are a major focus of attention, and may constitute the actual added value, because the design parameters for old furniture are often not fixed, and may not be viewed as 'anomalies'.</para>
    /// labels<para>Quality</para><para>Qualità</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between an action and a task, e.g. 'putting some water in a pot and putting the pot on a fire until the water starts bubbling' executes the task 'boiling'.</para>
    /// labels<para>esegue il task</para><para>executes task</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>A Concept that classifies an Event . An event type describes how an Event should be interpreted, executed, expected, seen, etc., according to the Description that the EventType isDefinedIn (or used in)</para>
    /// labels<para>Tipo di evento</para><para>Event type</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between situations and objects, e.g. 'this morning I've prepared my coffee and had my fingers burnt' (i.e.: the preparation of my coffee this morning included me).</para>
    /// labels<para>include l'oggetto</para><para>includes object</para></remarks>
    ...
    /// A collection is not a logical class: a collection is a first-order entity, while a class is second-order.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>A social norm.</para>
    /// labels<para>Norma</para><para>Norm</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between an object and a process, e.g. 'John took part in the discussion', 'a large mass of snow fell during the avalanche', or 'a cook, some sugar, flour, etc. are all present in the cooking of a cake'.</para>
    /// labels<para>è un partecipante a</para><para>is participant in</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between a Concept and an Entity, e.g. 'John is considered a typical rude man'; your last concert constitutes the achievement of a lifetime; '20-year-old means she's mature enough'.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>A physical objects with biological characteristics, typically that organisms can self-reproduce.</para>
    /// labels<para>Organismo</para><para>Organism</para></remarks>
    ...
    /// labels<para>Organizzazione</para><para>Organization</para></remarks>
    ...
    /// An occurrence of a pattern is an 'observable', or detected Situation</para>
    ...
    /// Since social objects are not formal entities, Relation includes here any 'relation-like' entity in common sense, including social relations.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>A SocialAgent that needs the existence of a specific NaturalPerson in order to act (but the lifetime of the NaturalPerson has only to overlap that of the SocialPerson).</para>
    /// labels<para>Persona sociale</para><para>Social person</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>A social entity with agentive features, but whose status is the result of a cultural transformation from e.g. a PhysicalObject, an Event, an Abstract, another SocialObject, etc. For example: the holy grail, deus ex machina, gods, magic wands, etc.</para>
    ...
    /// labels<para>Piano</para><para>Plan</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between entities and regions, e.g. 'the color of my car is red'.</para>
    /// labels<para>è una regione di</para><para>is region for</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>A physical object that is inherently located; for example, a water area.</para>
    /// labels<para>Physical place</para><para>Luogo fisico</para></remarks>
    ...
    /// In this generic sense, a Place is an "approximate" location. For an "absolute" location, see the class SpaceRegion</para>
    ...
    /// For 'absolute' locations, see SpaceRegion</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>The hasPart relation without transitivity, holding between an Object (the system) and another (the component), and assuming a Design that structures the Object.</para>
    /// labels<para>has component</para><para>ha componente</para></remarks>
    ...
    /// labels<para>Esecuzione di piano</para><para>Plan execution</para></remarks>
    ...
    /// See Event class for some thoughts on classifying events. See also 'Transition'.</para>
    /// labels<para>Processo</para><para>Process</para></remarks>
    ...
    /// For example, reaching a destination could be defined by a plan to get on holidays, while the plan to execute the task can consist of putting some travels into a sequence.</para>
    /// labels<para>Task</para><para>Task</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between a Description and a Concept, e.g. a Workflow for a governmental Organization defines the Role 'officer', or 'the Italian Traffic Law defines the role Vehicle'.</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between entities and regions, e.g. 'the number of wheels of that truck is 12', 'the time of the experiment is August 9th, 2004', 'the whale has been localized at 34 degrees E, 20 degrees S'.</para>
    ...
    /// Subproperties and restrictions can be used to specialize overlaps for objects, events, time intervals, etc.</para>
    ...
    /// Subproperties can be defined in order to distinguish the different uses.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>A legal position by which an Agent is entitled to obtain something from another Agent , under specified circumstances, through an enforcement explicited either in a Law, Contract , etc.</para>
    /// labels<para>Diritto</para><para>Right</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between a description and a task, e.g. the recipe for a cake defines the task 'boil'.</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between a description and a role, e.g. the recipe for a cake defines the role 'ingredient'.</para>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>Insieme {it}</para><para>Set</para></remarks>
    ...
    /// The intuition for 'meaning' is intended to be very broad. A separate, large comment is included in the encoding of 'expresses', for those who want to investigate more on what kind of meaning can be represented in what form.</para>
    /// labels<para>is expressed by</para><para>è espresso da</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Any Region in a dimensional space that is used to represent some characteristic of a SocialObject, e.g. judgment values, social scalars, statistical attributes over a collection of entities, etc.</para>
    /// labels<para>Caratteristica sociale</para><para>Social attribute</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between a description and a task, e.g. the task 'boil' is defined in a recipe for a cake.</para>
    ...
    /// This class can also be used to act as 'naturalized reifications' of logical theories (of course, they will be necessarily incomplete in this case, because second-order entities are represented as first-order ones).</para>
    /// labels<para>Teoria</para><para>Theory</para></remarks>
    ...
    /// labels<para>intervallo temporale di</para><para>is time interval of</para></remarks>
    ...
    /// A full representation of the transition ontology is outside the expressivity of OWL, because we would need qualified cardinality restrictions,  coreference, property equivalence, and property composition.</para>
    /// labels<para>Transizione</para><para>Transition</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between situations and time intervals, e.g. 'this morning I've prepared my coffee and had my fingers burnt' (i.e.: preparing my coffee was held this morning). A data value attached to the time interval typically complements this modelling pattern.</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between situations and events, e.g. 'this morning I've prepared my coffee and had my fingers burnt' (i.e.: the preparation of my coffee this morning included a burning of my fingers).</para>
    /// labels<para>include l'evento</para><para>includes event</para></remarks>
    ...
    /// This class is very useful to apply a variety of the so-called "ClassesAsValues" design pattern, when it is used to talk about the extensional aspect of a class. An alternative variety of the pattern applies to the intensional aspect of a class, and the class Concept should be used instead.</para>
    /// labels<para>Collezione di un tipo</para><para>Type collection</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Units of measure are conceptualized here as parameters on regions, which can be valued as datatype values.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>A Plan that defines Role(s), Task(s), and a specific structure for tasks to be executed, usually supporting the work of an Organization</para>
    /// labels<para>Workflow</para><para>Workflow</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>Esecuzione di workflow</para><para>Workflow execution</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>The relation holding between any Agent, and a SocialAgent. In principle, a SocialAgent requires at least one PhysicalAgent in order to act, but this dependency can be 'delegated'; e.g. a university can be acted for by a department, which on its turm is acted for by physical agents.</para>
    ...
    ///   <para>owl:SymmetricProperty</para>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A catch-all object property, useful for alignment and querying purposes.</para>
    /// labels<para>associatedWith</para></remarks>
    ...
    /// E.g. the collection of vintage saxophones is characterized by the Concept 'manufactured by hand', while it gets covered by the Concept 'Saxophone' with the Parameter 'Vintage'.</para>
    ...
    ///   <para>owl:ObjectProperty</para>
    ///
    /// labels<para>is characterized by {@en-us}</para><para>is characterized by</para><para>è caratterizzato da</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation stating that an Agent is internally representing a Description . E.g., 'John believes in the conspiracy theory'; 'Niels Bohr created a solar-system metaphor for his atomic theory'; 'Jacques assumes all swans are white'; 'the task force shares the attack plan'.</para>
    ...
    ///   <para>owl:ObjectProperty</para>
    ///   <para>owl:SymmetricProperty</para>
    ///   <para>A relation between two objects participating in a same Event; e.g., 'Vitas and Jimmy are playing tennis'.</para>
    ...
    /// E.g. the collection of vintage saxophones is covered by the Concept 'Saxophone' with the Parameter 'Vintage'.</para>
    ...
    ///   <para>A relation between concepts and collections, where a Concept is said to cover a Collection; it corresponds to a link between the (reified) intensional and extensional interpretations of a (reified) class.
    /// E.g. the collection of vintage saxophones is covered by the Concept 'Saxophone' with the Parameter 'Vintage'.</para>
    /// labels<para>è ricoperto da</para><para>is covered by</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A generic relation holding between a Description and a Concept. In order to be used, a Concept must be previously definedIn another Description. This last condition cannot be encoded for object properties in OWL.</para>
    /// labels<para>usa il concetto</para><para>uses concept</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between a description and a role, e.g. the role 'Ingredient' is defined in the recipe for a cake.</para>
    /// labels<para>è un ruolo definito in</para><para>is role defined in</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>The intransitive follows relation. For example, Wednesday directly precedes Thursday. Directness of precedence depends on the designer conceptualization.</para>
    ...
    /// It can be used between tasks, processes or time intervals, and subproperties would fit best in order to distinguish the different uses.</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>The intransitive precedes relation. For example, Monday directly precedes Tuesday. Directness of precedence depends on the designer conceptualization.</para>
    /// labels<para>precede direttamente</para><para>directly precedes</para></remarks>
    ...
    /// An 'intention' to expand must be present (unless purely formal theories are considered, but even in this case a criterion of relevance is usually active).</para>
    /// labels<para>espande</para><para>expands</para></remarks>
    ...
    ///   <para>owl:ObjectProperty</para>
    ///   <para>owl:SymmetricProperty</para>
    ///   <para>Any relation between descriptions.</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between an InformationObject and a Concept , e.g. the term "dog" expresses the Concept "dog". For expressing a relational meaning, see the more general object property: expresses</para>
    ...
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between an InformationObject and a Concept , e.g. the term "dog" expresses the Concept "dog". For expressing a relational meaning, see the more general object property: expresses</para>
    /// labels<para>è un concetto espresso da</para><para>is concept expressed by</para></remarks>
    ...
    ///   <para>owl:SymmetricProperty</para>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Generic distance relation between any Entity(s). E.g. Rome is far from Beijing, astronomy is far from necromancy.</para>
    ...
    /// For this reason, in this ontology both events and situations are allowed, together with descriptions, in order to encode the modelling needs, independently from the position (if any) chosen by the designer.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Any individual whose existence is granted simply by its social communicability and capability of action (through some PhysicalAgent).</para>
    /// labels<para>Agente sociale</para><para>Social agent</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>A quantity, independently from how it is measured, computed, etc.</para>
    /// labels<para>Quantità</para><para>Amount</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Any Region in a dimensional space that aims at representing time.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Any Region in a dimensional space that is used to localize an Entity ; i.e., it is not used to represent some characteristic (e.g. it excludes time intervals, colors, size values, judgment values, etc.). Differently from a Place , a space region has a specific dimensional space.</para>
    /// labels<para>Regione di spazio</para><para>Space region</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Physical bodies are PhysicalObject(s), for which we tend to neutralize any possible artifactual character. They can have several granularity levels: geological, chemical, physical, biological, etc.</para>
    ...
    /// In order to solve this issue, we need to create a sub-situation s' for the classification time: 'my old cradle is a flower pot in 8June2007'. Such sub-situation s' isPartOf s.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>A piece of information, be it concretely realized or not.</para>
    /// </remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>A concrete realization of an InformationObject, e.g. the written document containing the text of a law.</para>
    /// labels<para>Information realization</para><para>Informazione concreta</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between an information realization and an information object, e.g. the paper copy of the Italian Constitution realizes the text of the Constitution.</para>
    /// labels<para>è realizzato da</para><para>is realized by</para></remarks>
    ...
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between an InformationRealization and a Description, e.g. 'the printout of the Italian Constitution concretelyExpresses the Italian Constitution'. It should be supplied also with a rule stating that the InformationRealization realizes an InformationObject that expresses the Description</para>
    /// labels<para>esprime concretamente</para><para>concretely expresses</para></remarks>
    ...
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between an information realization and an information object, e.g. the paper copy of the Italian Constitution realizes the text of the Constitution.</para>
    /// labels<para>realizza</para><para>realizes</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Anything: real, possible, or imaginary, which some modeller wants to talk about for some purpose.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>A Concept that classifies an Object</para>
    ...
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between an object and a process, e.g. 'John took part in the discussion', 'a large mass of snow fell during the avalanche', or 'a cook, some sugar, flour, etc. are all present in the cooking of a cake'.</para>
    /// labels<para>ha come partecipante</para><para>has participant</para></remarks>
    ...
    /// A PhysicalAgent is a substrate for (actsFor) a Social Agent</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>The Description of a Situation that is desired by an Agent, and usually associated to a Plan that describes how to actually achieve it</para>
    /// labels<para>Scopo</para><para>Goal</para></remarks>
    ...
    /// A same Entity can be given different descriptions, for example, an old cradle can be given a unifying Description based on the original aesthetic design, the functionality it was built for, or a new aesthetic functionality in which it can be used as a flower pot.</para>
    ...
    /// Regions are not data values in the ordinary knowledge representation sense; in order to get patterns for modelling data, see the properties: representsDataValue and hasDataValue</para>
    ...
    /// Conceptualizations can be distinguished into different forms, primarily based on the type of SocialObject that is conceptualized. Descriptions and concepts can be 'assumed', situations can be 'believed' or 'known', plans can be 'adopted', etc. (see ontology: http://www.ontologydesignpatterns.org/ont/dul/Conceptualization.owl.</para>
    ...
    /// The intended semantics (not expressible in OWL) is that a Parameter isParameterFor a Concept that classifies an Entity; moreover, it entails that a Parameter parametrizes a Region that isRegionFor that Entity.</para>
    /// labels<para>has constraint</para><para>ha vincolo</para></remarks>
    ...
    /// Patterns (4) and (5) should be used instead when a constraint or a selection is modeled, independently from the actual observation of values in the real world.</para>
    ...
    /// <remarks>
    ///   <para>owl:DatatypeProperty</para>
    ///   <para>A datatype property that encodes values from xsd:date for an Event; a same Event can have more than one xsd:date value: begin date, end date, date at which the interval holds, as well as dates expressed in different formats: xsd:gYear, xsd:dateTime, etc.</para>
    ...
    /// <remarks>
    ///   <para>owl:DatatypeProperty</para>
    ///   <para>A datatype property that encodes values for a Region, e.g. a float for the Region Height.</para>
    /// labels<para>has region data value</para><para>regione ha valore</para></remarks>
    ...
    ///   <para>owl:ObjectProperty</para>
    ///   <para>owl:SymmetricProperty</para>
    ///   <para>Any relation between concepts, e.g. superordinated, conceptual parthood, having a parameter, having a task, superordination, etc.</para>
    ...
    /// Ordering on subparameters can be created by using or specializing the object property 'precedes'.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Physical value of a physical object, e.g. density, color, etc.</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between situations and entities, e.g. 'this morning I've prepared my coffee with a new fantastic Arabica', i.e.: the preparation of my coffee this morning is the setting for (an amount of) a new fantastic Arabica.</para>
    ...
    /// Subproperties and restrictions can be used to specialize hasPart for objects, events, etc.</para>
    /// labels<para>ha parte</para><para>has part</para></remarks>
    ...
    /// Collectives, facon de parler, can act as agents, although they are not assumed here to be agents (they are even disjoint from the class SocialAgent). This is represented by admitting collectives in the range of the relations having Agent in their domain or range.</para>
    ...
    /// For example, a Plan is a Description of some actions to be executed by agents in a certain way, with certain parameters; a Diagnosis is a Description that provides an interpretation for a set of observed entities, etc.</para>
    ...
    ///   <para>owl:ObjectProperty</para>
    ///
    /// labels<para>è un agente nel contesto di</para><para>is agent included in</para></remarks>
    ...
    ///   <para>owl:ObjectProperty</para>
    ///
    /// labels<para>è un oggetto nel contesto di</para><para>is object included in</para></remarks>
    ...
    /// The classifies relation relates Concept(s) to Entity(s) at some TimeInterval</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>A Concept that isDefinedIn exactly 1 Description. For example, the Concept 'coffee' in a 'preparesCoffee' relation can be defined in that relation, and for all other Description(s) that use it, the isConceptUsedIn property should be applied. Notice therefore that not necessarily all Concept(s) isDefinedIn exactly 1 Description.</para>
    ...
    /// It is different from a Plan, because plans could be carried out in order to follow a method, but a method can be followed by executing alternative plans.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>A person in the physical commonsense intuition: 'have you seen that person walking down the street?'</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Persons in commonsense intuition, which does not apparently distinguish between either natural or social persons.</para>
    /// labels<para>Persona {it}</para><para>Person</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between entities and qualities, e.g. 'Dmitri's skin is yellowish'.</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between an object and a role, e.g. the person 'John' has role 'student'.</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between entities and situations, e.g. 'this morning I've prepared my coffee with a new fantastic Arabica', i.e.: (an amount of) a new fantastic Arabica hasSetting the preparation of my coffee this morning.</para>
    /// labels<para>è nel contesto di</para><para>has setting</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between situations and persons, e.g. 'this morning I've prepared my coffee and had my fingers burnt' (i.e.: the preparation of my coffee this morning included me).</para>
    /// labels<para>include l'agente</para><para>includes agent</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between a Description and a SocialAgent, e.g. a Constitutional Charter introduces the SocialAgent 'PresidentOfRepublic'.</para>
    ...
    /// The isAbout relation is reflexive (not expressible in OWL1.0), because information objects are also about themselves.</para>
    ...
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between information objects and any Entity (including information objects). It can be used to talk about e.g. entities are references of proper nouns: the proper noun 'Leonardo da Vinci' isAbout the Person Leonardo da Vinci; as well as to talk about sets of entities that can be described by a common noun: the common noun 'person' isAbout the set of all persons in a domain of discourse, which can be represented in DOLCE-Ultralite as an individual of the class: Collection .
    /// The isReferenceOf relation is irreflexive, differently from its inverse isAbout.</para>
    ...
    ///   <para>owl:TransitiveProperty</para>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between any entities, e.g.'brain is a part of the human body'.</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A more generic relation holding between a Description and a Concept. In order to be used, a Concept must be previously definedIn another Description</para>
    ...
    /// labels<para>is constraint for</para><para>è un vincolo per</para></remarks>
    ...
    /// An alternative way of representing time is the datatype property: hasIntervalDate</para>
    /// labels<para>is observable at</para><para>è osservabile a</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>The relation between a Parameter, e.g. 'MajorAge', and a Region, e.g. '&gt;17 year'.</para>
    ...
    /// E.g., 'Taking some rest is a postcondition of my search for a hotel'.</para>
    /// labels<para>is postcondition of</para><para>è postcondizione di</para></remarks>
    ...
    /// E.g., 'claiming to find nuclear weapons in a foreign country is a precondition to declare war against it'.</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>The relation between entities and information realizations, e.g. between Italy and a paper copy of the text of the Italian Constitution.</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between an object and a role, e.g. 'student' is the role of 'John'.</para>
    ...
    /// E.g. PhDStudent Role specializes Student Role</para>
    /// labels<para>è specializzato da</para><para>is specialized by</para></remarks>
    ...
    /// Another possible use is between a Collection that isCoveredBy a Concept A, and another Collection that isCoveredBy a Concept B that on its turm specializes A. For example, the 70,000 series Selmer Mark VI saxophone Collection specializes the Selmer Mark VI saxophone Collection.</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Direct succession applied to concepts. E.g. the role 'Officer' is subordinated to 'Director'.</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Direct precedence applied to concepts. E.g. the role 'Executive' is superordinated to 'DepartmentManager'.</para>
    /// labels<para>is superordinated to</para><para>è superordinato a</para></remarks>
    ...
    ///   <para>owl:ObjectProperty</para>
    ///
    /// labels<para>è un tempo nel contesto di</para><para>is time included in</para></remarks>
    ...
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A Collection has a unification criterion, provided by a Description; for example, a community of practice can be unified by a shared theory or interest, e.g. the community that makes research on mirror neurons shares some core knowledge about mirror neurons, which can be represented as a Description MirrorNeuronTheory that unifies the community. There can be several unifying descriptions.</para>
    /// labels<para>è unificato da</para><para>is unified by</para></remarks>
    ...
    ///   <para>owl:ObjectProperty</para>
    ///   <para>owl:SymmetricProperty</para>
    ///   <para>Generic distance relation between any Entity(s). E.g. Rome is near to Florence, astronomy is near to physics.</para>
    ...
    ///
    /// This is only a first step to provide a framework, in which one can model different aspects of meaning. A more developed ontology should approach the problem of integrating the different uses of 'expresses', so that different theories, resources, methods can interoperate.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Any Object that has a proper space region. The prototypical physical object has also an associated mass, but the nature of its mass can greatly vary based on the epistemological status of the object (scientifically measured, subjectively possible, imaginary).</para>
    ...
    /// Being dependent on communication, all social objects need to be expressed by some information object (information object are self-expressing).</para>
    ...
    /// The difference between a CollectiveAgent and an Organization is that a Description that introduces a CollectiveAgent is also one that unifies the corresponding Collective. In practice, this difference makes collective agents 'less stable' than organizations, because they have a dedicated, publicly recognizable Description that is conceived to introduce them.</para>
    /// labels<para>Agente collettivo</para><para>Collective agent</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between collections and entities, e.g. 'my collection of saxophones includes an old Adolphe Sax original alto' (i.e. my collection has member an Adolphe Sax alto).</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between roles and tasks, e.g. 'students have the duty of giving exams' (i.e. the Role 'student' hasTask the Task 'giving exams').</para>
    /// labels<para>ha come obiettivo</para><para>has task</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between situations and actions, e.g. 'this morning I've prepared my coffee and had my fingers burnt' (i.e.: the preparation of my coffee this morning included a burning of my fingers).</para>
    ...
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Agent participation.</para>
    /// labels<para>coinvolge agente</para><para>involves agent</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>The relation holding between a PhysicalAgent and a SocialAgent. In principle, a SocialAgent requires at least one PhysicalAgent in order to act, but this dependency can be 'delegated', e.g. a university can be acted for by a department, which is acted for by physical agents. AKA isActedBy</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>A Plan that defines Role(s), Task(s), and a specific structure for tasks to be executed in relation to goals to be achieved, in order to achieve the main goal of the project. In other words, a project is a plan with a subgoal structure and multiple roles and tasks.</para>
    ...
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between entities and qualities, e.g. 'Dmitri's skin is yellowish'.</para>
    /// labels<para>è una qualità di</para><para>is quality of</para></remarks>
    ...
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between a Description and a SocialAgent, e.g. a Constitutional Charter introduces the SocialAgent 'PresidentOfRepublic'.</para>
    /// labels<para>is introduced by</para><para>è introdotto da</para></remarks>
    ...
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between a Description and a Concept, e.g. a Workflow for a governmental Organization defines the Role 'officer', or 'the Italian Traffic Law defines the role Vehicle'.</para>
    /// labels<para>è definito in</para><para>is defined in</para></remarks>
    ...
    /// In order to encode a specific time, a data value should be related to the TimeInterval.
    /// An alternative way of representing time is the datatype property: hasIntervalDate</para>
    /// labels<para>è il tempo di osservazione di</para><para>is time of observation of</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A Concept can have a Parameter that constrains the attributes that a classified Entity can have in a certain Situation, e.g. a 4WheelDriver Role definedIn the ItalianTrafficLaw has a MinimumAge parameter on the Amount 16.</para>
    /// labels<para>è un parametro per</para><para>is parameter for</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>A piece of information, such as a musical composition, a text, a word, a picture, independently from how it is concretely realized.</para>
    /// labels<para>Oggetto informativo</para><para>Information object</para></remarks>
    ...
    /// While designs typically describe entities to be constructed, they can also be used to describe 'refunctionalized' entities, or to hypothesize unknown functions. For example, a cradle can be refunctionalized as a flowerpot based on a certain home design.</para>
    /// labels<para>Design</para><para>Design</para></remarks>
    ...
    /// Immaterial (non-physical) artifacts (e.g. texts, ideas, cultural movements, corporations, communities, etc. can be modelled as social objects (see SocialObject), which are all 'artifactual' in the weak sense assumed here.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>(The content of) an agreement between at least two agents that play a Party Role, about some contract object (a Task to be executed).</para>
    /// labels<para>Contratto</para><para>Contract</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>A PhysicalArtifact that is also described by a Design. This excludes simple recycling or refunctionalization of natural objects. Most common sense 'artifacts' can be included in this class: cars, lamps, houses, chips, etc.</para>
    /// labels<para>Artefatto progettato</para><para>Designed artifact</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>A Description of the Situation of a system, usually applied in order to control a normal behaviour, or to explain a notable behavior (e.g. a functional breakdown).</para>
    ...
    /// labels<para>ha intervallo temporale</para><para>has time interval</para></remarks>
    ...
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between roles and tasks, e.g. 'students have the duty of giving exams' (i.e. the Role 'student' hasTask the Task 'giving exams').</para>
    /// labels<para>è un obiettivo per</para><para>is task of</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between a Concept and an Entity, e.g. the Role 'student' classifies a Person 'John'.</para>
    ...
    /// The second notion is social, localized in space-time ('social semantics'), and can be used to interpret entities in the intensional sense. For example, the Collection of all quarks isCoveredBy the Concept 'Quark', which is also expressed by the term 'Quark'.</para>
    ...
    /// For a more data-oriented relation, see hasDataValue</para>
    ...
    /// In this sense, an artistic object made of trash or a dose of medicine in the form of a pill would be a FunctionalSubstance, and a DesignedArtifact, since its boundaries are specified by a Design; aleatoric objects that are outcomes of an artistic process might be still considered DesignedArtifact(s), and Substance(s).</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>A CollectiveAgent whose acting agents conceptualize a same SocialRelation .</para>
    ...
    ///   <para>owl:SymmetricProperty</para>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation to encode either formal or informal characterizations of 'boundaries' common to two different entities: an Event that ends when another begins, two abstract regions that have a common topological boundary, two objects that are said to be 'in contact' from a commonsense perspective, etc.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Any social relationship</para>
    /// labels<para>Relazione sociale</para><para>Social relation</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:DatatypeProperty</para>
    ///   <para>A datatype property that encodes values from xsd:date for a TimeInterval; a same TimeInterval can have more than one xsd:date value: begin date, end date, date at which the interval holds, as well as dates expressed in different formats: xsd:gYear, xsd:dateTime, etc.</para>
    ...
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A Concept can have a Parameter that constrains the attributes that a classified Entity can have in a certain Situation, e.g. a 4WheelDriver Role definedIn the ItalianTrafficLaw has a MinimumAge parameter on the Amount 16.</para>
    /// labels<para>has parameter</para><para>ha parametro</para></remarks>
    ...
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Direct succession applied to situations.
    /// E.g., 'A postcondition of our Plan is to have things settled'.</para>
    ...
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Direct precedence applied to situations.
    /// E.g., 'A precondition to declare war against a foreign country is claiming to find nuclear weapons in it'.</para>
