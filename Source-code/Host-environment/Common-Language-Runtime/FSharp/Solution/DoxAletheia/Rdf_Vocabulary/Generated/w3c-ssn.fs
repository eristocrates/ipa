...
module w3c_ssn =
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Total useful life of a battery.</para>
    /// labels<para>Battery Lifetime</para></remarks>
    ...
    ///   <para>The chain here means that if a sensor made an observation and that observation has a result, then the result is the one produced by the sensor.  Just ensures that the sensor and the resulting observation agree on the result.</para>
    /// </remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>The closeness of agreement between the value of an observation and the true value of the observed quality.</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Relation linking an Observation to the Property that was observed.  The observedProperty should be a Property (hasProperty) of the FeatureOfInterest (linked by featureOfInterest) of this observation.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>A sensor outputs a piece of information (an observed value), the value itself being represented by an ObservationValue.</para>
    /// labels<para>Sensor Output</para></remarks>
    ...
    /// labels<para>Survival Property</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>An Entity to which other Entities can be attached - particuarly Sensors and other Platforms.  For example, a post might act as the Platform, a bouy might act as a Platform, or a fish might act as a Platform for an attached sensor.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Place to group all the various Processes related to Deployment.  For example, as well as Deplyment, installation, maintenance, deployment of further sensors and the like would all be classified under DeploymentRelatedProcess.</para>
    /// labels<para>Deployment-related Process</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>A feature is an abstraction of real world phenomena (thing, person, event, etc).</para>
    ...
    ///
    /// can't be used, because (by the OWL2 decidability restrictions) that would mean cardinality restrictions couldn't be applied to observedProperty (see definition of Observation).</para>
    ///   <para>A relation between a FeatureOfInterest and a Property of that feature.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>An identifiable characteristic of the environmental and other conditions in which the sensor is intended to operate.  May include power ranges, power sources, standard configurations, attachments and the like.</para>
    /// labels<para>Operating Property</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>An identifiable and observable characteristic of a sensor's observations or ability to make observations.</para>
    /// labels<para>Measurement Property</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>An observable Quality of an Event or Object.  That is, not a quality of an abstract entity as is also allowed by DUL's Quality, but rather an aspect of an entity that is intrinsic to and cannot exist without the entity and is observable by a sensor.</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Relation between a deployment and the deployed system.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>System is a unit of abstraction for pieces of infrastructure (and we largely care that they are) for sensing. A system has components, its subsystems, which are other systems.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>An observed value for which the probability of falsely claiming the absence of a component in a material is Î², given a probability Î± of falsely claiming its presence.</para>
    /// labels<para>detection limit</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>A, continuous or incremental, change in the reported values of observations over time for an unchanging quality.</para>
    /// labels<para>Drift</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Schedule of maintenance for a system/sensor in the specified conditions.</para>
    /// labels<para>Maintenance Schedule</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Describes the prevailing environmental conditions for MeasurementCapabilites, OperatingConditions and SurvivalRanges.  Used for example to say that a sensor has a particular accuracy in particular conditions.  (see also MeasurementCapability)</para>
    /// labels<para>in condition</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Relation from a MeasurementCapability to a MeasurementProperty.  For example, to an accuracy (see notes at MeasurementCapability).</para>
    /// labels<para>has measurement property</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>The smallest possible time between one observation and the next.</para>
    /// labels<para>MeasurementFrequency</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between an observation and the entity whose quality was observed.   For example, in an observation of the weight of a person, the feature of interest is the person and the quality is weight.</para>
    /// labels<para>feature of interest</para></remarks>
    /// <seealso href="https://www.w3.org/ns/ssn/featureOfInterest">https://www.w3.org/ns/ssn/featureOfInterest</seealso>
    let featureOfInterest = Prefixed_Name(w3c_ssn, "featureOfInterest") |> PrefixedName
    /// <summary>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>A sensor can do (implements) sensing: that is, a sensor is any entity that can follow a sensing method and thus observe some Property of a FeatureOfInterest.  Sensors may be physical devices, computational methods, a laboratory setup with a person following a method, or any other thing that can follow a Sensing Method to observe a Property.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>An Event in the real world that 'triggers' the sensor.  The properties associated to the stimulus may be different to eventual observed property.  It is the event, not the object that triggers the sensor.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>The value of the result of an Observation.  An Observation has a result which is the output of some sensor, the result is an information object that encodes some value for a Feature.</para>
    /// labels<para>Observation Value</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Relation between a System and a Deployment, recording that the System/Sensor was deployed in that Deployment.</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between the description of an algorithm, procedure or method and an entity that implements that method in some executable way.  For example, between a scientific measuring method and a sensor the senses via that method.</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>The result time is the time when the procedure associated with the observation act was applied.</para>
    ///   <para>The result time shall describe the time when the result became available, typically when the procedure associated with the observation was completed For some observations this is identical to the phenomenonTime. However, there are important cases where they differ.[O&amp;M]</para>
    /// labels<para>observation result time</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Rebadged as phenomenon time in [O&amp;M]. The phenomenon time shall describe the time that the result applies to the property of the feature-of-interest. This is often the time of interaction by a sampling procedure or observation procedure with a real-world feature.</para>
    ///   <para>The sampling time is the time that the result applies to the feature-of-interest. This is the time usually required for geospatial analysis of the result.</para>
    /// labels<para>observation sampling time</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Relation linking an Observation to the adjudged quality of the result.  This is of course complimentary to the MeasurementCapability information recorded for the Sensor that made the Observation.</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Relation between a Sensor and Observations it has made.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>The time between a request for an observation and the sensor producing a result (not including network latency to retrieve the result, just time from request to measurement.).</para>
    /// labels<para>MeasurementLatency</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>The description of a process (i.e. describes the temporal and dataflow dependencies and relationships amongst its parts) that results in the estimation, or calculation, of the value of a phenomenon.</para>
    /// labels<para>Sensing</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A (measurement) procedure is a detailed description of a measurement according to one or more measurement principles and to a given measurement method, based on a measurement model and including any calculation to obtain a measurement result [VIM 2.6]</para>
    /// labels<para>sensing method used</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Relation linking an Observation (i.e., a description of the context, the Situation, in which the observatioin was made) and a Result, which contains a value representing the value associated with the observed Property.</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Relation from an OperatingRange to a Property.  For example, to a battery lifetime.</para>
    /// labels<para>has operating property</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Relation between a FeatureOfInterest and a Property (a Quality observable by a sensor) of that feature.</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Relation from a Sensor to a MeasurementCapability describing the measurement properties of the sensor.</para>
    /// labels<para>has measurement capability</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Power range in which system/sensor is expected to operate.</para>
    /// labels<para>Operating Power Range</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Relation between a Platform and a Deployment, recording that the object was used as a platform for a system/sensor for a particular deployment: as in this PhysicalObject is acting as a Platform inDeployment Deployment.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>The closeness of agreement between replicate observations on an unchanged or similar quality value: i.e., a measure of a sensor's ability to consitently reproduce an observation.</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Relation between a Platform and any Systems (e.g., Sensors) that are attached to the Platform.</para>
    /// labels<para>attached system</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>The smallest difference in the value of a quality being observed that would result in perceptably different values of observation results.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Selectivity is a property of a sensor whereby it provides observed values for one or more qualities such that the values of each quality are independent of other qualities in the phenomenon, body, or substance being investigated.</para>
    /// labels<para>Selectivity</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Sensitivity is the quotient of the change in a result of sensor and the corresponding change in a value of a quality being observed.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>The time between a (step) change inthe value of an observed quality and a sensor (possibly with specified error) 'settling' on an observed value.</para>
    ...
    /// labels<para>Sensing Device</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation between an entity that implements a method in some executable way and the description of an algorithm, procedure or method.  For example, between a Sensor and the scientific measuring method that the Sensor uses to observe a Property.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>The conditions a sensor can be exposed to without damage: i.e., the sensor continues to operate as defined using MeasurementCapability.  If, however, the SurvivalRange is exceeded, the sensor is 'damaged' and MeasurementCapability specifications may no longer hold.</para>
    /// labels<para>Survival Range</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Haspart relation between a system and its parts.</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A Relation from a System to a SurvivalRange.</para>
    /// labels<para>has survival range</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Relation between a producer and a produced entity: for example, between a sensor and the produced output.</para>
    ...
    /// The Stimulus itself will be serving as a proxy for (see isProxyOf) some observable property.</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>A relation from a Stimulus to the Property that the Stimulus is serving as a proxy for.  For example, the expansion of the quicksilver is a stimulus that serves as a proxy for temperature, or an increase or decrease in the spinning of cups on a wind sensor is serving as a proxy for wind speed.</para>
    /// labels<para>isProxyFor</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Relation from a SurvivalRange to a Property describing the survial range of a system.  For example, to the temperature extreme that a system can withstand before being considered damaged.</para>
    /// labels<para>has survival property</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Relation from a System to an OperatingRange describing the normal operating environment of the System.</para>
    /// labels<para>has operating range</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Relation between a System (e.g., a Sensor) and a Platform.  The relation locates the sensor relative to other described entities entities: i.e., the Sensor s1's location is Platform p1.  More precise locations for sensors in space (relative to other entities, where attached to another entity, or in 3D space) are made using DOLCE's Regions (SpaceRegion).</para>
    /// labels<para>on platform</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Total useful life of a sensor/system (expressed as total life since manufacture, time in use, number of operations, etc.).</para>
    /// labels<para>System Lifetime</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Relation between a deployment and the platform on which the system was deployed.</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>Has part relation between a deployment process and its constituent processes.</para>
    /// labels<para>deployment process part</para></remarks>
    ...
    ///
    /// The conditions specified here are those that affect the measurement properties, while those in OperatingRange represent the sensor's standard operating conditions, including conditions that don't affect the observations.</para>
    /// labels<para>Measurement Capability</para></remarks>
