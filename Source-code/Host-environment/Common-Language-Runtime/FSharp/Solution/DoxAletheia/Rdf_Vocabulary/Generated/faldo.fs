...
module faldo =
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>The C-terminus is the end of an amino acid chain (protein or polypeptide), terminated by a free carboxyl group (-COOH).</para>
    /// labels<para>C-Terminal position</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Used to describe a location that consists of a number of Regions but where the order is not known. e.g. the oddly named order() keyword in a INSDC file.</para>
    /// labels<para>Bag of regions</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Part of the coordinate system denoting on which strand the feature can be found. If you do not yet know which stand the feature is on, you should tag the position with just this class. If you know more you should use one of the subclasses. This means a region described with a '.' in GFF3. A GFF3 unstranded position does not have this type in FALDO -- those are just a 'position'.</para>
    /// labels<para>Stranded position</para></remarks>
    ...
    ///   <para>Denoted in 1-based closed coordinates, i.e. the position on the first amino acid or nucleotide of a sequence has the value 1. For nucleotide sequences we count from the 5'end of the sequence, while for Aminoacid sequences we start counting from the N-Terminus.</para>
    ///   <para>The position value is the offset along the reference where this position is found. Thus the only the position value in combination with the reference determines where a position is.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>A position that lacks exact data.</para>
    /// labels<para>Fuzzy position</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>The 'both strands position' indicates a region that is best described as being on 'both' strands of a double-stranded sequence, rather than one or the other.</para>
    /// labels<para>Both strands position</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>A region describes a length of sequence with a start position and end position that represents a feature on a sequence, e.g. a gene.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>This indicates that a feature is between two other positions that are both known exactly and that are next to each other. An example is a restriction enzyme cutting site. The cut is after one position and before the other position (hence, in between).</para>
    /// labels<para>In between positions</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Use when you have an idea of the range in which you can find the position, but you cannot be sure about the exact position.</para>
    /// labels<para>Indeterminate position within a range</para></remarks>
    ...
    ///   <para>The inclusive beginning of a position. Also known as start.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>As an ordered list of regions (but the list might not be complete).</para>
    ///   <para>Should be used when the location of a region is defined by an ordered list of Regions. However, try to avoid using these types in favor of using more explicit semantics about why the order is important.</para>
    /// labels<para>List of regions</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>A position that is exactly known.</para>
    /// labels<para>Exact position</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>The position of the starting amino-acid a protein or polypeptide terminated by an amino acid with a free amine group (-NH2). The convention for writing peptide sequences is to put the N-terminus on the left and write the sequence from N- to C-terminus. Instances of this class are often used when the reference sequence is not complete </para>
    /// </remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>The position is known to be one of the more detailed positions listed by the location predicate.</para>
    /// labels<para>One of positions</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Superclass for the general concept of a position on a sequence. The sequence is designated with the reference predicate.</para>
    /// labels<para>Position</para></remarks>
    /// <seealso href="http://biohackathon.org/resource/faldo#Position">http://biohackathon.org/resource/faldo#Position</seealso>
    let Position = Prefixed_Name(faldo, "Position") |> PrefixedName
    /// <summary>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>The reference is the resource that the position value is anchored to.  For example, a contig or chromosome in a genome assembly.</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>This predicate is used to indicate that the feature is found before the exact position. Use to indicate, for example, a cleavage site. The cleavage happens between two amino acids before one and after the other.</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>This is the inverse of the begin:end. It is included to make it easier to write a number of OWL axioms. You should rarely use this in your raw data.</para>
    /// labels<para>endOf</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>This is the inverse of the begin:property. It is included to make it easier to write a number of OWL axioms. You should rarely use this in your raw data.</para>
    /// labels<para>beginOf</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>This is the link between the concept whose location you are annotating and its range or position. For example, when annotating the region that describes an exon, the exon would be the subject and the region would be the object of the triple or: 'active site' 'location' [is] 'position 3'.</para>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>Sometimes a location of a feature is defined by a collection of regions e.g. join() and order() in INSDC records. One should always try to model the semantics more accurately than this, these are fallback options to encode legacy data.</para>
    /// labels<para>Collection of regions</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>The position is on the forward (positive, 5' to 3') strand. Shown as a '+' in GFF3 and GTF.</para>
    /// labels<para>Forward/positive strand position</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:Class</para>
    ///   <para>The position is on the reverse (complement, 3' to 5') strand of the sequence. Shown as '-' in GTF and GFF3.</para>
    /// labels<para>Negative/reverse strand position</para></remarks>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>This predicate is used when you want to describe a non-inclusive range. Only used in the InBetweenPosition to say it is after a nucleotide, but before the next one.</para>
    ...
    /// <remarks>
    ///   <para>owl:ObjectProperty</para>
    ///   <para>The inclusive end of the position.</para>
