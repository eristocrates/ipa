...
module lexinfov3 =
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>bijvoeglijk naamwoord</para><para>прилагателно име</para><para>přídavné jméno</para><para>прилагательное</para><para>īpašības vārds</para><para>prídavné meno</para><para>lýsingarorð</para><para>przymiotnik</para><para>прикметник</para><para>adjektiivi</para><para>tillægsord</para><para>omadussõna</para><para>pridevnik</para><para>adxectivo</para><para>adjektibo</para><para>прыметнік</para><para>adjective</para><para>būdvardis</para><para>aggettivo</para><para>melléknév</para><para>придавка</para><para>adjektiv</para><para>adjetivo</para><para>Adjektiv</para><para>adjectif</para><para>aidiacht</para><para>adjektiv</para><para>adjectiu</para><para>aġġettiv</para><para>adjectiv</para><para>adjetivo</para><para>pridjevi</para><para>adjektiv</para><para>שם תואר</para><para>επίθετο</para><para>mbiemri</para><para>придев</para></remarks>
    ...
    /// labels<para>kategoria morfosintaktiko</para><para>categoría gramatical</para><para>categoria gramatical</para><para>categoría gramatical</para><para>γραμματική κατηγορία</para><para>parte del discorso</para><para>classe gramatical</para><para>parte de vorbire</para><para>klasë e fjalëve</para><para>зборовна група</para><para>part of speech</para><para>част на речта</para><para>besedna vrsta</para><para>часціны мовы</para><para>kalbos dalis</para><para>частина мови</para><para>slovný druh</para><para>slovní druh</para><para>część mowy</para><para>woordsoort</para><para>חלקי הדיבר</para><para>врста речи</para><para>sanaluokka</para><para>часть речи</para><para>orðflokkur</para><para>ordklasse</para><para>ordklasse</para><para>vārdšķira</para><para>ordklasse</para><para>ordklass</para><para>sõnaliik</para><para>Wortart</para><para>szófaj</para><para>nature</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>apposizione</para><para>adposisjon</para><para>adposition</para><para>adposition</para><para>forsetning</para><para>adposition</para><para>adposition</para><para>Adposition</para><para>Prepoziție</para><para>адпозиција</para><para>adposition</para><para>adpozicija</para><para>adposición</para><para>adpositio</para><para>aposición</para><para>parafjalë</para><para>adposizio</para><para>adposició</para><para>adposição</para><para>adpozíció</para><para>przyimek</para><para>kaassõna</para><para>מילת יחס</para><para>πρόθεση</para><para>záložka</para><para>афикс</para><para>адлог</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>apstākļa vārds</para><para>prieveiksmis</para><para>dobhriathar</para><para>határozószó</para><para>прислівник</para><para>przysłówek</para><para>příslovce</para><para>ndajfolje</para><para>atviksorð</para><para>תואר פועל</para><para>príslovka</para><para>advérbio</para><para>adverbio</para><para>adberbio</para><para>adverbio</para><para>määrsõna</para><para>επίρρημα</para><para>прыслоўе</para><para>avverbio</para><para>bijwoord</para><para>наречие</para><para>prilozi</para><para>adverbi</para><para>adverbe</para><para>prislov</para><para>наречие</para><para>adverbi</para><para>adverb</para><para>прилог</para><para>adverb</para><para>Adverb</para><para>adverb</para><para>avverb</para><para>adverb</para><para>adverb</para><para>прилог</para><para>biord</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>determinativt pronomen</para><para>определяющее слово</para><para>määrav artikkel</para><para>determinatzaile</para><para>determinantas</para><para>προσδιοριστής</para><para>determinator</para><para>determinante</para><para>Determinativ</para><para>determinante</para><para>determinátor</para><para>determinativ</para><para>determinativ</para><para>determinante</para><para>детерминатор</para><para>determinativ</para><para>determinante</para><para>determinant</para><para>determináns</para><para>déterminant</para><para>determinant</para><para>Determinant</para><para>determiner</para><para>ákvæðisorð</para><para>визначник</para><para>określnik</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>граматички члан</para><para>членуване</para><para>przedimek</para><para>artikelis</para><para>artikkeli</para><para>artículo</para><para>artikuls</para><para>lidwoord</para><para>artikkel</para><para>artikulu</para><para>kendeord</para><para>articolo</para><para>artikkel</para><para>artikel</para><para>articol</para><para>артикль</para><para>article</para><para>artiklu</para><para>Artikel</para><para>greinir</para><para>Artikel</para><para>артыкль</para><para>артикль</para><para>article</para><para>article</para><para>artigo</para><para>artigo</para><para>trajta</para><para>névelő</para><para>תווית</para><para>άρθρο</para><para>член</para><para>član</para><para>člen</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>determinativt pronomen</para><para>определяющее слово</para><para>determinatzaile</para><para>määrav artikkel</para><para>determinantas</para><para>προσδιοριστής</para><para>determinativ</para><para>determinante</para><para>determinativ</para><para>детерминатор</para><para>Determinativ</para><para>determinante</para><para>determinante</para><para>determinator</para><para>determinante</para><para>determinátor</para><para>determinativ</para><para>determinant</para><para>Determinant</para><para>déterminant</para><para>determinant</para><para>determináns</para><para>determiner</para><para>ákvæðisorð</para><para>określnik</para><para>визначник</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>aspecte gramatical</para><para>aspecto gramatical</para><para>aspecto gramatical</para><para>Ποιόν ενέργειας</para><para>Вид на глагола</para><para>Glagolski vid</para><para>глаголски вид</para><para>igeszemlélet</para><para>Slovesný vid</para><para>slovesný vid</para><para>вид глагола</para><para>veikslas</para><para>aspecto</para><para>aspekti</para><para>aspetto</para><para>Aspekt</para><para>aspect</para><para>Aspekt</para><para>aspekt</para><para>aspect</para><para>aspekt</para><para>aspekt</para><para>aspect</para><para>Aspect</para><para>aspekt</para><para>אספקט</para><para>вид</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>pagalbinis veiksmažodis</para><para>вспомогательный глагол</para><para>czasownik posiłkowy</para><para>допоміжне дієслово</para><para>aditz laguntzaile</para><para>verbe auxiliaire</para><para>pomocné sloveso</para><para>verb awżiljarju</para><para>pomožni glagol</para><para>verbo auxiliar</para><para>помошен глагол</para><para>verbo auxiliar</para><para>verbo auxiliar</para><para>briathar cúnta</para><para>помоћни глагол</para><para>hulpwerkwoord</para><para>Verb auxiliar</para><para>hjælpeverbum</para><para>abitegusõna</para><para>hjálparsögn</para><para>hjelpeverb</para><para>hjelpeverb</para><para>Hilfsverb</para><para>auxiliary</para><para>ausiliare</para><para>hjälpverb</para><para>βοηθητικό</para><para>segédige</para><para>פועל עזר</para><para>auxiliar</para><para>apuverbi</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>darbības vārds</para><para>veiksmažodis</para><para>udsagnsord</para><para>werkwoord</para><para>czasownik</para><para>дзеяслоў</para><para>briathar</para><para>tegusõna</para><para>дієслово</para><para>sagnorð</para><para>sloveso</para><para>glagoli</para><para>sloveso</para><para>глагол</para><para>глагол</para><para>глагол</para><para>глагол</para><para>glagol</para><para>verbi</para><para>folje</para><para>verbo</para><para>verbo</para><para>aditz</para><para>verbo</para><para>verbe</para><para>verbo</para><para>Verb</para><para>verb</para><para>verb</para><para>ρήμα</para><para>פועל</para><para>Verb</para><para>verb</para><para>verb</para><para>verb</para><para>verb</para><para>verb</para><para>ige</para></remarks>
    ...
    /// labels<para>pagalbinis veiksmažodis</para><para>вспомогательный глагол</para><para>czasownik posiłkowy</para><para>допоміжне дієслово</para><para>aditz laguntzaile</para><para>verbe auxiliaire</para><para>verb awżiljarju</para><para>pomocné sloveso</para><para>briathar cúnta</para><para>помошен глагол</para><para>verbo auxiliar</para><para>verbo auxiliar</para><para>помоћни глагол</para><para>pomožni glagol</para><para>verbo auxiliar</para><para>hulpwerkwoord</para><para>Verb auxiliar</para><para>hjælpeverbum</para><para>hjálparsögn</para><para>abitegusõna</para><para>hjelpeverb</para><para>hjelpeverb</para><para>hjälpverb</para><para>auxiliary</para><para>βοηθητικό</para><para>Hilfsverb</para><para>ausiliare</para><para>פועל עזר</para><para>apuverbi</para><para>auxiliar</para><para>segédige</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#auxiliary">http://www.lexinfo.net/ontology/3.0/lexinfo#auxiliary</seealso>
    let auxiliary = Prefixed_Name(lexinfov3, "auxiliary") |> PrefixedName
    /// <summary>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>szimbólum</para><para>Simbolis</para><para>siombail</para><para>σύμβολο</para><para>símbolo</para><para>símbolo</para><para>Simboli</para><para>symbole</para><para>simbolo</para><para>símbolo</para><para>simbols</para><para>symbool</para><para>symboli</para><para>simbol</para><para>симбол</para><para>symbol</para><para>Symbol</para><para>símbol</para><para>символ</para><para>симбол</para><para>символ</para><para>symbol</para><para>sümbol</para><para>Simbol</para><para>symbol</para><para>Символ</para><para>Simbol</para><para>сімвал</para><para>symbol</para><para>symbol</para><para>symbol</para><para>symbol</para><para>symbol</para><para>ikur</para><para>Tákn</para><para>סמל</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>имя числительное</para><para>числително име</para><para>skaitļa vārds</para><para>zenbatzaile</para><para>skaitvardis</para><para>αριθμητικό</para><para>числівник</para><para>liczebnik</para><para>numeraali</para><para>numerale</para><para>räkneord</para><para>číslovka</para><para>číslovka</para><para>numerale</para><para>лічэбнік</para><para>telwoord</para><para>Numerale</para><para>arvsõna</para><para>števnik</para><para>Numeral</para><para>számnév</para><para>нумерал</para><para>Numeral</para><para>brojevi</para><para>numeral</para><para>numeral</para><para>numeral</para><para>tallord</para><para>numeral</para><para>numeral</para><para>numéral</para><para>töluorð</para><para>talord</para><para>מספר</para><para>број</para></remarks>
    ...
    /// labels<para>количественное числительное</para><para>kiekinis skaitvardis</para><para>zenbatzaile kardinal</para><para>adjetivo cardinal</para><para>číslovka základní</para><para>liczebnik główny</para><para>numeral cardinal</para><para>απόλυτος αριθμός</para><para>numeral cardinal</para><para>cardinal numeral</para><para>numero cardinale</para><para>Glavni števnik</para><para>Hoofdtelwoord</para><para>Kardinalzahl</para><para>основни број</para><para>mængdetal</para><para>grunntall</para><para>tőszámnév</para><para>Grundtal</para><para>frumtala</para><para>cardinal</para><para>põhiarv</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>Rasa gramatikore</para><para>caso gramatical</para><para>kasu gramatikal</para><para>caso gramatical</para><para>cas grammatical</para><para>caso gramatical</para><para>cas gramatical</para><para>mluvnický pád</para><para>przypadek</para><para>locījums</para><para>відмінок</para><para>linksnis</para><para>naamval</para><para>kasus</para><para>kasus</para><para>падеж</para><para>падеж</para><para>kääne</para><para>sklon</para><para>Kasus</para><para>kasus</para><para>падеж</para><para>kasus</para><para>padež</para><para>падеж</para><para>склон</para><para>πτώση</para><para>case</para><para>sija</para><para>caso</para><para>fall</para><para>יחסה</para><para>eset</para><para>caz</para><para>pád</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>circumposición</para><para>circumposition</para><para>zirkumposizio</para></remarks>
    ...
    /// labels<para>kommatecken</para><para>Przecinek</para><para>Kablelis</para><para>Virgulă</para><para>vírgula</para><para>Запетая</para><para>virgola</para><para>Запирка</para><para>virgule</para><para>запятая</para><para>Presja</para><para>pilkku</para><para>Komats</para><para>vessző</para><para>Čiarka</para><para>Vejica</para><para>komma</para><para>Camóg</para><para>komma</para><para>коска</para><para>Komma</para><para>Komma</para><para>Зарез</para><para>komma</para><para>Zarez</para><para>čárka</para><para>comma</para><para>Komma</para><para>Κόμμα</para><para>פסיק</para><para>coma</para><para>Кома</para><para>coma</para><para>Coma</para><para>Koma</para><para>Koma</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>bendrinis daiktavardis</para><para>rzeczownik pospolity</para><para>заједничка именица</para><para>имя нарицательное</para><para>іменник загальний</para><para>substantiv comun</para><para>všeobecné meno</para><para>хадзячая назва</para><para>nombre común</para><para>izen arrunt</para><para>nome comune</para><para>common noun</para><para>appellativ</para><para>appellativ</para><para>sugasvārds</para><para>Appellativ</para><para>ουσιαστικό</para><para>nome comum</para><para>fellesnavn</para><para>nome común</para><para>apelativum</para><para>nom commun</para><para>yleisnimi</para><para>soortnaam</para><para>občno ime</para><para>nom comú</para><para>üldnimi</para><para>samnamn</para><para>samnafn</para><para>köznév</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>partícula comparativa</para><para>partikula konparatibo</para><para>comparative particle</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>congiunzione</para><para>conjunction</para><para>Konjunktion</para><para>konjunktion</para><para>konjunksjon</para><para>konġunzjoni</para><para>konjunktion</para><para>konjunksjon</para><para>conjonction</para><para>conjuncție</para><para>conjunción</para><para>samtenging</para><para>konjunktio</para><para>conxunción</para><para>מילת קישור</para><para>juntagailu</para><para>bindvlies</para><para>conjunção</para><para>jungtukas</para><para>σύνδεσμος</para><para>сполучник</para><para>conjunció</para><para>sidesõna</para><para>kötőszó</para><para>злучнік</para><para>lidhëza</para><para>spójnik</para><para>сврзник</para><para>veznici</para><para>saiklis</para><para>везник</para><para>spojka</para><para>veznik</para><para>spojka</para><para>съюз</para><para>союз</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>Hyppighet i statistikk</para><para>Релативна фреквенција</para><para>Frekvens i statistikk</para><para>Частота події</para><para>частотность</para><para>фреквенција</para><para>frekwencja</para><para>freqüència</para><para>frecuencia</para><para>frecuencia</para><para>frequentie</para><para>gyakoriság</para><para>frequência</para><para>Häufigkeit</para><para>frekvenca</para><para>Maiztasun</para><para>συχνότητα</para><para>frequenza</para><para>frecvență</para><para>frequency</para><para>fréquence</para><para>frekvens</para><para>dažnumas</para><para>sagedus</para><para>četnost</para><para>Частата</para><para>שכיחות</para><para>tíðni</para></remarks>
    ...
    /// labels<para>неопределённое местоимение</para><para>Onbepaald voornaamwoord</para><para>неозначений займенник</para><para>pronombre indefinido</para><para>Neodređene zamjenice</para><para>nežymimasis įvardis</para><para>zaimek nieokreślony</para><para>határozatlan névmás</para><para>izenordain mugagabe</para><para>неодређена заменица</para><para>indefinite pronoun</para><para>umbmäärane asesõna</para><para>pronome indefinito</para><para>indefinit pronomen</para><para>indefinit pronomen</para><para>pronome indefinido</para><para>pronome indefinido</para><para>αόριστη αντωνυμία</para><para>Indefinitpronomen</para><para>Pronume nehotărât</para><para>óákveðið fornafn</para><para>pronom indefinit</para><para>neurčité zájmeno</para><para>pronom indéfini</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>pasthirrma - themelet e ngurtësuara të gjuhës</para><para>izsauksmes vārds</para><para>tussenwerpsel</para><para>inteġrezzjoni</para><para>interjection</para><para>interjeksjon</para><para>Interjektion</para><para>interjektion</para><para>interxección</para><para>interjeksjon</para><para>interjection</para><para>interiezione</para><para>interjección</para><para>interjektion</para><para>interjecció</para><para>interjecție</para><para>interjeição</para><para>interjekzio</para><para>interjektio</para><para>междуметие</para><para>citoslovce</para><para>междометие</para><para>citoslovce</para><para>מילת קריאה</para><para>indulatszó</para><para>wykrzyknik</para><para>выклічнік</para><para>επιφώνημα</para><para>intriacht</para><para>jaustukas</para><para>upphrópun</para><para>hüüdsõna</para><para>usklici</para><para>medmet</para><para>извик</para><para>вигук</para><para>узвик</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>имя числительное</para><para>числително име</para><para>skaitļa vārds</para><para>skaitvardis</para><para>zenbatzaile</para><para>αριθμητικό</para><para>liczebnik</para><para>числівник</para><para>numeraali</para><para>räkneord</para><para>numerale</para><para>лічэбнік</para><para>numerale</para><para>číslovka</para><para>telwoord</para><para>Numerale</para><para>číslovka</para><para>нумерал</para><para>számnév</para><para>numeral</para><para>numeral</para><para>numéral</para><para>števnik</para><para>töluorð</para><para>numeral</para><para>arvsõna</para><para>brojevi</para><para>tallord</para><para>numeral</para><para>Numeral</para><para>Numeral</para><para>numeral</para><para>talord</para><para>מספר</para><para>број</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>voornaamwoordelijk bijwoord</para><para>местоименное наречие</para><para>izenordain-adberbio</para><para>pronominaladverbium</para><para>adverbio pronominal</para><para>adverbe pronominal</para><para>pronominal adverb</para><para>pronominaladverb</para><para>Pronominaladverb</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>Bară oblică spre dreapta</para><para>Pasvirasis brūkšnys</para><para>Schuine streep</para><para>Barra diagonal</para><para>barra obliqua</para><para>barra obliqua</para><para>barre oblique</para><para>Schrägstrich</para><para>Скісна риска</para><para>косая черта</para><para>ferde vonal</para><para>snedstreck</para><para>Коса црта</para><para>Коса црта</para><para>skråstrek</para><para>Skråstreg</para><para>Poševnica</para><para>Vinoviiva</para><para>Kosa crta</para><para>lomítko</para><para>Ukośnik</para><para>לוכסן</para><para>Lomka</para><para>barra</para><para>slash</para><para>Barra</para><para>barra</para></remarks>
    ...
    /// labels<para>Bară oblică spre dreapta</para><para>Pasvirasis brūkšnys</para><para>Barra diagonal</para><para>Schuine streep</para><para>barra obliqua</para><para>barra obliqua</para><para>barre oblique</para><para>Скісна риска</para><para>Schrägstrich</para><para>ferde vonal</para><para>косая черта</para><para>snedstreck</para><para>Коса црта</para><para>Kosa crta</para><para>skråstrek</para><para>Poševnica</para><para>Skråstreg</para><para>Коса црта</para><para>Vinoviiva</para><para>lomítko</para><para>Ukośnik</para><para>Barra</para><para>לוכסן</para><para>Lomka</para><para>barra</para><para>barra</para><para>slash</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#slash">http://www.lexinfo.net/ontology/3.0/lexinfo#slash</seealso>
    let slash = Prefixed_Name(lexinfov3, "slash") |> PrefixedName
    /// <summary>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>darbības vārds</para><para>veiksmažodis</para><para>udsagnsord</para><para>werkwoord</para><para>czasownik</para><para>briathar</para><para>дзеяслоў</para><para>tegusõna</para><para>дієслово</para><para>sagnorð</para><para>glagoli</para><para>sloveso</para><para>sloveso</para><para>глагол</para><para>glagol</para><para>глагол</para><para>глагол</para><para>глагол</para><para>verbi</para><para>verbo</para><para>verbo</para><para>verbo</para><para>folje</para><para>verbo</para><para>verbe</para><para>aditz</para><para>verb</para><para>Verb</para><para>verb</para><para>verb</para><para>verb</para><para>Verb</para><para>verb</para><para>verb</para><para>פועל</para><para>verb</para><para>ρήμα</para><para>ige</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>Darbības vārda kārta</para><para>грамматический залог</para><para>Glagolski način</para><para>actief/passief</para><para>voz gramatical</para><para>veu gramatical</para><para>slovesný rod</para><para>slovesný rod</para><para>מעמד דקדוקי</para><para>voz verbal</para><para>Sagnmyndir</para><para>Pääluokka</para><para>diathèse</para><para>Tegumood</para><para>Diathese</para><para>Diateză</para><para>Diatese</para><para>diatese</para><para>diatesi</para><para>diatese</para><para>Diateza</para><para>strona</para><para>diates</para><para>igenem</para><para>Залог</para><para>rūšis</para><para>voice</para><para>Залог</para><para>Φωνή</para><para>стан</para><para>voz</para></remarks>
    ...
    /// labels<para>definitud gramatical</para><para>определённость</para><para>definiteness</para><para>определеност</para><para>визначеність</para><para>określoność</para><para>Definitheit</para><para>bestemthed</para><para>määräisyys</para><para>bestemthet</para><para>určitost</para><para>species</para><para>binding</para><para>Species</para><para>יידוע</para></remarks>
    ...
    /// labels<para>determinativt pronomen</para><para>определяющее слово</para><para>määrav artikkel</para><para>determinatzaile</para><para>προσδιοριστής</para><para>determinantas</para><para>determinante</para><para>детерминатор</para><para>determinator</para><para>determinante</para><para>determinante</para><para>Determinativ</para><para>determinativ</para><para>determinátor</para><para>determinativ</para><para>determinativ</para><para>determinante</para><para>determináns</para><para>determinant</para><para>déterminant</para><para>determinant</para><para>Determinant</para><para>determiner</para><para>ákvæðisorð</para><para>визначник</para><para>określnik</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#determiner">http://www.lexinfo.net/ontology/3.0/lexinfo#determiner</seealso>
    let determiner = Prefixed_Name(lexinfov3, "determiner") |> PrefixedName
    /// <summary>
    ...
    ///
    /// labels<para>Innskeyti</para><para>Intarpas</para><para>wrostek</para><para>erdizki</para><para>infisso</para><para>infiks</para><para>инфикс</para><para>infixo</para><para>infixe</para><para>інфікс</para><para>infiks</para><para>Infixo</para><para>infijo</para><para>Infiks</para><para>инфикс</para><para>Инфикс</para><para>infix</para><para>infix</para><para>infix</para><para>Infix</para><para>infix</para><para>Vpona</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#infix">http://www.lexinfo.net/ontology/3.0/lexinfo#infix</seealso>
    let infix = Prefixed_Name(lexinfov3, "infix") |> PrefixedName
    ...
    /// labels<para>proprietate morfosintactică a persoanei</para><para>darbības vārda persona</para><para>ιδιότητα του προσώπου</para><para>proprietà di persona</para><para>pertsona gramatikal</para><para>persona gramatical</para><para>grammatisk person</para><para>persoa gramatical</para><para>grammatisk person</para><para>pessoa gramatical</para><para>grammatisk person</para><para>Veta gramatikore</para><para>граматична особа</para><para>Slovnična oseba</para><para>asmens požymis</para><para>personne</para><para>persoona</para><para>persóna</para><para>persona</para><para>persoon</para><para>személy</para><para>person</para><para>Person</para><para>person</para><para>osoba</para><para>Osoba</para><para>osoba</para><para>лице</para><para>лице</para><para>лицо</para><para>isik</para><para>גוף</para></remarks>
    ...
    ///   <para>lexinfov3:TermElement</para>
    ///
    /// labels<para>prapashtesa</para><para>przyrostek</para><para>Viðskeyti</para><para>piedēklis</para><para>Наставка</para><para>suffiksi</para><para>наставка</para><para>Priesaga</para><para>suffisso</para><para>suffiss</para><para>suffiks</para><para>επίθημα</para><para>Pripona</para><para>prípona</para><para>суффикс</para><para>suffiks</para><para>atzizki</para><para>suffiks</para><para>Iarmhír</para><para>suffixe</para><para>suffix</para><para>suffix</para><para>суфикс</para><para>Suffix</para><para>суфікс</para><para>sufijo</para><para>суфікс</para><para>Sufixo</para><para>sufiks</para><para>sufixo</para><para>Sufiks</para><para>suffix</para><para>סיומת</para><para>sufix</para><para>sufix</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>bijvoeglijk naamwoord</para><para>прилагателно име</para><para>přídavné jméno</para><para>īpašības vārds</para><para>прилагательное</para><para>prídavné meno</para><para>lýsingarorð</para><para>przymiotnik</para><para>tillægsord</para><para>omadussõna</para><para>adjektiivi</para><para>прикметник</para><para>būdvardis</para><para>aggettivo</para><para>прыметнік</para><para>adxectivo</para><para>melléknév</para><para>adjective</para><para>pridevnik</para><para>adjektibo</para><para>adjektiv</para><para>pridjevi</para><para>aġġettiv</para><para>adjectiv</para><para>adjectif</para><para>придавка</para><para>aidiacht</para><para>adjektiv</para><para>adjetivo</para><para>adjectiu</para><para>adjektiv</para><para>Adjektiv</para><para>adjetivo</para><para>επίθετο</para><para>mbiemri</para><para>שם תואר</para><para>придев</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>įvardinis, įvardžiuotinis</para><para>voornaamwoord</para><para>vietniekvārds</para><para>местоимённый</para><para>pronominale</para><para>asesõnaline</para><para>местоимение</para><para>izenordain</para><para>pronominal</para><para>pronominal</para><para>pronominal</para><para>pronominal</para><para>pronominal</para><para>pronomini</para><para>כינוי גוף</para><para>zamjenice</para><para>αντωνυμία</para><para>займеннік</para><para>займенник</para><para>zaimkowy</para><para>pronomen</para><para>Pronomen</para><para>pronomen</para><para>заменица</para><para>pronomen</para><para>pronome</para><para>zájmeno</para><para>fornafn</para><para>forainm</para><para>заменка</para><para>pronoun</para><para>përemër</para><para>zaimek</para><para>pronom</para><para>névmás</para><para>zámeno</para><para>pronom</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>partícula afirmativa</para><para>affirmative particle</para><para>baieztapen partikula</para></remarks>
    ...
    /// labels<para>articolo determinativo</para><para>определенный артикль</para><para>žymimasis artikelis</para><para>przedimek określony</para><para>bestimmter Artikel</para><para>articolul hotărât</para><para>határozott névelő</para><para>artículo definido</para><para>artikulu mugatua</para><para>definite article</para><para>Bepaald lidwoord</para><para>ákveðinn greinir</para><para>bestemt artikel</para><para>määrav artikkel</para><para>artigo definido</para><para>article definit</para><para>artigo definido</para><para>article défini</para><para>οριστικό άρθρο</para><para>одређени члан</para><para>člen určitý</para><para>הא הידיעה</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>Aanwijzend voornaamwoord</para><para>указательное местоимение</para><para>demonstratiivipronomini</para><para>demonstrativt pronomen</para><para>pronome demonstrativo</para><para>demonstrative pronoun</para><para>Demonstrativpronomen</para><para>demonstrativpronomen</para><para>Pronume demonstrativ</para><para>izenordain erakusle</para><para>pronom démonstratif</para><para>вказівний займенник</para><para>ukazovací zájmeno</para><para>ábendingarfornafn</para><para>Pokazne zamjenice</para><para>zaimek wskazujący</para><para>показна заменица</para><para>näitav asesõna</para><para>mutató névmás</para><para>demostrativo</para><para>demonstrativ</para><para>demonstrativ</para><para>dimostrativo</para><para>demostrativo</para><para>parodomasis</para><para>demostratiu</para><para>כינוי רמז</para></remarks>
    ...
    /// labels<para>Aanwijzend voornaamwoord</para><para>указательное местоимение</para><para>demonstratiivipronomini</para><para>demonstrativt pronomen</para><para>pronome demonstrativo</para><para>demonstrative pronoun</para><para>demonstrativpronomen</para><para>Demonstrativpronomen</para><para>Pronume demonstrativ</para><para>вказівний займенник</para><para>izenordain erakusle</para><para>pronom démonstratif</para><para>ukazovací zájmeno</para><para>zaimek wskazujący</para><para>Pokazne zamjenice</para><para>ábendingarfornafn</para><para>показна заменица</para><para>näitav asesõna</para><para>mutató névmás</para><para>demonstrativ</para><para>demostrativo</para><para>demostrativo</para><para>dimostrativo</para><para>demonstrativ</para><para>demostratiu</para><para>parodomasis</para><para>כינוי רמז</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#demonstrativePronoun">http://www.lexinfo.net/ontology/3.0/lexinfo#demonstrativePronoun</seealso>
    let demonstrativePronoun =
        Prefixed_Name(lexinfov3, "demonstrativePronoun") |> PrefixedName
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>Bókstafur</para><para>Buchstabe</para><para>Shkronja</para><para>Buschtaf</para><para>bogstav</para><para>lettera</para><para>bokstav</para><para>bokstav</para><para>písmeno</para><para>písmeno</para><para>bokstav</para><para>kirjain</para><para>lletra</para><para>letter</para><para>lettre</para><para>літара</para><para>letter</para><para>literă</para><para>litera</para><para>Γράμμα</para><para>letra</para><para>burts</para><para>буква</para><para>letra</para><para>letra</para><para>Slovo</para><para>Raidė</para><para>Буква</para><para>litir</para><para>буква</para><para>letra</para><para>слово</para><para>буква</para><para>betű</para><para>Črka</para><para>täht</para><para>אות</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>proprietate morfosintactică a numărului</para><para>gramatiskais skaitlis</para><para>propiedade de número</para><para>proprietà di numero</para><para>Kieliopillinen luku</para><para>nombre grammatical</para><para>número gramatical</para><para>Slovnično število</para><para>numero gramatikal</para><para>skaičiaus požymis</para><para>numër gramatikor</para><para>rasgo de número</para><para>grammatisk tall</para><para>свойство числа</para><para>arvukategooria</para><para>numerus</para><para>numerus</para><para>Numerus</para><para>Αριθμός</para><para>number</para><para>nombre</para><para>liczba</para><para>число</para><para>getal</para><para>číslo</para><para>číslo</para><para>число</para><para>מספר</para><para>tala</para><para>szám</para><para>број</para><para>број</para><para>tal</para><para>лік</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>притяжательный детерминатив</para><para>determinatzaile posesibo</para><para>determinante posesivo</para><para>possessive determiner</para><para>déterminant possessif</para><para>aggettivo possessivo</para><para>присвојни придеви</para><para>adjectiv posesiv</para></remarks>
    ...
    /// labels<para>притяжательный детерминатив</para><para>determinatzaile posesibo</para><para>déterminant possessif</para><para>determinante posesivo</para><para>possessive determiner</para><para>aggettivo possessivo</para><para>присвојни придеви</para><para>adjectiv posesiv</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#possessiveDeterminer">http://www.lexinfo.net/ontology/3.0/lexinfo#possessiveDeterminer</seealso>
    let possessiveDeterminer =
        Prefixed_Name(lexinfov3, "possessiveDeterminer") |> PrefixedName
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>притяжательное местоимение</para><para>bezittelijk voornaamwoord</para><para>přivlastňovací zájmeno</para><para>присвійний займенник</para><para>possessiivipronomini</para><para>izenordain posesibo</para><para>possessivt pronomen</para><para>possessivt pronomen</para><para>possessiivpronoomen</para><para>pronome possessivo</para><para>possessive pronoun</para><para>pronombre posesivo</para><para>pronome possessivo</para><para>posvojne zamjenice</para><para>присвојна заменица</para><para>savybinis įvardis</para><para>Κτητική αντωνυμία</para><para>Possessivpronomen</para><para>pronom possessif</para><para>pronome posesivo</para><para>pronom possessiu</para><para>pronume posesiv</para><para>birtokos névmás</para><para>eignarfornafn</para></remarks>
    ...
    /// labels<para>preposizzjoni</para><para>preposizione</para><para>Prepositioun</para><para>præposition</para><para>preposición</para><para>préposition</para><para>preposition</para><para>прыназоўнік</para><para>preposition</para><para>prepozicija</para><para>preposisjon</para><para>Präposition</para><para>preposisjon</para><para>preposición</para><para>прийменник</para><para>prepozíció</para><para>prepositio</para><para>preposizio</para><para>voorzetsel</para><para>forsetning</para><para>prijedlozi</para><para>prepoziție</para><para>preposição</para><para>preposició</para><para>předložka</para><para>prievārds</para><para>predložka</para><para>מילת יחס</para><para>przyimek</para><para>предлог</para><para>πρόθεση</para><para>предлог</para><para>предлог</para><para>предлог</para><para>eessõna</para><para>predlog</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>pertsona-izenordain bihurkai</para><para>pronombre personal reflexivo</para><para>reflexive personal pronoun</para></remarks>
    ...
    /// labels<para>Zënia e Hënës (Eklipsi)</para><para>punti di sospensione</para><para>points de suspension</para><para>Puncte de suspensie</para><para>Udeladelsesprikker</para><para>uteslutningstecken</para><para>puntos suspensivos</para><para>puntos suspensivos</para><para>Auslassungspunkte</para><para>suspension points</para><para>Три тачке (знак)</para><para>punts suspensius</para><para>Kolme pistettä</para><para>beletselteken</para><para>Mõttepunktid</para><para>Αποσιωπητικά</para><para>שלוש נקודות</para><para>etenpuntuak</para><para>Daudzpunkte</para><para>reticências</para><para>Wielokropek</para><para>Многоточие</para><para>многоточие</para><para>Daugtaškis</para><para>три крапки</para><para>három pont</para><para>Три точки</para><para>Tri bodky</para><para>výpustka</para><para>Focalbhá</para><para>Tripičje</para><para>Trotočje</para><para>ellipse</para><para>ellipse</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>szimbólum</para><para>siombail</para><para>Simbolis</para><para>simbolo</para><para>symbool</para><para>symbole</para><para>symboli</para><para>símbolo</para><para>σύμβολο</para><para>simbols</para><para>símbolo</para><para>Simboli</para><para>símbolo</para><para>symbol</para><para>симбол</para><para>symbol</para><para>symbol</para><para>Символ</para><para>simbol</para><para>сімвал</para><para>symbol</para><para>símbol</para><para>symbol</para><para>Simbol</para><para>Symbol</para><para>symbol</para><para>sümbol</para><para>симбол</para><para>symbol</para><para>symbol</para><para>Simbol</para><para>символ</para><para>символ</para><para>Tákn</para><para>ikur</para><para>סמל</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>Gramatická kategória času</para><para>darbības vārda laiks</para><para>denbora gramatikal</para><para>γραμματικός χρόνος</para><para>tiempo gramatical</para><para>Gramatinis laikas</para><para>temps gramatical</para><para>Tíðbeyging sagna</para><para>tempo gramatical</para><para>tempo gramatical</para><para>глаголско време</para><para>werkwoordstijd</para><para>Glagolski čas</para><para>aikamuoto</para><para>tempus</para><para>tempus</para><para>igeidő</para><para>Tempus</para><para>Време</para><para>tempo</para><para>temps</para><para>tense</para><para>время</para><para>време</para><para>czas</para><para>Timp</para><para>час</para><para>זמן</para><para>час</para><para>Aeg</para><para>tid</para><para>čas</para><para>tid</para></remarks>
    ...
    /// labels<para>apposizione</para><para>adposition</para><para>адпозиција</para><para>adpozicija</para><para>adposition</para><para>adposition</para><para>forsetning</para><para>adposición</para><para>adposisjon</para><para>Adposition</para><para>adposition</para><para>adposition</para><para>Prepoziție</para><para>adposizio</para><para>adpozíció</para><para>adposició</para><para>adpositio</para><para>parafjalë</para><para>adposição</para><para>aposición</para><para>przyimek</para><para>מילת יחס</para><para>kaassõna</para><para>πρόθεση</para><para>záložka</para><para>адлог</para><para>афикс</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#adposition">http://www.lexinfo.net/ontology/3.0/lexinfo#adposition</seealso>
    let adposition = Prefixed_Name(lexinfov3, "adposition") |> PrefixedName
    ...
    /// labels<para>apstākļa vārds</para><para>prieveiksmis</para><para>határozószó</para><para>dobhriathar</para><para>przysłówek</para><para>прислівник</para><para>príslovka</para><para>תואר פועל</para><para>atviksorð</para><para>ndajfolje</para><para>příslovce</para><para>avverbio</para><para>adverbio</para><para>επίρρημα</para><para>määrsõna</para><para>advérbio</para><para>adverbio</para><para>bijwoord</para><para>прыслоўе</para><para>adberbio</para><para>adverbe</para><para>наречие</para><para>наречие</para><para>prilozi</para><para>adverbi</para><para>prislov</para><para>adverbi</para><para>adverb</para><para>adverb</para><para>avverb</para><para>Adverb</para><para>adverb</para><para>adverb</para><para>adverb</para><para>прилог</para><para>прилог</para><para>biord</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#adverb">http://www.lexinfo.net/ontology/3.0/lexinfo#adverb</seealso>
    let adverb = Prefixed_Name(lexinfov3, "adverb") |> PrefixedName
    /// <summary>
    ...
    /// labels<para>animacidade (linguística)</para><para>animatum og inanimatum</para><para>Belebtheitshierarchie</para><para>назви істот і неістот</para><para>Животност (граматика)</para><para>animat și inanimat</para><para>одушевлённость</para><para>адушаўлёнасць</para><para>biziduntasun</para><para>Animacitet</para><para>animacitat</para><para>animacidad</para><para>animathet</para><para>żywotność</para><para>životnost</para><para>životnosť</para><para>animéité</para><para>Gyvumas</para><para>animacy</para><para>מונפשות</para><para>Élőség</para></remarks>
    ...
    /// labels<para>congiunzione</para><para>Konjunktion</para><para>konjunksjon</para><para>konjunktion</para><para>conjonction</para><para>conjunction</para><para>konġunzjoni</para><para>konjunksjon</para><para>konjunktion</para><para>conxunción</para><para>conjunción</para><para>conjuncție</para><para>konjunktio</para><para>samtenging</para><para>מילת קישור</para><para>juntagailu</para><para>сполучник</para><para>jungtukas</para><para>conjunção</para><para>conjunció</para><para>σύνδεσμος</para><para>bindvlies</para><para>sidesõna</para><para>kötőszó</para><para>lidhëza</para><para>saiklis</para><para>сврзник</para><para>злучнік</para><para>veznici</para><para>spójnik</para><para>везник</para><para>veznik</para><para>spojka</para><para>spojka</para><para>союз</para><para>съюз</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#conjunction">http://www.lexinfo.net/ontology/3.0/lexinfo#conjunction</seealso>
    let conjunction = Prefixed_Name(lexinfov3, "conjunction") |> PrefixedName
    /// <summary>
    ...
    /// labels<para>zelfstandig naamwoord</para><para>имя существительное</para><para>съществително име</para><para>podstatné jméno</para><para>podstatné meno</para><para>substantiivi</para><para>daiktavardis</para><para>substantivo</para><para>samostalnik</para><para>substantivo</para><para>sostantivo</para><para>substantiv</para><para>rzeczownik</para><para>sustantivo</para><para>substantiu</para><para>Substantiv</para><para>ουσιαστικό</para><para>ainmfhocal</para><para>substantiv</para><para>супстантив</para><para>substantiv</para><para>Substantiv</para><para>substantiv</para><para>lietvārds</para><para>nimisõna</para><para>назоўнік</para><para>nafnorð</para><para>imenice</para><para>іменник</para><para>שם עצם</para><para>именка</para><para>főnév</para><para>noun</para><para>nume</para><para>emër</para><para>izen</para><para>nom</para><para>nom</para></remarks>
    ...
    /// labels<para>proprietate morfosintactică a numărului</para><para>gramatiskais skaitlis</para><para>propiedade de número</para><para>Kieliopillinen luku</para><para>proprietà di numero</para><para>nombre grammatical</para><para>numero gramatikal</para><para>número gramatical</para><para>Slovnično število</para><para>skaičiaus požymis</para><para>numër gramatikor</para><para>rasgo de número</para><para>grammatisk tall</para><para>arvukategooria</para><para>свойство числа</para><para>numerus</para><para>Αριθμός</para><para>numerus</para><para>Numerus</para><para>liczba</para><para>number</para><para>nombre</para><para>число</para><para>číslo</para><para>getal</para><para>число</para><para>číslo</para><para>број</para><para>број</para><para>tala</para><para>מספר</para><para>szám</para><para>tal</para><para>лік</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#number">http://www.lexinfo.net/ontology/3.0/lexinfo#number</seealso>
    let number = Prefixed_Name(lexinfov3, "number") |> PrefixedName
    /// <summary>
    ///   <para>lexinfov3:numeral</para>
    ...
    /// labels<para>имя числительное</para><para>числително име</para><para>skaitļa vārds</para><para>zenbatzaile</para><para>skaitvardis</para><para>αριθμητικό</para><para>numeraali</para><para>числівник</para><para>liczebnik</para><para>лічэбнік</para><para>räkneord</para><para>numerale</para><para>telwoord</para><para>číslovka</para><para>Numerale</para><para>numerale</para><para>číslovka</para><para>Numeral</para><para>numeral</para><para>számnév</para><para>numeral</para><para>numeral</para><para>Numeral</para><para>numéral</para><para>töluorð</para><para>нумерал</para><para>numeral</para><para>arvsõna</para><para>numeral</para><para>tallord</para><para>brojevi</para><para>števnik</para><para>talord</para><para>број</para><para>מספר</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#numeral">http://www.lexinfo.net/ontology/3.0/lexinfo#numeral</seealso>
    let numeral = Prefixed_Name(lexinfov3, "numeral") |> PrefixedName
    /// <summary>
    ...
    /// labels<para>įvardinis, įvardžiuotinis</para><para>voornaamwoord</para><para>vietniekvārds</para><para>местоимённый</para><para>asesõnaline</para><para>местоимение</para><para>pronominale</para><para>pronominal</para><para>pronominal</para><para>pronominal</para><para>pronominal</para><para>izenordain</para><para>pronominal</para><para>pronomini</para><para>займеннік</para><para>zamjenice</para><para>αντωνυμία</para><para>כינוי גוף</para><para>займенник</para><para>заменица</para><para>pronomen</para><para>zaimkowy</para><para>pronomen</para><para>Pronomen</para><para>pronomen</para><para>zájmeno</para><para>pronome</para><para>forainm</para><para>fornafn</para><para>pronoun</para><para>заменка</para><para>përemër</para><para>zámeno</para><para>pronom</para><para>névmás</para><para>pronom</para><para>zaimek</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#pronoun">http://www.lexinfo.net/ontology/3.0/lexinfo#pronoun</seealso>
    let pronoun = Prefixed_Name(lexinfov3, "pronoun") |> PrefixedName
    /// <summary>
    ...
    /// labels<para>Hizkuntza-erregistro</para><para>registro lingüístico</para><para>registo linguístico</para><para>registre de langue</para><para>Registru de limbă</para><para>языковой регистр</para><para>Sprachebene</para><para>regiszter</para><para>rekisteri</para><para>registras</para><para>stilleje</para><para>registro</para><para>Register</para><para>register</para><para>register</para><para>rexistro</para><para>registre</para><para>register</para><para>регистар</para><para>málsnið</para><para>rejestr</para><para>משלב</para></remarks>
    ...
    /// labels<para>darbības vārds</para><para>veiksmažodis</para><para>udsagnsord</para><para>czasownik</para><para>werkwoord</para><para>дієслово</para><para>дзеяслоў</para><para>briathar</para><para>tegusõna</para><para>sagnorð</para><para>glagoli</para><para>sloveso</para><para>sloveso</para><para>глагол</para><para>глагол</para><para>глагол</para><para>glagol</para><para>глагол</para><para>verbo</para><para>verbe</para><para>aditz</para><para>verbo</para><para>verbo</para><para>verbi</para><para>verbo</para><para>folje</para><para>פועל</para><para>Verb</para><para>verb</para><para>verb</para><para>verb</para><para>verb</para><para>verb</para><para>verb</para><para>verb</para><para>Verb</para><para>ρήμα</para><para>ige</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#verb">http://www.lexinfo.net/ontology/3.0/lexinfo#verb</seealso>
    let verb = Prefixed_Name(lexinfov3, "verb") |> PrefixedName
    /// <summary>
    ///   <para>lexinfov3:verbFormMood</para>
    ...
    /// labels<para>Darbības vārda kārta</para><para>грамматический залог</para><para>Glagolski način</para><para>voz gramatical</para><para>actief/passief</para><para>veu gramatical</para><para>slovesný rod</para><para>slovesný rod</para><para>מעמד דקדוקי</para><para>Sagnmyndir</para><para>voz verbal</para><para>Pääluokka</para><para>Diathese</para><para>Tegumood</para><para>diathèse</para><para>Diatese</para><para>Diateza</para><para>diatese</para><para>diatese</para><para>diatesi</para><para>Diateză</para><para>strona</para><para>igenem</para><para>diates</para><para>voice</para><para>rūšis</para><para>Залог</para><para>Залог</para><para>Φωνή</para><para>стан</para><para>voz</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#voice">http://www.lexinfo.net/ontology/3.0/lexinfo#voice</seealso>
    let voice = Prefixed_Name(lexinfov3, "voice") |> PrefixedName
    /// <summary>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>apposizione</para><para>adposición</para><para>forsetning</para><para>adposition</para><para>adposition</para><para>adposisjon</para><para>adposition</para><para>adpozicija</para><para>адпозиција</para><para>Prepoziție</para><para>adposition</para><para>Adposition</para><para>adposition</para><para>adpositio</para><para>adposició</para><para>adposição</para><para>adposizio</para><para>aposición</para><para>parafjalë</para><para>adpozíció</para><para>מילת יחס</para><para>przyimek</para><para>kaassõna</para><para>záložka</para><para>πρόθεση</para><para>афикс</para><para>адлог</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>граматички члан</para><para>przedimek</para><para>artikelis</para><para>членуване</para><para>artikkeli</para><para>artikkel</para><para>artikulu</para><para>artículo</para><para>artikkel</para><para>lidwoord</para><para>articolo</para><para>artikuls</para><para>kendeord</para><para>Artikel</para><para>article</para><para>article</para><para>artikel</para><para>артикль</para><para>article</para><para>артыкль</para><para>artiklu</para><para>greinir</para><para>артикль</para><para>Artikel</para><para>articol</para><para>artigo</para><para>trajta</para><para>artigo</para><para>névelő</para><para>άρθρο</para><para>תווית</para><para>član</para><para>член</para><para>člen</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>количественное числительное</para><para>kiekinis skaitvardis</para><para>zenbatzaile kardinal</para><para>adjetivo cardinal</para><para>číslovka základní</para><para>numero cardinale</para><para>numeral cardinal</para><para>cardinal numeral</para><para>numeral cardinal</para><para>liczebnik główny</para><para>απόλυτος αριθμός</para><para>Glavni števnik</para><para>Hoofdtelwoord</para><para>Kardinalzahl</para><para>основни број</para><para>tőszámnév</para><para>mængdetal</para><para>grunntall</para><para>cardinal</para><para>Grundtal</para><para>frumtala</para><para>põhiarv</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#CardinalNumeral">http://www.lexinfo.net/ontology/3.0/lexinfo#CardinalNumeral</seealso>
    let CardinalNumeral = Prefixed_Name(lexinfov3, "CardinalNumeral") |> PrefixedName
    /// <summary>
    ///   <para>lexinfov3:closeParenthesis</para>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>Kaksoispiste</para><para>Două puncte</para><para>deux-points</para><para>dubbelepunt</para><para>dous puntos</para><para>dois pontos</para><para>Doppelpunkt</para><para>kettőspont</para><para>dos puntos</para><para>Dvitaškis</para><para>Dvojbodka</para><para>Две точки</para><para>Dvojtečka</para><para>Dwukropek</para><para>двоеточие</para><para>двукроп'е</para><para>Две тачке</para><para>Двоеточие</para><para>двокрапка</para><para>dos punts</para><para>due punti</para><para>Dvotočje</para><para>Bi puntu</para><para>נקודתיים</para><para>Dy pikat</para><para>Dvopičje</para><para>Koolon</para><para>kolon</para><para>kolon</para><para>kolon</para><para>kolon</para><para>colon</para><para>kols</para></remarks>
    ...
    /// labels<para>Kaksoispiste</para><para>dois pontos</para><para>Două puncte</para><para>Doppelpunkt</para><para>deux-points</para><para>dous puntos</para><para>dubbelepunt</para><para>kettőspont</para><para>dos puntos</para><para>Dwukropek</para><para>Две тачке</para><para>Dvitaškis</para><para>двокрапка</para><para>due punti</para><para>двоеточие</para><para>dos punts</para><para>Dvojtečka</para><para>Двоеточие</para><para>двукроп'е</para><para>Dvojbodka</para><para>Две точки</para><para>Dvopičje</para><para>נקודתיים</para><para>Dy pikat</para><para>Bi puntu</para><para>Dvotočje</para><para>Koolon</para><para>kolon</para><para>kolon</para><para>kolon</para><para>kolon</para><para>colon</para><para>kols</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#colon">http://www.lexinfo.net/ontology/3.0/lexinfo#colon</seealso>
    let colon = Prefixed_Name(lexinfov3, "colon") |> PrefixedName
    /// <summary>
    ///   <para>lexinfov3:Comma</para>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>kommatecken</para><para>Przecinek</para><para>Kablelis</para><para>запятая</para><para>virgola</para><para>vírgula</para><para>virgule</para><para>Virgulă</para><para>Запирка</para><para>Запетая</para><para>Komats</para><para>vessző</para><para>Čiarka</para><para>Presja</para><para>Vejica</para><para>pilkku</para><para>Komma</para><para>komma</para><para>komma</para><para>Zarez</para><para>comma</para><para>коска</para><para>Komma</para><para>Camóg</para><para>Зарез</para><para>Κόμμα</para><para>komma</para><para>Komma</para><para>čárka</para><para>Coma</para><para>Koma</para><para>coma</para><para>פסיק</para><para>Koma</para><para>coma</para><para>Кома</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#Comma">http://www.lexinfo.net/ontology/3.0/lexinfo#Comma</seealso>
    let Comma = Prefixed_Name(lexinfov3, "Comma") |> PrefixedName
    ...
    /// labels<para>conjonction de coordination</para><para>nebenordnende Konjunktion</para><para>congiunzione coordinante</para><para>coordinating conjunction</para><para>konġunzjoni koordinanti</para><para>conjunção coordenativa</para><para>conjunción coordinante</para><para>emendiozko juntagailu</para><para>sideordnet bindeord</para><para>priraďovacia spojka</para><para>соединительный союз</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>Còpula (lingüística)</para><para>koppelwerkwoord</para><para>Verb copulativ</para><para>Tengisögn</para><para>Koopula</para><para>copule</para><para>Kopula</para><para>copula</para><para>Kopula</para><para>kopula</para><para>copula</para><para>Copail</para><para>copula</para><para>kopula</para><para>kopula</para><para>kopula</para><para>Kopula</para><para>связка</para><para>copula</para><para>Spona</para><para>אוגד</para><para>Vez</para></remarks>
    ...
    /// labels<para>Còpula (lingüística)</para><para>koppelwerkwoord</para><para>Verb copulativ</para><para>Tengisögn</para><para>Koopula</para><para>copula</para><para>kopula</para><para>kopula</para><para>Kopula</para><para>kopula</para><para>copula</para><para>copule</para><para>Kopula</para><para>связка</para><para>Kopula</para><para>copula</para><para>copula</para><para>Copail</para><para>kopula</para><para>Spona</para><para>אוגד</para><para>Vez</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#copula">http://www.lexinfo.net/ontology/3.0/lexinfo#copula</seealso>
    let copula = Prefixed_Name(lexinfov3, "copula") |> PrefixedName
    /// <summary>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>articolo determinativo</para><para>определенный артикль</para><para>žymimasis artikelis</para><para>przedimek określony</para><para>bestimmter Artikel</para><para>artículo definido</para><para>articolul hotărât</para><para>határozott névelő</para><para>definite article</para><para>ákveðinn greinir</para><para>artikulu mugatua</para><para>Bepaald lidwoord</para><para>määrav artikkel</para><para>article definit</para><para>bestemt artikel</para><para>artigo definido</para><para>artigo definido</para><para>οριστικό άρθρο</para><para>article défini</para><para>одређени члан</para><para>člen určitý</para><para>הא הידיעה</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#DefiniteArticle">http://www.lexinfo.net/ontology/3.0/lexinfo#DefiniteArticle</seealso>
    let DefiniteArticle = Prefixed_Name(lexinfov3, "DefiniteArticle") |> PrefixedName
    /// <summary>
    ///   <para>lexinfov3:Definiteness</para>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>definitud gramatical</para><para>определённость</para><para>определеност</para><para>визначеність</para><para>definiteness</para><para>Definitheit</para><para>określoność</para><para>määräisyys</para><para>bestemthet</para><para>bestemthed</para><para>určitost</para><para>binding</para><para>species</para><para>Species</para><para>יידוע</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#Definiteness">http://www.lexinfo.net/ontology/3.0/lexinfo#Definiteness</seealso>
    let Definiteness = Prefixed_Name(lexinfov3, "Definiteness") |> PrefixedName
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>articolo indeterminativo</para><para>неопределенный артикль</para><para>przedimek nieokreślony</para><para>nežymimasis artikelis</para><para>artículo indefinido</para><para>umbmäärane artikkel</para><para>határozatlan névelő</para><para>articolul nehotărât</para><para>indefinite article</para><para>Onbepaald lidwoord</para><para>artigo indefinido</para><para>article indefinit</para><para>artikulu mugagabe</para><para>óákveðinn greinir</para><para>artigo indefinido</para><para>ubestemt artikel</para><para>article indéfini</para><para>Indefinitartikel</para><para>неодређени члан</para><para>αόριστο άρθρο</para><para>člen neurčitý</para></remarks>
    ...
    /// labels<para>articolo indeterminativo</para><para>неопределенный артикль</para><para>przedimek nieokreślony</para><para>nežymimasis artikelis</para><para>határozatlan névelő</para><para>artículo indefinido</para><para>articolul nehotărât</para><para>umbmäärane artikkel</para><para>indefinite article</para><para>Onbepaald lidwoord</para><para>artigo indefinido</para><para>artikulu mugagabe</para><para>article indefinit</para><para>óákveðinn greinir</para><para>artigo indefinido</para><para>ubestemt artikel</para><para>Indefinitartikel</para><para>article indéfini</para><para>неодређени члан</para><para>αόριστο άρθρο</para><para>člen neurčitý</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#indefiniteArticle">http://www.lexinfo.net/ontology/3.0/lexinfo#indefiniteArticle</seealso>
    let indefiniteArticle =
        Prefixed_Name(lexinfov3, "indefiniteArticle") |> PrefixedName
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>неопределённое местоимение</para><para>Onbepaald voornaamwoord</para><para>неозначений займенник</para><para>Neodređene zamjenice</para><para>pronombre indefinido</para><para>határozatlan névmás</para><para>неодређена заменица</para><para>izenordain mugagabe</para><para>nežymimasis įvardis</para><para>zaimek nieokreślony</para><para>pronome indefinito</para><para>indefinite pronoun</para><para>pronome indefinido</para><para>indefinit pronomen</para><para>indefinit pronomen</para><para>umbmäärane asesõna</para><para>pronome indefinido</para><para>αόριστη αντωνυμία</para><para>Pronume nehotărât</para><para>Indefinitpronomen</para><para>pronom indefinit</para><para>neurčité zájmeno</para><para>óákveðið fornafn</para><para>pronom indéfini</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#IndefinitePronoun">http://www.lexinfo.net/ontology/3.0/lexinfo#IndefinitePronoun</seealso>
    let IndefinitePronoun =
        Prefixed_Name(lexinfov3, "IndefinitePronoun") |> PrefixedName
    ...
    ///   <para>owl:Thing</para>
    ///
    /// labels<para>Buchstabe</para><para>Bókstafur</para><para>Shkronja</para><para>Buschtaf</para><para>bokstav</para><para>lettera</para><para>bokstav</para><para>bogstav</para><para>písmeno</para><para>kirjain</para><para>bokstav</para><para>písmeno</para><para>letter</para><para>lettre</para><para>litera</para><para>lletra</para><para>literă</para><para>letter</para><para>літара</para><para>Γράμμα</para><para>letra</para><para>letra</para><para>Slovo</para><para>letra</para><para>буква</para><para>Raidė</para><para>буква</para><para>слово</para><para>burts</para><para>буква</para><para>litir</para><para>Буква</para><para>letra</para><para>betű</para><para>Črka</para><para>täht</para><para>אות</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#letter">http://www.lexinfo.net/ontology/3.0/lexinfo#letter</seealso>
    let letter = Prefixed_Name(lexinfov3, "letter") |> PrefixedName
    /// <summary>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>modalinis veiksmažodis</para><para>czasownik modalny</para><para>způsobové sloveso</para><para>modalt hjelpeverb</para><para>modalt hjelpeverb</para><para>модальній дієслів</para><para>модальный глагол</para><para>modalt hjälpverb</para><para>modaal werkwoord</para><para>модални глагол</para><para>модален глагол</para><para>moduzko aditz</para><para>modalverbum</para><para>modális ige</para><para>verbe modal</para><para>verbo modal</para><para>háttarsögn</para><para>Verb modal</para><para>modaalverb</para><para>Modalverb</para><para>modale</para><para>modal</para><para>modal</para><para>modal</para><para>modal</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>Format e shtjelluara të foljes</para><para>gramatiskā izteiksme</para><para>наклонение глагола</para><para>grammatisk modus</para><para>Glagolski naklon</para><para>modo gramatical</para><para>способи дієслів</para><para>mod al verbului</para><para>slovesný způsob</para><para>глаголски начин</para><para>modo gramatical</para><para>глаголски начин</para><para>modu gramatikal</para><para>slovesný spôsob</para><para>лад дзеяслова</para><para>Hættir sagna</para><para>modo verbal</para><para>tapaluokka</para><para>Наклонение</para><para>Kõneviis</para><para>Nuosaka</para><para>έγκλιση</para><para>igemód</para><para>Modus</para><para>מודוס</para><para>modus</para><para>Modus</para><para>modus</para><para>modo</para><para>wijs</para><para>mode</para><para>tryb</para><para>mode</para><para>mood</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>zelfstandig naamwoord</para><para>имя существительное</para><para>съществително име</para><para>podstatné jméno</para><para>podstatné meno</para><para>substantiivi</para><para>daiktavardis</para><para>samostalnik</para><para>substantivo</para><para>substantivo</para><para>substantiv</para><para>substantiv</para><para>sustantivo</para><para>ainmfhocal</para><para>ουσιαστικό</para><para>substantiu</para><para>супстантив</para><para>substantiv</para><para>rzeczownik</para><para>Substantiv</para><para>Substantiv</para><para>sostantivo</para><para>substantiv</para><para>lietvārds</para><para>nimisõna</para><para>назоўнік</para><para>іменник</para><para>imenice</para><para>nafnorð</para><para>именка</para><para>שם עצם</para><para>főnév</para><para>noun</para><para>izen</para><para>emër</para><para>nume</para><para>nom</para><para>nom</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>marco nominal</para><para>izen-marko</para><para>noun frame</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>persoonlijk voornaamwoord</para><para>προσωπική αντωνυμία</para><para>personligt pronomen</para><para>pertsona-izenordain</para><para>personligt pronomen</para><para>personleg pronomen</para><para>личное местоимение</para><para>pronombre personal</para><para>особовий займенник</para><para>asmeninis įvardis</para><para>persoonapronomini</para><para>лично местоимение</para><para>pronome personale</para><para>isikuline asesõna</para><para>personal pronoun</para><para>Pronume personal</para><para>Personalpronomen</para><para>osobna zamjenica</para><para>pronom personnel</para><para>személyes névmás</para><para>pronome persoal</para><para>pronome pessoal</para><para>pronom personal</para><para>osobní zájmeno</para><para>zaimek osobowy</para><para>лична заменица</para><para>persónufornafn</para><para>osebni zaimek</para><para>כינוי גוף</para></remarks>
    ...
    /// labels<para>persoonlijk voornaamwoord</para><para>pertsona-izenordain</para><para>personligt pronomen</para><para>personligt pronomen</para><para>προσωπική αντωνυμία</para><para>personleg pronomen</para><para>личное местоимение</para><para>особовий займенник</para><para>pronombre personal</para><para>pronome personale</para><para>asmeninis įvardis</para><para>persoonapronomini</para><para>лично местоимение</para><para>isikuline asesõna</para><para>personal pronoun</para><para>Personalpronomen</para><para>pronom personnel</para><para>személyes névmás</para><para>Pronume personal</para><para>osobna zamjenica</para><para>pronome pessoal</para><para>pronome persoal</para><para>pronom personal</para><para>persónufornafn</para><para>osobní zájmeno</para><para>лична заменица</para><para>zaimek osobowy</para><para>osebni zaimek</para><para>כינוי גוף</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#personalPronoun">http://www.lexinfo.net/ontology/3.0/lexinfo#personalPronoun</seealso>
    let personalPronoun = Prefixed_Name(lexinfov3, "personalPronoun") |> PrefixedName
    /// <summary>
    ...
    /// labels<para>притяжательное местоимение</para><para>bezittelijk voornaamwoord</para><para>přivlastňovací zájmeno</para><para>possessiivipronomini</para><para>присвійний займенник</para><para>possessiivpronoomen</para><para>possessivt pronomen</para><para>possessivt pronomen</para><para>izenordain posesibo</para><para>pronombre posesivo</para><para>pronome possessivo</para><para>pronome possessivo</para><para>posvojne zamjenice</para><para>присвојна заменица</para><para>possessive pronoun</para><para>savybinis įvardis</para><para>Κτητική αντωνυμία</para><para>Possessivpronomen</para><para>pronom possessiu</para><para>pronom possessif</para><para>pronome posesivo</para><para>birtokos névmás</para><para>pronume posesiv</para><para>eignarfornafn</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#possessivePronoun">http://www.lexinfo.net/ontology/3.0/lexinfo#possessivePronoun</seealso>
    let possessivePronoun =
        Prefixed_Name(lexinfov3, "possessivePronoun") |> PrefixedName
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>postposition</para><para>postpozicija</para><para>postposition</para><para>postpozīcija</para><para>postposition</para><para>післяйменник</para><para>postposisjon</para><para>постпозиција</para><para>posposizione</para><para>postposition</para><para>postposisjon</para><para>Postposition</para><para>eftirsetning</para><para>achterzetsel</para><para>постпозиция</para><para>postpoziție</para><para>posposición</para><para>postpositio</para><para>posposición</para><para>postposizio</para><para>posposició</para><para>posposição</para><para>μετά-θεση</para><para>tagasõna</para><para>poimek</para><para>névutó</para></remarks>
    ...
    /// labels<para>postposisjon</para><para>posposizione</para><para>postposisjon</para><para>Postposition</para><para>postposition</para><para>postposition</para><para>postpozicija</para><para>постпозиција</para><para>eftirsetning</para><para>postpozīcija</para><para>післяйменник</para><para>achterzetsel</para><para>postposition</para><para>postposition</para><para>постпозиция</para><para>postpositio</para><para>postpoziție</para><para>posposición</para><para>postposizio</para><para>posposición</para><para>posposició</para><para>posposição</para><para>μετά-θεση</para><para>tagasõna</para><para>poimek</para><para>névutó</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#postposition">http://www.lexinfo.net/ontology/3.0/lexinfo#postposition</seealso>
    let postposition = Prefixed_Name(lexinfov3, "postposition") |> PrefixedName
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>preposizzjoni</para><para>preposizione</para><para>Prepositioun</para><para>Präposition</para><para>preposition</para><para>preposición</para><para>prepozicija</para><para>præposition</para><para>прыназоўнік</para><para>préposition</para><para>preposition</para><para>preposisjon</para><para>preposición</para><para>preposisjon</para><para>prijedlozi</para><para>voorzetsel</para><para>prepozíció</para><para>forsetning</para><para>preposizio</para><para>prepoziție</para><para>прийменник</para><para>preposició</para><para>preposição</para><para>prepositio</para><para>předložka</para><para>predložka</para><para>prievārds</para><para>przyimek</para><para>מילת יחס</para><para>предлог</para><para>предлог</para><para>предлог</para><para>предлог</para><para>predlog</para><para>eessõna</para><para>πρόθεση</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#Preposition">http://www.lexinfo.net/ontology/3.0/lexinfo#Preposition</seealso>
    let Preposition = Prefixed_Name(lexinfov3, "Preposition") |> PrefixedName
    /// <summary>
    ...
    /// labels<para>vlastné podstatné meno</para><para>tikrinis daiktavardis</para><para>substantivo próprio</para><para>rzeczownik własny</para><para>властита именица</para><para>ainmfhocal dílis</para><para>имя собственное</para><para>nombre propio</para><para>vlastní jméno</para><para>власна назва</para><para>лична именка</para><para>nome proprio</para><para>nume propriu</para><para>שם עצם פרטי</para><para>уласнае імя</para><para>nome propio</para><para>κύριο όνομα</para><para>izen propio</para><para>tulajdonnév</para><para>proper noun</para><para>Lastno ime</para><para>nom propju</para><para>nom propre</para><para>pärisnimi</para><para>Eigenname</para><para>eigennaam</para><para>nom propi</para><para>īpašvārds</para><para>egennamn</para><para>proprium</para><para>erisnimi</para><para>egennavn</para><para>sérnafn</para><para>særnamn</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>Interpunktsioon</para><para>Zeichensetzung</para><para>punteggiatura</para><para>interpunktion</para><para>интерпункција</para><para>Σημείο στίξης</para><para>интерпункција</para><para>interpunkcija</para><para>interpunkcia</para><para>interpunkcja</para><para>teiknsetjing</para><para>interpunctie</para><para>välimerkkejä</para><para>interpunkce</para><para>tegnsætning</para><para>tegnsetting</para><para>punctuation</para><para>ponctuation</para><para>punctuație</para><para>puntuación</para><para>пунктуація</para><para>puntuación</para><para>poncaíocht</para><para>пунктуация</para><para>пунктуация</para><para>geнктуацыя</para><para>puntuazio</para><para>puntuació</para><para>pontuação</para><para>razgodak</para><para>skyryba</para><para>ločilo</para><para>פיסוק</para></remarks>
    ...
    /// labels<para>Interpunktsioon</para><para>Zeichensetzung</para><para>Σημείο στίξης</para><para>interpunkcija</para><para>интерпункција</para><para>punteggiatura</para><para>интерпункција</para><para>interpunktion</para><para>interpunkcja</para><para>teiknsetjing</para><para>interpunkcia</para><para>interpunctie</para><para>välimerkkejä</para><para>tegnsætning</para><para>interpunkce</para><para>punctuation</para><para>tegnsetting</para><para>ponctuation</para><para>poncaíocht</para><para>geнктуацыя</para><para>пунктуація</para><para>puntuación</para><para>punctuație</para><para>пунктуация</para><para>пунктуация</para><para>puntuación</para><para>puntuació</para><para>pontuação</para><para>puntuazio</para><para>razgodak</para><para>skyryba</para><para>ločilo</para><para>פיסוק</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#punctuation">http://www.lexinfo.net/ontology/3.0/lexinfo#punctuation</seealso>
    let punctuation = Prefixed_Name(lexinfov3, "punctuation") |> PrefixedName
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>signo de interrogación</para><para>signo de interrogación</para><para>point d'interrogation</para><para>ponto de interrogação</para><para>signe d'interrogació</para><para>вопросительный знак</para><para>punto interrogativo</para><para>Semnul întrebării</para><para>Въпросителен знак</para><para>Comhartha ceiste</para><para>Spurningarmerki</para><para>Jautājuma zīme</para><para>Znak zapytania</para><para>spørgsmålstegn</para><para>spørsmålsteikn</para><para>Kysymysmerkki</para><para>Spørsmålstegn</para><para>question mark</para><para>galdera-marka</para><para>Знак питання</para><para>Fragezeichen</para><para>Знак питања</para><para>frågetecken</para><para>ερωτηματικό</para><para>Klaustukas</para><para>vraagteken</para><para>Прашалник</para><para>סימן שאלה</para><para>Пытальнік</para><para>kérdőjel</para><para>Küsimärk</para><para>otazník</para><para>Vprašaj</para><para>Upitnik</para><para>Otáznik</para><para>?</para></remarks>
    ...
    /// labels<para>signo de interrogación</para><para>signo de interrogación</para><para>point d'interrogation</para><para>ponto de interrogação</para><para>signe d'interrogació</para><para>вопросительный знак</para><para>punto interrogativo</para><para>Semnul întrebării</para><para>Въпросителен знак</para><para>Comhartha ceiste</para><para>Spurningarmerki</para><para>spørsmålsteikn</para><para>Jautājuma zīme</para><para>spørgsmålstegn</para><para>Znak zapytania</para><para>Spørsmålstegn</para><para>question mark</para><para>galdera-marka</para><para>Kysymysmerkki</para><para>Знак питання</para><para>Fragezeichen</para><para>frågetecken</para><para>Знак питања</para><para>ερωτηματικό</para><para>Klaustukas</para><para>vraagteken</para><para>Пытальнік</para><para>סימן שאלה</para><para>Прашалник</para><para>Küsimärk</para><para>kérdőjel</para><para>Upitnik</para><para>Otáznik</para><para>otazník</para><para>Vprašaj</para><para>?</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#questionMark">http://www.lexinfo.net/ontology/3.0/lexinfo#questionMark</seealso>
    let questionMark = Prefixed_Name(lexinfov3, "questionMark") |> PrefixedName
    /// <summary>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>wederkerig voornaamwoord</para><para>αλληλοπαθής αντωνυμία</para><para>взаимное местоимение</para><para>izenordain elkarkari</para><para>gagnverkandi fornafn</para><para>vastastikune asesõna</para><para>reciprokinis įvardis</para><para>реципрочна заменица</para><para>pronombre recíproco</para><para>resiprokt pronomen</para><para>Взаємні займенники</para><para>reciprokt pronomen</para><para>reciprocal pronoun</para><para>pronome recíproco</para><para>pronome reciproco</para><para>kölcsönös névmás</para><para>Reziprokpronomen</para><para>pronom recíproc</para></remarks>
    ...
    /// labels<para>wederkerig voornaamwoord</para><para>αλληλοπαθής αντωνυμία</para><para>взаимное местоимение</para><para>gagnverkandi fornafn</para><para>vastastikune asesõna</para><para>izenordain elkarkari</para><para>reciprokinis įvardis</para><para>реципрочна заменица</para><para>pronombre recíproco</para><para>reciprocal pronoun</para><para>reciprokt pronomen</para><para>resiprokt pronomen</para><para>Взаємні займенники</para><para>pronome reciproco</para><para>pronome recíproco</para><para>Reziprokpronomen</para><para>kölcsönös névmás</para><para>pronom recíproc</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#reciprocalPronoun">http://www.lexinfo.net/ontology/3.0/lexinfo#reciprocalPronoun</seealso>
    let reciprocalPronoun =
        Prefixed_Name(lexinfov3, "reciprocalPronoun") |> PrefixedName
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>marku trantsitibo</para><para>transitive frame</para><para>marco transitivo</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>Hizkuntza-erregistro</para><para>registro lingüístico</para><para>registo linguístico</para><para>registre de langue</para><para>Registru de limbă</para><para>языковой регистр</para><para>Sprachebene</para><para>regiszter</para><para>rekisteri</para><para>registras</para><para>регистар</para><para>registro</para><para>register</para><para>register</para><para>register</para><para>stilleje</para><para>registre</para><para>Register</para><para>rexistro</para><para>málsnið</para><para>rejestr</para><para>משלב</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#Register">http://www.lexinfo.net/ontology/3.0/lexinfo#Register</seealso>
    let Register = Prefixed_Name(lexinfov3, "Register") |> PrefixedName
    /// <summary>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>betrekkelijk voornaamwoord</para><para>относительное местоимение</para><para>izenordain erlatibo</para><para>αναφορική αντωνυμία</para><para>відносний займенник</para><para>relatiivipronomini</para><para>santykinis įvardis</para><para>pronombre relativo</para><para>tilvísunarfornafn</para><para>relativt pronomen</para><para>odnosne zamjenice</para><para>relativt pronomen</para><para>vonatkozó névmás</para><para>relative pronoun</para><para>односна заменица</para><para>pronome relativo</para><para>pronome relativo</para><para>pronome relativo</para><para>relativpronomen</para><para>zájmeno vztažné</para><para>zaimek względny</para><para>Relativpronomen</para><para>Pronume relativ</para><para>pronom relatiu</para><para>pronom relatif</para><para>siduv asesõna</para></remarks>
    ...
    /// labels<para>Rasa gramatikore</para><para>cas grammatical</para><para>caso gramatical</para><para>kasu gramatikal</para><para>caso gramatical</para><para>caso gramatical</para><para>cas gramatical</para><para>mluvnický pád</para><para>przypadek</para><para>locījums</para><para>linksnis</para><para>відмінок</para><para>naamval</para><para>kääne</para><para>πτώση</para><para>padež</para><para>падеж</para><para>падеж</para><para>kasus</para><para>Kasus</para><para>kasus</para><para>sklon</para><para>падеж</para><para>падеж</para><para>kasus</para><para>склон</para><para>kasus</para><para>caso</para><para>sija</para><para>eset</para><para>fall</para><para>יחסה</para><para>case</para><para>caz</para><para>pád</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#case">http://www.lexinfo.net/ontology/3.0/lexinfo#case</seealso>
    let case = Prefixed_Name(lexinfov3, "case") |> PrefixedName
    ...
    /// labels<para>Hyppighet i statistikk</para><para>Релативна фреквенција</para><para>Frekvens i statistikk</para><para>Частота події</para><para>фреквенција</para><para>частотность</para><para>frekwencja</para><para>frequentie</para><para>gyakoriság</para><para>freqüència</para><para>frecuencia</para><para>frecuencia</para><para>Häufigkeit</para><para>frequência</para><para>frequenza</para><para>frekvenca</para><para>frecvență</para><para>Maiztasun</para><para>frequency</para><para>συχνότητα</para><para>fréquence</para><para>frekvens</para><para>dažnumas</para><para>Частата</para><para>sagedus</para><para>četnost</para><para>שכיחות</para><para>tíðni</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#frequency">http://www.lexinfo.net/ontology/3.0/lexinfo#frequency</seealso>
    let frequency = Prefixed_Name(lexinfov3, "frequency") |> PrefixedName
    /// <summary>
    ...
    /// labels<para>Format e shtjelluara të foljes</para><para>gramatiskā izteiksme</para><para>наклонение глагола</para><para>Glagolski naklon</para><para>grammatisk modus</para><para>modo gramatical</para><para>способи дієслів</para><para>slovesný způsob</para><para>mod al verbului</para><para>modu gramatikal</para><para>глаголски начин</para><para>modo gramatical</para><para>slovesný spôsob</para><para>глаголски начин</para><para>лад дзеяслова</para><para>Hættir sagna</para><para>modo verbal</para><para>tapaluokka</para><para>Наклонение</para><para>Kõneviis</para><para>Nuosaka</para><para>έγκλιση</para><para>igemód</para><para>Modus</para><para>modus</para><para>Modus</para><para>מודוס</para><para>modus</para><para>wijs</para><para>mood</para><para>mode</para><para>modo</para><para>tryb</para><para>mode</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#mood">http://www.lexinfo.net/ontology/3.0/lexinfo#mood</seealso>
    let mood = Prefixed_Name(lexinfov3, "mood") |> PrefixedName
    ...
    /// labels<para>szimbólum</para><para>Simbolis</para><para>siombail</para><para>simbols</para><para>σύμβολο</para><para>symboli</para><para>simbolo</para><para>símbolo</para><para>symbole</para><para>símbolo</para><para>símbolo</para><para>Simboli</para><para>symbool</para><para>Символ</para><para>символ</para><para>symbol</para><para>símbol</para><para>симбол</para><para>sümbol</para><para>symbol</para><para>symbol</para><para>Simbol</para><para>simbol</para><para>символ</para><para>Symbol</para><para>симбол</para><para>symbol</para><para>symbol</para><para>symbol</para><para>Simbol</para><para>symbol</para><para>symbol</para><para>сімвал</para><para>Tákn</para><para>ikur</para><para>סמל</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#symbol">http://www.lexinfo.net/ontology/3.0/lexinfo#symbol</seealso>
    let symbol = Prefixed_Name(lexinfov3, "symbol") |> PrefixedName
    /// <summary>
    ...
    /// labels<para>pasthirrma - themelet e ngurtësuara të gjuhës</para><para>izsauksmes vārds</para><para>tussenwerpsel</para><para>inteġrezzjoni</para><para>interjektion</para><para>interjeksjon</para><para>interjection</para><para>interxección</para><para>interjección</para><para>interjektion</para><para>Interjektion</para><para>interjection</para><para>interjeksjon</para><para>interiezione</para><para>interjeição</para><para>interjecció</para><para>interjecție</para><para>interjekzio</para><para>interjektio</para><para>междометие</para><para>מילת קריאה</para><para>citoslovce</para><para>wykrzyknik</para><para>citoslovce</para><para>indulatszó</para><para>междуметие</para><para>jaustukas</para><para>выклічнік</para><para>intriacht</para><para>επιφώνημα</para><para>upphrópun</para><para>hüüdsõna</para><para>usklici</para><para>medmet</para><para>извик</para><para>вигук</para><para>узвик</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#interjection">http://www.lexinfo.net/ontology/3.0/lexinfo#interjection</seealso>
    let interjection = Prefixed_Name(lexinfov3, "interjection") |> PrefixedName
    ...
    /// labels<para>modalinis veiksmažodis</para><para>způsobové sloveso</para><para>modalt hjelpeverb</para><para>czasownik modalny</para><para>модальній дієслів</para><para>modalt hjelpeverb</para><para>modaal werkwoord</para><para>модальный глагол</para><para>modalt hjälpverb</para><para>модални глагол</para><para>модален глагол</para><para>moduzko aditz</para><para>modális ige</para><para>verbo modal</para><para>modalverbum</para><para>verbe modal</para><para>Verb modal</para><para>háttarsögn</para><para>modaalverb</para><para>Modalverb</para><para>modale</para><para>modal</para><para>modal</para><para>modal</para><para>modal</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#modal">http://www.lexinfo.net/ontology/3.0/lexinfo#modal</seealso>
    let modal = Prefixed_Name(lexinfov3, "modal") |> PrefixedName
    /// <summary>
    ///   <para>lexinfov3:ModificationType</para>
    ...
    /// labels<para>aspecto gramatical</para><para>aspecte gramatical</para><para>aspecto gramatical</para><para>Ποιόν ενέργειας</para><para>Вид на глагола</para><para>Glagolski vid</para><para>глаголски вид</para><para>igeszemlélet</para><para>slovesný vid</para><para>Slovesný vid</para><para>вид глагола</para><para>veikslas</para><para>aspetto</para><para>aspecto</para><para>aspekti</para><para>Aspekt</para><para>aspect</para><para>aspect</para><para>Aspect</para><para>aspekt</para><para>aspekt</para><para>aspekt</para><para>aspekt</para><para>aspect</para><para>Aspekt</para><para>אספקט</para><para>вид</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#aspect">http://www.lexinfo.net/ontology/3.0/lexinfo#aspect</seealso>
    let aspect = Prefixed_Name(lexinfov3, "aspect") |> PrefixedName
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>grammaticaal partikel</para><para>partícula gramatical</para><para>particella</para><para>partikkeli</para><para>particule</para><para>partikula</para><para>partícula</para><para>partikkel</para><para>particulă</para><para>partícula</para><para>partícula</para><para>partikula</para><para>partikula</para><para>partykuła</para><para>dalelytė</para><para>particle</para><para>честичка</para><para>partikel</para><para>partikel</para><para>Partikel</para><para>частица</para><para>častica</para><para>částice</para><para>частица</para><para>abisõna</para><para>Часціца</para><para>Čestice</para><para>Členek</para><para>частка</para><para>речца</para><para>מילית</para><para>μόριο</para><para>ögn</para><para>mír</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>įvardinis, įvardžiuotinis</para><para>voornaamwoord</para><para>vietniekvārds</para><para>местоимённый</para><para>местоимение</para><para>pronominale</para><para>asesõnaline</para><para>pronominal</para><para>izenordain</para><para>pronominal</para><para>pronominal</para><para>pronominal</para><para>pronominal</para><para>zamjenice</para><para>займеннік</para><para>αντωνυμία</para><para>כינוי גוף</para><para>займенник</para><para>pronomini</para><para>заменица</para><para>pronomen</para><para>pronomen</para><para>Pronomen</para><para>pronomen</para><para>zaimkowy</para><para>pronoun</para><para>zájmeno</para><para>fornafn</para><para>përemër</para><para>заменка</para><para>pronome</para><para>forainm</para><para>zámeno</para><para>névmás</para><para>pronom</para><para>zaimek</para><para>pronom</para></remarks>
    ...
    /// labels<para>граматички члан</para><para>artikkeli</para><para>artikelis</para><para>членуване</para><para>przedimek</para><para>articolo</para><para>lidwoord</para><para>artikkel</para><para>artículo</para><para>kendeord</para><para>artikkel</para><para>artikulu</para><para>artikuls</para><para>article</para><para>article</para><para>Artikel</para><para>Artikel</para><para>artiklu</para><para>артикль</para><para>артикль</para><para>article</para><para>artikel</para><para>greinir</para><para>артыкль</para><para>articol</para><para>artigo</para><para>névelő</para><para>trajta</para><para>artigo</para><para>άρθρο</para><para>תווית</para><para>член</para><para>člen</para><para>član</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#article">http://www.lexinfo.net/ontology/3.0/lexinfo#article</seealso>
    let article = Prefixed_Name(lexinfov3, "article") |> PrefixedName
    ...
    /// labels<para>propiedade de xénero</para><para>proprietà di genere</para><para>rodzaj gramatyczny</para><para>gjinia gramatikore</para><para>genre grammatical</para><para>genero gramatikal</para><para>género gramatical</para><para>grammatisk kjønn</para><para>Γραμματικό γένος</para><para>grammatisk kjønn</para><para>rasgo de género</para><para>giminės požymis</para><para>Граматычны род</para><para>slovnični spol</para><para>категория рода</para><para>gramatický rod</para><para>Gramatički rod</para><para>kyneiginleiki</para><para>jmenný rod</para><para>מין דקדוקי</para><para>geslacht</para><para>gender</para><para>dzimte</para><para>gènere</para><para>genus</para><para>genus</para><para>Genus</para><para>Genus</para><para>sugu</para><para>suku</para><para>nem</para><para>род</para><para>gen</para><para>рід</para><para>Род</para><para>род</para></remarks>
    ...
    /// labels<para>betrekkelijk voornaamwoord</para><para>относительное местоимение</para><para>відносний займенник</para><para>izenordain erlatibo</para><para>αναφορική αντωνυμία</para><para>relatiivipronomini</para><para>pronombre relativo</para><para>santykinis įvardis</para><para>odnosne zamjenice</para><para>relativt pronomen</para><para>relativt pronomen</para><para>tilvísunarfornafn</para><para>relative pronoun</para><para>pronome relativo</para><para>vonatkozó névmás</para><para>односна заменица</para><para>pronome relativo</para><para>pronome relativo</para><para>zaimek względny</para><para>zájmeno vztažné</para><para>Relativpronomen</para><para>relativpronomen</para><para>Pronume relativ</para><para>pronom relatif</para><para>pronom relatiu</para><para>siduv asesõna</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#relativePronoun">http://www.lexinfo.net/ontology/3.0/lexinfo#relativePronoun</seealso>
    let relativePronoun = Prefixed_Name(lexinfov3, "relativePronoun") |> PrefixedName
    /// <summary>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>kategoria morfosintaktiko</para><para>categoría gramatical</para><para>γραμματική κατηγορία</para><para>categoria gramatical</para><para>categoría gramatical</para><para>parte del discorso</para><para>classe gramatical</para><para>parte de vorbire</para><para>klasë e fjalëve</para><para>зборовна група</para><para>part of speech</para><para>част на речта</para><para>besedna vrsta</para><para>kalbos dalis</para><para>часціны мовы</para><para>частина мови</para><para>slovný druh</para><para>slovní druh</para><para>woordsoort</para><para>sanaluokka</para><para>orðflokkur</para><para>חלקי הדיבר</para><para>часть речи</para><para>część mowy</para><para>врста речи</para><para>ordklasse</para><para>ordklasse</para><para>vārdšķira</para><para>ordklasse</para><para>sõnaliik</para><para>ordklass</para><para>Wortart</para><para>nature</para><para>szófaj</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#PartOfSpeech">http://www.lexinfo.net/ontology/3.0/lexinfo#PartOfSpeech</seealso>
    let PartOfSpeech = Prefixed_Name(lexinfov3, "PartOfSpeech") |> PrefixedName
    /// <summary>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>apstākļa vārds</para><para>prieveiksmis</para><para>dobhriathar</para><para>határozószó</para><para>przysłówek</para><para>прислівник</para><para>příslovce</para><para>príslovka</para><para>atviksorð</para><para>תואר פועל</para><para>ndajfolje</para><para>adberbio</para><para>bijwoord</para><para>adverbio</para><para>määrsõna</para><para>επίρρημα</para><para>adverbio</para><para>avverbio</para><para>прыслоўе</para><para>advérbio</para><para>prilozi</para><para>наречие</para><para>adverbe</para><para>prislov</para><para>adverbi</para><para>наречие</para><para>adverbi</para><para>Adverb</para><para>adverb</para><para>avverb</para><para>прилог</para><para>adverb</para><para>adverb</para><para>adverb</para><para>adverb</para><para>прилог</para><para>biord</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>zelfstandig naamwoord</para><para>имя существительное</para><para>съществително име</para><para>podstatné jméno</para><para>podstatné meno</para><para>daiktavardis</para><para>substantiivi</para><para>substantivo</para><para>substantivo</para><para>samostalnik</para><para>substantiv</para><para>супстантив</para><para>Substantiv</para><para>substantiv</para><para>ουσιαστικό</para><para>rzeczownik</para><para>substantiv</para><para>substantiv</para><para>sostantivo</para><para>sustantivo</para><para>Substantiv</para><para>substantiu</para><para>ainmfhocal</para><para>lietvārds</para><para>nimisõna</para><para>назоўнік</para><para>imenice</para><para>іменник</para><para>nafnorð</para><para>именка</para><para>שם עצם</para><para>főnév</para><para>emër</para><para>izen</para><para>nume</para><para>noun</para><para>nom</para><para>nom</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#Noun">http://www.lexinfo.net/ontology/3.0/lexinfo#Noun</seealso>
    let Noun = Prefixed_Name(lexinfov3, "Noun") |> PrefixedName
    /// <summary>
    ///   <para>lexinfov3:commonNoun</para>
    ...
    /// labels<para>bendrinis daiktavardis</para><para>rzeczownik pospolity</para><para>заједничка именица</para><para>имя нарицательное</para><para>іменник загальний</para><para>substantiv comun</para><para>хадзячая назва</para><para>všeobecné meno</para><para>nombre común</para><para>common noun</para><para>izen arrunt</para><para>nome comune</para><para>Appellativ</para><para>appellativ</para><para>fellesnavn</para><para>ουσιαστικό</para><para>appellativ</para><para>apelativum</para><para>sugasvārds</para><para>nom commun</para><para>nome común</para><para>nome comum</para><para>občno ime</para><para>soortnaam</para><para>yleisnimi</para><para>nom comú</para><para>üldnimi</para><para>samnamn</para><para>samnafn</para><para>köznév</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#commonNoun">http://www.lexinfo.net/ontology/3.0/lexinfo#commonNoun</seealso>
    let commonNoun = Prefixed_Name(lexinfov3, "commonNoun") |> PrefixedName
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>congiunzione</para><para>konjunksjon</para><para>conjonction</para><para>konjunktion</para><para>Konjunktion</para><para>konġunzjoni</para><para>konjunktion</para><para>conjunction</para><para>konjunksjon</para><para>konjunktio</para><para>מילת קישור</para><para>samtenging</para><para>conxunción</para><para>conjunción</para><para>juntagailu</para><para>conjuncție</para><para>bindvlies</para><para>conjunção</para><para>conjunció</para><para>σύνδεσμος</para><para>сполучник</para><para>jungtukas</para><para>sidesõna</para><para>spójnik</para><para>saiklis</para><para>сврзник</para><para>злучнік</para><para>kötőszó</para><para>veznici</para><para>lidhëza</para><para>spojka</para><para>veznik</para><para>spojka</para><para>везник</para><para>союз</para><para>съюз</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>conjonction de coordination</para><para>nebenordnende Konjunktion</para><para>coordinating conjunction</para><para>congiunzione coordinante</para><para>konġunzjoni koordinanti</para><para>conjunção coordenativa</para><para>conjunción coordinante</para><para>emendiozko juntagailu</para><para>priraďovacia spojka</para><para>соединительный союз</para><para>sideordnet bindeord</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#CoordinatingConjunction">http://www.lexinfo.net/ontology/3.0/lexinfo#CoordinatingConjunction</seealso>
    let CoordinatingConjunction =
        Prefixed_Name(lexinfov3, "CoordinatingConjunction") |> PrefixedName
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>marko ditrantsitibo</para><para>marco ditransitivo</para><para>ditransitive frame</para></remarks>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>propiedade de xénero</para><para>proprietà di genere</para><para>gjinia gramatikore</para><para>rodzaj gramatyczny</para><para>género gramatical</para><para>genre grammatical</para><para>genero gramatikal</para><para>grammatisk kjønn</para><para>Γραμματικό γένος</para><para>grammatisk kjønn</para><para>giminės požymis</para><para>rasgo de género</para><para>gramatický rod</para><para>Gramatički rod</para><para>Граматычны род</para><para>категория рода</para><para>slovnični spol</para><para>kyneiginleiki</para><para>מין דקדוקי</para><para>jmenný rod</para><para>geslacht</para><para>gender</para><para>dzimte</para><para>gènere</para><para>Genus</para><para>genus</para><para>genus</para><para>Genus</para><para>suku</para><para>sugu</para><para>рід</para><para>gen</para><para>Род</para><para>nem</para><para>род</para><para>род</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#Gender">http://www.lexinfo.net/ontology/3.0/lexinfo#Gender</seealso>
    let Gender = Prefixed_Name(lexinfov3, "Gender") |> PrefixedName
    ...
    /// labels<para>partícula interrogativa</para><para>вопросительная частица</para><para>interrogative particle</para><para>partikula galdetzaile</para><para>Fragepartikel</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#interrogativeParticle">http://www.lexinfo.net/ontology/3.0/lexinfo#interrogativeParticle</seealso>
    let interrogativeParticle =
        Prefixed_Name(lexinfov3, "interrogativeParticle") |> PrefixedName
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>proprietate morfosintactică a persoanei</para><para>darbības vārda persona</para><para>ιδιότητα του προσώπου</para><para>proprietà di persona</para><para>pertsona gramatikal</para><para>persona gramatical</para><para>grammatisk person</para><para>grammatisk person</para><para>grammatisk person</para><para>pessoa gramatical</para><para>persoa gramatical</para><para>Veta gramatikore</para><para>граматична особа</para><para>Slovnična oseba</para><para>asmens požymis</para><para>personne</para><para>persoona</para><para>persona</para><para>persoon</para><para>persóna</para><para>személy</para><para>person</para><para>person</para><para>Person</para><para>osoba</para><para>Osoba</para><para>osoba</para><para>лице</para><para>лице</para><para>лицо</para><para>isik</para><para>גוף</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#Person">http://www.lexinfo.net/ontology/3.0/lexinfo#Person</seealso>
    let Person = Prefixed_Name(lexinfov3, "Person") |> PrefixedName
    /// <summary>
    ///   <para>lexinfov3:ProperNoun</para>
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>vlastné podstatné meno</para><para>tikrinis daiktavardis</para><para>substantivo próprio</para><para>rzeczownik własny</para><para>ainmfhocal dílis</para><para>властита именица</para><para>имя собственное</para><para>vlastní jméno</para><para>nombre propio</para><para>nome proprio</para><para>nume propriu</para><para>лична именка</para><para>власна назва</para><para>izen propio</para><para>nome propio</para><para>κύριο όνομα</para><para>уласнае імя</para><para>tulajdonnév</para><para>proper noun</para><para>שם עצם פרטי</para><para>Lastno ime</para><para>nom propre</para><para>nom propju</para><para>īpašvārds</para><para>eigennaam</para><para>nom propi</para><para>Eigenname</para><para>pärisnimi</para><para>erisnimi</para><para>proprium</para><para>egennavn</para><para>egennamn</para><para>sérnafn</para><para>særnamn</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#ProperNoun">http://www.lexinfo.net/ontology/3.0/lexinfo#ProperNoun</seealso>
    let ProperNoun = Prefixed_Name(lexinfov3, "ProperNoun") |> PrefixedName
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>conjonction de subordination</para><para>konkunkzio subordinatzaile</para><para>unterordnende Konjunktion</para><para>congiunzione subordinante</para><para>subordinating conjunction</para><para>conjunção subordinativa</para><para>conjunción subordinante</para><para>betrekkelijk voegwoord</para><para>underordnende bindeord</para><para>Podraďovacia spojka</para><para>подчинительный союз</para><para>alistuskonjunktio</para><para>podřízená spojka</para><para>subjunksjon</para><para>subjunksjon</para><para>subjunktion</para></remarks>
    ...
    /// labels<para>conjonction de subordination</para><para>konkunkzio subordinatzaile</para><para>unterordnende Konjunktion</para><para>subordinating conjunction</para><para>congiunzione subordinante</para><para>conjunción subordinante</para><para>conjunção subordinativa</para><para>betrekkelijk voegwoord</para><para>underordnende bindeord</para><para>Podraďovacia spojka</para><para>подчинительный союз</para><para>alistuskonjunktio</para><para>podřízená spojka</para><para>subjunksjon</para><para>subjunksjon</para><para>subjunktion</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#subordinatingConjunction">http://www.lexinfo.net/ontology/3.0/lexinfo#subordinatingConjunction</seealso>
    let subordinatingConjunction =
        Prefixed_Name(lexinfov3, "subordinatingConjunction") |> PrefixedName
    ...
    ///   <para>owl:Class</para>
    ///
    /// labels<para>Zënia e Hënës (Eklipsi)</para><para>punti di sospensione</para><para>points de suspension</para><para>Puncte de suspensie</para><para>puntos suspensivos</para><para>uteslutningstecken</para><para>Udeladelsesprikker</para><para>puntos suspensivos</para><para>Auslassungspunkte</para><para>suspension points</para><para>Три тачке (знак)</para><para>punts suspensius</para><para>Kolme pistettä</para><para>beletselteken</para><para>Αποσιωπητικά</para><para>Mõttepunktid</para><para>שלוש נקודות</para><para>Daudzpunkte</para><para>etenpuntuak</para><para>Wielokropek</para><para>reticências</para><para>три крапки</para><para>многоточие</para><para>Многоточие</para><para>három pont</para><para>Daugtaškis</para><para>Три точки</para><para>Tri bodky</para><para>Tripičje</para><para>Trotočje</para><para>výpustka</para><para>Focalbhá</para><para>ellipse</para><para>ellipse</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#SuspensionPoints">http://www.lexinfo.net/ontology/3.0/lexinfo#SuspensionPoints</seealso>
    let SuspensionPoints = Prefixed_Name(lexinfov3, "SuspensionPoints") |> PrefixedName
    ...
    /// labels<para>bijvoeglijk naamwoord</para><para>прилагателно име</para><para>přídavné jméno</para><para>īpašības vārds</para><para>прилагательное</para><para>prídavné meno</para><para>przymiotnik</para><para>lýsingarorð</para><para>прикметник</para><para>adjektiivi</para><para>omadussõna</para><para>tillægsord</para><para>adxectivo</para><para>adjective</para><para>aggettivo</para><para>adjektibo</para><para>melléknév</para><para>būdvardis</para><para>прыметнік</para><para>pridevnik</para><para>adjectiv</para><para>aidiacht</para><para>adjetivo</para><para>Adjektiv</para><para>adjectiu</para><para>придавка</para><para>aġġettiv</para><para>adjektiv</para><para>adjektiv</para><para>adjectif</para><para>pridjevi</para><para>adjetivo</para><para>adjektiv</para><para>שם תואר</para><para>mbiemri</para><para>επίθετο</para><para>придев</para></remarks>
    /// <seealso href="http://www.lexinfo.net/ontology/3.0/lexinfo#adjective">http://www.lexinfo.net/ontology/3.0/lexinfo#adjective</seealso>
    let adjective = Prefixed_Name(lexinfov3, "adjective") |> PrefixedName
    /// <summary>
