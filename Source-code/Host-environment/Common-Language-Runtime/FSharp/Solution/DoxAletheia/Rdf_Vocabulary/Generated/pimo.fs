...
module pimo =
    ...
    ///   <para>relation to the organization in an OrganizationMember association.</para>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>the person taking the role</para>
    ...
    ///
    /// labels<para>AbstractClass</para></remarks>
    ...
    ///   <para>Entities that are in the direct attention of the user when doing knowledge work.</para>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>The role of someone attending a social event.</para>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>A structure that has a roof and walls and stands more or less permanently in one place; "there was a three-story building on the corner"; "it was an imposing edifice". (Definition from SUMO).</para>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>Superclass of class and thing. To add properties to both class and thing.</para>
    /// labels<para>ClassOrThing</para></remarks>
    ...
    ///   <para>pimo:ClassRole</para>
    ///
    /// labels<para>ConcreteClass</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>The territory occupied by a nation; "he returned to the land of his birth"; "he visited several European countries". (Definition from SUMO)</para>
    ...
    /// An Event is conceived as compact in time. (Definition from Merriam-Webster)</para>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>Concepts that relate to a series of actions or operations conducing to an end. Abstract class. Defines optional start and endtime properties, names taken from NCAL.</para>
    /// labels<para>Process Concept</para></remarks>
    ...
    ///
    /// labels<para>GroundingClosure</para></remarks>
    ...
    ///   <para>nrl:GraphView</para>
    ///
    /// labels<para>InferOccurrences</para></remarks>
    ...
    ///   <para>The object topic is more specific in meaning than the subject topic. Transitive. Similar in meaning to skos:narrower</para>
    /// labels<para>sub topic</para></remarks>
    ...
    ///   <para>rdf:Property</para>
    ///   <para>nrl:TransitiveProperty</para>
    ///   <para>The object topic is more general in meaning than the subject topic. Transitive. Similar to skos:broader.</para>
    /// labels<para>broader topic</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>A wiki-like free-text description of a Thing or a Class. The text can be formatted using a limited set of HTML elements and can contain links to other Things. The format is described in detail in the WIF specification (http://semanticweb.org/wiki/Wiki_Interchange_Format).</para>
    /// labels<para>wiki text</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>A collection of Things, independent of their class. The items in the collection share a common property. Which property may be modelled explicitly or mentioned in the description of the Collection. The requirement of explicit modelling the semantic meaning of the collection is not mandatory, as collections can be created ad-hoc. Implizit modelling can be applied by the system by learning the properties. For example, a Collection of "Coworkers" could be defined as that all elements must be of class "Person" and have an attribute "work for the same Organization as the user". Further standards can be used to model these attributes.</para>
    ...
    /// (inspired by FOAF).
    /// Agent is not a subclass of NAO:Party.</para>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>Roles of classes in PIMO: concrete instances are Abstract and Concrete.</para>
    /// labels<para>ClassRole</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>Annotating abstract and concrete classes. Implementations may offer the feature to hide abstract classes. By default, classes are concrete. Classes can be declared abstract by setting their classRole to abstract. Instances should not have an abstract class as type (if not inferred).</para>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>A social event is attended by a person.</para>
    /// labels<para>attendee</para></remarks>
    /// <seealso href="http://www.semanticdesktop.org/ontologies/2007/11/01/pimo#attendee">http://www.semanticdesktop.org/ontologies/2007/11/01/pimo#attendee</seealso>
    let attendee = Prefixed_Name(pimo, "attendee") |> PrefixedName
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>Superclass of resources that can be generated by the user.</para>
    /// labels<para>ClassOrThingOrPropertyOrAssociation</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>A person takes a certain role in a given context. The role can be that of "a mentor or another person" or "giving a talk at a meeting", etc.</para>
    /// labels<para>Role of Person</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>A blog note. You just want to write something down right now and need a place to do that. Add a blog-note! This is an example class for a document type, there are more detailled ontologies to model Blog-Posts (like SIOC).</para>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>A large and densely populated urban area; may include several independent administrative districts; "Ancient Troy was a great city". (Definition from SUMO)</para>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>A binding agreement between two or more persons that is enforceable by law. (Definition from SUMO). This is an example class for a document type, there are more detailled ontologies to model Contracts.</para>
    ...
    /// The groundingOccurrence of a LogicalMediaType is the Document that stores the content.</para>
    /// labels<para>Logical Media Type</para></remarks>
    ...
    ///   <para>nrl:GraphView</para>
    ///
    /// labels<para>FullPimoView</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>Things that can be at a location. Abstract class, use it as a superclass of things that can be placed in physical space.</para>
    /// labels<para>Locatable</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>The social act of assembling for some common purpose; "his meeting with the salesman was the high point of his day". (Definition from SUMO)</para>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>when is this task due? Represented in ISO 8601, example: 2003-11-22T17:00:00</para>
    /// labels<para>task due time</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>A note. The textual contents of the note should be expressed in the nao:description value of the note.</para>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>A social occasion or activity. (Definition from Merriam-Webster)</para>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>An administrative and functional structure (as a business or a political party). (Definition from Merriam-Webster)</para>
    /// labels<para>Organization</para></remarks>
    /// <seealso href="http://www.semanticdesktop.org/ontologies/2007/11/01/pimo#Organization">http://www.semanticdesktop.org/ontologies/2007/11/01/pimo#Organization</seealso>
    let Organization = Prefixed_Name(pimo, "Organization") |> PrefixedName
    /// <summary>
    ...
    ///   <para>nrl:GraphView</para>
    ///
    /// labels<para>OccurrenceClosure</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>Represents a person. Either living, dead, real or imaginary. (Definition from foaf:Person)</para>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>The role of one or multiple persons being a member in one or multiple organizations. Use pimo:organization and pimo:roleHolder to link to the organizations and persons.</para>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>A Personal Information Model (PIMO) of a user. Represents the sum of all information from the personal knowledge workspace (in literature also referred to as Personal Space of Information (PSI)) which a user needs for Personal Information Management (PIM).</para>
    /// labels<para>Personal Information Model</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>A group of Persons. They are connected to each other by sharing a common attribute, for example they all belong to the same organization or have a common interest. Refer to pimo:Collection for more information about defining collections.</para>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>Any piece of work that is undertaken or attempted (Wordnet). An enterprise carefully planned to achieve a particular aim (Oxford Dictionary).</para>
    ...
    ///
    /// labels<para>RuleViewSpecificationGroundingClosure</para></remarks>
    ...
    ///   <para>nrl:RuleViewSpecification</para>
    ///
    /// labels<para>RuleViewSpecificationInferOccurrences</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>A properPart of a Building which is separated from the exterior of the Building and/or other Rooms of the Building by walls. Some Rooms may have a specific purpose, e.g. sleeping, bathing, cooking, entertainment, etc. (Definition from SUMO).</para>
    ...
    ///   <para>nrl:RuleViewSpecification</para>
    ///
    /// labels<para>RuleViewSpecificationOccurrenceClosure</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>Administrative subdivisions of a Nation that are broader than any other political subdivisions that may exist. This Class includes the states of the United States, as well as the provinces of Canada and European countries. (Definition from SUMO).</para>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>A topic is the subject of a discussion or document. Topics are distinguished from Things in their taxonomic nature, examples are scientific areas such as "Information Science", "Biology", or categories used in content syndication such as "Sports", "Politics". They are specific to the user's domain.</para>
    ...
    /// There can be multiple effectual periods.</para>
    /// labels<para>association effectual during</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>Tags in the context of PIMO. A marker class for Things that are used to categorize documents (or other things). Tags must be a kind of Thing and must have a unique label. Documents should not be Tags by default.</para>
    ...
    /// <remarks>
    ///   <para>rdfs:Class</para>
    ///   <para>A (usually assigned) piece of work (often to be finished within a certain time). (Definition from Merriam-Webster)</para>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>An super-property of all roles that an entity can have in an association. Member is the generic role of a thing in an association. Association subclasses should define sub-properties of this property. Associations can have Things as</para>
    /// labels<para>association member</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>A person attends a social event.</para>
    /// labels<para>attends event</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>The thing is related to the other thing. Similar in meaning to skos:related. Symmetric but not transitive.</para>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>The subject location is contained within the object location. For example, a room is located within a building or a city is located within a country.</para>
    /// labels<para>located within</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>The creator of the Personal Information Model. The human being whose mental models are represented in the PIMO.</para>
    /// labels<para>created Pimo</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>The creator of the Personal Information Model. A subproperty of NAO:creator. The human being whose mental models are represented in the PIMO. Range is an Agent.</para>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>The object of statements is a literal, resource, or datatype value describing the subject thing. Users should be able to edit statements defined with this property. Abstract super-property.</para>
    /// labels<para>datatype property</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>The duration of the process (meeting, event, etc). Difference between start and end time.</para>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>The global namespace of this user using the semdesk uri scheme, based on the Global Identifier of the user. Example semdesk://bob@example.com/things/. See http://dev.nepomuk.semanticdesktop.org/repos/trunk/doc/2008_09_semdeskurischeme/index.html</para>
    /// labels<para>global namespace</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>The subject's contents describes the object. Or the subject can be seen as belonging to the thing described by the object.  Similar semantics as skos:subject.</para>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>The local namespace of this user using the semdesk uri scheme, based on the Local Identifier of the user. Example semdesk://bob@/things/. See http://dev.nepomuk.semanticdesktop.org/repos/trunk/doc/2008_09_semdeskurischeme/index.html</para>
    /// labels<para>local namespace</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>Defines which agents form part of a given person group.</para>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>This property can be used to organise agents within groups. Supplemented with an inverse property.</para>
    /// labels<para>member of group</para></remarks>
    ...
    /// hasOtherConceptualization is an transitive relation, but not equivalent (not symmetric nor reflexive).</para>
    /// labels<para>has other conceptualization</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>This is part of the object. Like a page is part of a book or an engine is part of a car. You can make sub-properties of this to reflect more detailed relations.</para>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>This thing is described further in the object thing. Similar  semantics as skos:isSubjectOf.</para>
    /// labels<para>is tag for</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>The context where the role-holder impersonates this role. For example, the company where a person is employed.</para>
    /// labels<para>role context</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>The subject location contains the object location. For example, a building contains a room or a country contains a city.</para>
    /// labels<para>contains location</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>The object is part of the subject. Like a page is part of a book or an engine is part of a car. You can make sub-properties of this to reflect more detailed relations. The semantics of this relations is the same as skos:narrowerPartitive</para>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>This property specifies the date and time when a process ends. Inspired by NCAL:dtend.</para>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>This property specifies when the process begins. Inspired by NCAL:dtstart.</para>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>Marks a pimo:Tag or a pimo:Topic (its subclass) as an interest of a person or a group.</para>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>Links interests, as instances of pimo:Tag or a pimo:Topic (its subclass), to a person or a group.</para>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>The subject thing is currently located at the object location.</para>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>The subject location is the current location of the object.</para>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>The subject organization has the object person or organization (Agent) as a member.</para>
    /// labels<para>organization has member</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>The subject person or organozation (Agent) is member of the object organization.</para>
    /// labels<para>is member of organization</para></remarks>
    ...
    ///   <para>nrl:TransitiveProperty</para>
    ///   <para>rdf:Property</para>
    ///   <para>hasOtherSlot points from a clot  in your PIMO to a slot in a domain ontology that represents the same connection idea.</para>
    /// labels<para>represented by other slot</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>The object of statements is another Thing. Users should be able to edit statements defined with this property. Abstract super-property.</para>
    /// labels<para>object property</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>The root topics of this PersonalInformationModel's topic hierarchy. Every topic that has no pimo:superTopic is a root topic. Semantically equivalent to skos:hasTopConcept.</para>
    /// labels<para>root topic</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>Each element in a PIMO must be connected to the PIMO, to be able to track multiple PIMOs in a distributed scenario. Also, this is the way to find the user that this Thing belongs to.</para>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>Defines if this information model can be modified by the user of the system. This is usually false for imported ontologies and true for the user's own PersonalInformationModel.</para>
    /// labels<para>is writeable</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>Jabber-ID of the user. Used to communicate amongst peers in the social scenario of the semantic desktop. Use the xmpp node identifier as specified by RFC3920, see http://www.xmpp.org/specs/rfc3920.html#addressing-node. The format is the same as e-mail addresses: username@hostname.</para>
    /// labels<para>jabber ID of the user</para></remarks>
    ...
    /// <remarks>
    ///   <para>rdf:Property</para>
    ///   <para>the attended meeting</para>
    /// labels<para>attending meeting</para></remarks>
