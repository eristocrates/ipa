...
let prefix_registry = JsonProvider<Document.PrefixRegistry.literal_path>.Load Document.PrefixRegistry.literal_path
let prefix_map = 
    ...
    |> Array.map (fun prefix_id -> 
        ...
        | "https://www.w3.org/ns/posix/stat#", namespace_prefix -> prefix_id.NamespaceName, {namespace_name = "https://www.w3.org/ns/posix/stat.rdf#" ; namespace_prefix = prefix_id.NamespacePrefix }
        | _, _-> prefix_id.NamespaceName , {namespace_name = prefix_id.NamespaceName; namespace_prefix = prefix_id.NamespacePrefix  }
    )
    |> Map.ofArray
...
type Iri =
    ...
    static member from_vds_node(vds_node: UriNode) =
        ...
        | Some prefix_id ->
            ...
            Prefixed_Name(prefix_id, local_name) |> PrefixedName
        | None -> Iri_Reference vds_node.Uri.OriginalString |> IRIREF
...
and Iri_Reference(lexical_form:string) =
    ...
    let _nt = sprintf "<%s>" lexical_form
...
and Namespace_Iri(prefix_id: Prefix_ID) =
    ...
    let _nt = sprintf "<%s>" _lexical_form
...
and  RDF_Variable(identifier:string) = 
    ...
    let _binding : Adaptive.aval<RDF_Term option> = _binding_cell :> Adaptive.aval<RDF_Term option>
    ...
    static member from_vds_node(vds_node: VariableNode) =
        RDF_Variable vds_node.VariableName
...
and RDF_Literal =
    ...
    static member from_vds_node(vds_node: LiteralNode) =
        ...
        | lexical_form, datatype_uri, "" -> RDF_Literal.datatyped lexical_form (new UriNode(datatype_uri) |> Iri.from_vds_node )
    ...
    member this.maybe_base_direction = 
        ...
        | TypedLiteral typed_literal -> None
    static member  simple (lexical_form:string):RDF_Literal  = Simple_String(lexical_form)  |> SimpleString |> StringLiteral 
    static member language (lexical_form:string) (language_subtag:Language_Subtag) =
        Language_String(lexical_form, Language_Tag language_subtag ) |> LanguageString  |> StringLiteral
    static member region (lexical_form:string) (language_subtag:Language_Subtag) (region_subtag:Region_Subtag) =
        Language_String(lexical_form, Language_Tag(language_subtag,region_subtag)) |> LanguageString  |> StringLiteral
    ...
    static member datatyped (lexical_form:string) (datatype_iri:Iri) =
        Datatyped_Literal(lexical_form,datatype_iri) |> DatatypedLiteral |> TypedLiteral
    static member inline autotyped<'ValueType> (value:'ValueType) =
        ...
        let datatyped_literal =
            ...
            | :? Boolean as value -> Datatyped_Literal((if value then "true" else "false"), Prefixed_Name(xsd, "boolean").iri)
            | :? (Byte array) as value -> Datatyped_Literal(Convert.ToBase64String(value), Prefixed_Name(xsd, "base64Binary").iri)
            | :? Byte as value -> Datatyped_Literal(invariant_string, Prefixed_Name(xsd, "unsignedByte").iri)
            | :? DateOnly as value -> Datatyped_Literal(value.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture), Prefixed_Name(xsd, "date").iri)
            | :? DateTime as value -> Datatyped_Literal(value.ToString("o", CultureInfo.InvariantCulture), Prefixed_Name(xsd, "dateTime").iri)
            | :? DateTimeOffset as value -> Datatyped_Literal(value.ToString("o", CultureInfo.InvariantCulture), Prefixed_Name(xsd, "dateTimeStamp").iri)
            | :? Decimal as value -> Datatyped_Literal(invariant_string, Prefixed_Name(xsd, "decimal").iri)
            | :? Double as value -> Datatyped_Literal(value.ToString("R", CultureInfo.InvariantCulture), Prefixed_Name(xsd, "double").iri)
            ...
            | :? Single as value -> Datatyped_Literal(value.ToString("R", CultureInfo.InvariantCulture), Prefixed_Name(xsd, "float").iri)
            | :? TimeOnly as value -> Datatyped_Literal(value.ToString("HH:mm:ss.fffffff", CultureInfo.InvariantCulture), Prefixed_Name(xsd, "time").iri)
            | :? TimeSpan as value -> Datatyped_Literal(Xml.XmlConvert.ToString(value), Prefixed_Name(xsd, "duration").iri)
            ...
            | value when value.GetType() = typeof<Object> -> Datatyped_Literal( invariant_string, Prefixed_Name(xdt, "anyAtomicType").iri)
            | value -> Datatyped_Literal(invariant_string, Prefixed_Name(xsd, "string").iri)

        datatyped_literal |> DatatypedLiteral  |> TypedLiteral
and String_Literal = 
...
and Language_String(lexical_form:string,language_tag:Language_Tag) = 
    ...
    let _nt = sprintf "\"%s\"@%s" lexical_form language_tag.as_string
    member this.lexical_form = _lexical_form
...
and Directed_Language_String(lexical_form:string,language_tag:Language_Tag,base_direction:Initial_Text_Direction)  =
    ...
    let _directed_language_datatype_iri = Iri_Reference($"https://www.w3.org/ns/i18n#{language_tag.as_string}_{base_direction.as_string}").iri
    let _vds_node = new LiteralNode(lexical_form,_directed_language_datatype_iri.uri) 
    let _nt = sprintf "\"%s\"@%s--%s" lexical_form language_tag.as_string base_direction.as_string
...
and Datatyped_Literal(lexical_form:string,datatype_iri:Iri) =
    ...
    let _nt = sprintf "\"%s\"^^<%s>" lexical_form datatype_iri.lexical_form
...
and Boolean_Literal(boolean_value:bool) =
    ...
    let _nt = sprintf "\"%s\"^^<%s>" _lexical_form _datatype_iri.lexical_form
...
and SignedByte_Literal(signed_byte_value:sbyte) =
    ...
    let _lexical_form = Convert.ToString(signed_byte_value, CultureInfo.InvariantCulture)
...
and UnsignedLong_Literal(unsigned_long_value:UInt64) =
    ...
    let _lexical_form = Convert.ToString(unsigned_long_value, CultureInfo.InvariantCulture)
...
and Triple_Term(triple:RDF_Triple) =
    ...
    let _lexical_form = sprintf  "%s %s %s" _ttSubject.lexical_form _ttPredicate.lexical_form _ttObject.lexical_form
    let _nt = sprintf "<<( %s %s %s )>>" _ttSubject.nt _ttPredicate.nt _ttObject.nt
    let _curie = sprintf "<<( %s %s %s )>>" _ttSubject.curie _ttPredicate.curie _ttObject.curie
...
and RDF_Triple =
    ...
    member this.lexical_forms = this.curSubject.lexical_form,this.curPredicate.lexical_form,this.curObject.lexical_form
    member this.nt = sprintf "%s %s %s ." this.curSubject.nt this.curPredicate.nt this.curObject.nt
    ...
    member this.vds = new Triple(this.curSubject.vds_node, this.curPredicate.vds_node, this.curObject.vds_node)
...
and Formula =
    ...
    static member from_vds_node(vds_node: GraphLiteralNode) = Formula.from_vds_graph vds_node.SubGraph
