// https://github.com/brianberns/FRange/blob/master/FRange/Range.fs
namespace FRange
...
type Range<'t when 't: comparison and 't :> IComparable<'t>> =
    ...
    override range.ToString() =
        ...
        if range._Lower = Unbounded
           && range._Upper = Unbounded then
            "infinite"
        else
            $"{sLower}x{sUpper}"
...
module Range =
    ...
    let tryCreate lower upper =
        let isValid =
            ...
            | Inclusive lower, Inclusive higher -> lower <= higher // range contains a single value
            | Inclusive lower, Exclusive higher
            | Exclusive lower, Inclusive higher
            | Exclusive lower, Exclusive higher -> lower < higher
            | _ -> true

        if isValid then
            Some { _Lower = lower; _Upper = upper }
        else
            None
    ...
    let create lower upper =
        ...
        |> Option.defaultWith (fun () -> failwith "Invalid range")
    ...
    let private toBoundDirs ranges =
        seq {
            for range in ranges do
                yield BoundDir.create range._Lower BoundType.Lower
                yield BoundDir.create range._Upper BoundType.Upper
        }
            ...
            ||> Seq.fold (fun (activeCount, lowerBound, outRanges) boundDir ->
                ...
                | BoundType.Upper ->
                    ...
                    if activeCount' = 0 then
                        ...
                        activeCount', lowerBound, outRanges
    ...
    let union rangesA rangesB = Seq.append rangesA rangesB |> merge
    ...
    let intersection rangesA rangesB =
        ...
        let activeCounts', lowerBound, outRanges =
            ((ImmutableArray.ToImmutableArray [| 0
                                                 0 |],
              Unbounded,
              []),
             pairs)
            ||> Seq.fold (fun (activeCounts, lowerBound, outRanges) (boundDir, idx) ->
                ...
                | BoundType.Lower ->
                    ...
                    let lowerBound' =
                        if (activeCounts.[idx] = 0)
                           && (activeCounts.[1 - idx] > 0) then
                            boundDir.Bound
                        else
                            lowerBound
    ...
    let difference rangesA rangesB =
        ...
        let activeCounts', lowerBound, outRanges =
            ...
            ||> Seq.fold (fun (activeCounts, lowerBound, outRanges) (boundDir, idx) ->
                ...
                let activeCounts' =
                    activeCounts.SetItem(idx, activeCounts.[idx] - int boundDir.Direction)

                assert
                    (activeCounts'
                     |> Seq.forall (fun count -> count >= 0))

                let lowerBound', finished =
                    match idx, boundDir.Direction with

                    // start new range?
                    | 0, BoundType.Lower when activeCounts'.[0] = 1 && activeCounts.[1] = 0 -> boundDir.Bound, false
                    | 1, BoundType.Upper when activeCounts'.[1] = 0 && activeCounts'.[0] > 0 ->
                        Bound.inverse boundDir.Bound, false

                    // finish new range?
                    | 1, BoundType.Lower when activeCounts'.[1] = 1 && activeCounts'.[0] > 0 -> Unbounded, true
                    | 0, BoundType.Upper when activeCounts'.[0] = 0 && activeCounts.[1] = 0 -> Unbounded, true
    ...
    let inverse ranges = difference [ infinite ] ranges
