...
module fibo =
    ...
    let directory_path =
        ensure_path (Path.Combine(VocabularyDirectory, "https", "spec.edmcouncil.org"))

    let download_link =
        "https://spec.edmcouncil.org/fibo/ontology/master/2026Q1/prod.ttl.zip"

    let path_segments =
        (iriToRelativePath download_link)
            .Replace("\\bare", "")
            .Split("\\")
    ...
    let zip_directory_path =
        ensure_path (Path.Combine(VocabularyDirectory, relative_directory_path))

    let zip_file_path = Path.Combine(zip_directory_path, file_name)
    ...
    let refresh () =
        ...
        if response.originalHttpResponseMessage.IsSuccessStatusCode then
            Response.saveFile zip_file_path response
            ZipFile.ExtractToDirectory(zip_file_path, directory_path)

    let vocabulary_files =
        Directory.GetFiles(directory_path, "*.ttl", SearchOption.AllDirectories)
...
let https_directory = Path.Combine(VocabularyDirectory, "https\\")
...
let fibo_content =
    ...
    |> Array.Parallel.choose (fun ttl_path ->
        ...
        let from_index = https_directory.Length
        let to_index =
        ...
        let maybe_namespace_prefix =
            ...
            |> Array.Parallel.tryFind (fun (namespace_name, prefix_label) -> namespace_name = reconstructed_namespace
...
test <@ fibo.vocabulary_files.Length = fibo_content.Length @>
...
let file_name = path_segments |> Array.last
let relative_directory_path = path_segments |> Array.string_slice 1 -1
