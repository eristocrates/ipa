            ...
            module www =
                module assignments =
                    module media_types =
                        ...
                        let xml =
                            MediaTypesXmlProvider.Load("https://www.iana.org/assignments/media-types/media-types.xml")
...
let ianaPersonByName =
    ianaPeople |> Array.map (fun person -> person.name, person) |> Map.ofArray
...
type IanaDocProvider = JsonProvider<UseOriginalNames=true, OmitNullFields=true, RootName="doc", Sample= @"C:\Repositories\eristocrates\ipa\Source-code\Host-environment\Common-Language-Runtime\FSharp\Interactive\DoxAletheia\ianaDraftSample.json">
...
type IanaDraft =
    ...
    member this.docUrl =
        DomUrl $"https://datatracker.ietf.org/doc/{this.draftName}/doc.json"
...
let rfc_errataXref =
    ianaXrefs |> Array.filter (fun xref -> xref.xrefType = "rfc-errata")
