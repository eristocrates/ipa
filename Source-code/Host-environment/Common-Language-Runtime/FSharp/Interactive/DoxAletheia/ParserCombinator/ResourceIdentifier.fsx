...
module Uniform =
        ...
        let reg_name = 
                {
                    ...
                    code_points = 
                        ...
                        Array.concat [|
                            ...
                            Terminal.unreserved.code_points
                            Terminal.sub_delims.code_points
    ...
    type Segment = 
        ...
        member this.as_code_line = 
            ...
            | FromNonzeroNoncolonSegment  code_line -> code_line
    ...
    type Host = 
        | FromIPLiteral of Nonterminal.IP_Literal
        | FromIPv4Address of Nonterminal.IPv4_Address
        | FromRegisteredName of Registered_Name
         ...
         static member parser :Parser<Host, Code_Point, unit, ReadableMemory<Code_Point>>   = 
                    parser {
                        ...
                        let! host = 
                            choice [
                                Nonterminal.IP_Literal.parser |>> FromIPLiteral
                                Nonterminal.IPv4_Address.parser |>> FromIPv4Address
        ...
        static member parse (input_string:string) =  
            ...
            | Ok success -> success
            | Error failure -> failwith $"{failure} : {code_line_message input_string.as_code_line  Nonterminal.IP_Literal.metasyntax}"
         member this.as_code_square = 
            match this with 
    ...
    type Authority = 
        ...
        | FromHostPort of Host * Nonterminal.Port
        | FromUserinfoHostPort of Userinfo * Host * Nonterminal.Port
         ...
         static member parser :Parser<Authority, Code_Point, unit, ReadableMemory<Code_Point>>   = 
                    parser {
                        ...
                        let! maybe_port = opt Nonterminal.Port.parser
    ...
    type Absolute_URI = 
        | FromSchemePart of Nonterminal.Scheme * Hierarchical_Part
        | FromSchemePartQuery of Nonterminal.Scheme * Hierarchical_Part * Query
        member this.scheme = 
            match this with 
            | FromSchemePart(scheme,hier_part) -> scheme
            | FromSchemePartQuery(scheme,hier_part,query_) -> scheme

         static member parser :Parser<Absolute_URI, Code_Point, unit, ReadableMemory<Code_Point>>   = 
                    parser {
                        let! scheme = Nonterminal.Scheme.parser
                        do! skip_char ':'
    ...
    type URI = 
        | FromSchemePart of Nonterminal.Scheme * Hierarchical_Part
        | FromSchemePartQuery of Nonterminal.Scheme * Hierarchical_Part * Query
        | FromSchemePartFragment of Nonterminal.Scheme * Hierarchical_Part * Fragment
        | FromSchemePartQueryFragment of Nonterminal.Scheme * Hierarchical_Part * Query * Fragment
        member this.scheme = 
            match this with 
            | FromSchemePart(scheme,hier_part) -> scheme
            | FromSchemePartQuery(scheme,hier_part,query_) -> scheme
            | FromSchemePartFragment(scheme,hier_part,fragment) -> scheme
            | FromSchemePartQueryFragment(scheme,hier_part,query_,fragment) -> scheme
        member this.hierarchical_part = 
            match this with 
            | FromSchemePart(scheme,hier_part) -> hier_part
            | FromSchemePartQuery(scheme,hier_part,query_) -> hier_part
            | FromSchemePartFragment(scheme,hier_part,fragment) -> hier_part
            | FromSchemePartQueryFragment(scheme,hier_part,query_,fragment) -> hier_part
        member this.maybe_query = 
            match this with 
            | FromSchemePart(scheme,hier_part) -> None
            | FromSchemePartQuery(scheme,hier_part,query_) -> Some query_
            | FromSchemePartFragment(scheme,hier_part,fragment) -> None
            | FromSchemePartQueryFragment(scheme,hier_part,query_,fragment) -> Some query_
        member this.maybe_fragment = 
            match this with 
            | FromSchemePart(scheme,hier_part) -> None
            | FromSchemePartQuery(scheme,hier_part,query_) -> None
            | FromSchemePartFragment(scheme,hier_part,fragment) -> Some fragment
            | FromSchemePartQueryFragment(scheme,hier_part,query_,fragment) -> Some fragment
...
module Internationalized = 
        ...
        let iprivate = 
                {
                    ...
                    code_points = 
                        ...
                        Array.concat [|
                            0xE000 +-+ 0xF8FF |> Interval_Range.as_array
        ...
        let ucschar = 
                {
                    ...
                    code_points = 
                        ...
                        Array.concat [|
                                        0xA0 +-+ 0xD7FF  |> Interval_Range.as_array
                    ...
                    parser {
                        ...
                        let! immutable_array = 
                                ...
                                many (choice [
                                    ...
                                    Terminal.ipchar_unencoded.code_point_parser
                                    parse_char '/'
                                    parse_char '?'
                    ...
                    parser {
                        ...
                        let! immutable_array = 
                                ...
                                many (choice [
                                    ...
                                    Terminal.ipchar_unencoded.code_point_parser
                                    Terminal.iprivate.code_point_parser
                                    parse_char '/'
                                    parse_char '?'
    ...
    // TODO continue with Fragment (Interationalized )
...
let test_uri_string = "https://roboz0r.github.io/XParsec/combinators.html#Error-Customization-Combinators"
let test_reg_name = "roboz0r.github.io"
Uniform.Registered_Name.parser test_reg_name.as_parser_input
let test_scheme = "https"
Nonterminal.Scheme.parser



let test_uri = Uniform.URI.parse test_uri_string
test_uri.as_rendered_string

test_uri.scheme
test_uri.hierarchical_part
test_uri.maybe_query
test_uri.maybe_fragment


// TODO next add internationalized
