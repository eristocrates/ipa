...
// TODO replace old FSLang references
type Rune with
    member this.codePoint = this.Value
...
module uri =
    module schemes =
        ...
        [<Literal>]
        let filePath = @"C:\Repositories\eristocrates\ipa\Source-code\Host-environment\Common-Language-Runtime\FSharp\Interactive\WorldWideWeb\uri-schemes.xml"
...
let schemeBindings =
    schemes
    |> Array.Parallel.map (fun scheme -> FSLang.Ensure.letBinding scheme.Value.Value.XElement.Value)
    |> String.concat " ; "
