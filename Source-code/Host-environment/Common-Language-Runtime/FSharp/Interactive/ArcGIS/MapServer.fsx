...
module Document =
    let dacpac =
        FilePathInfo(FileInfo @"C:\Repositories\eristocrates\ipa\Source-code\Host-environment\Common-Language-Runtime\FSharp\Interactive\SqlDatabase\Metadata\operations.dacpac")
    let infor_graph =
        FilePathInfo(FileInfo @"D:\Artifact\Company\Infor\Metadata\MetaData.ttl.gz")

module InforProdSql =
    let _namespace_name = "https://inforprodsql.LeonAD.gov#"
    let _prefix_id =
        { namespace_name = _namespace_name
          namespace_prefix = "inforprodsql" }
    let _prefix (local_name: string) =
        Prefixed_Name(_prefix_id, local_name.low_lined) |> PrefixedName
...
module dbug =
    ...
    let _prefix (local_name: string) =
        Prefixed_Name(xsi, local_name.low_lined) |> PrefixedName
...
module Esri =
    ...
    module ArcGISRuntime =
        ...
        let _prefix_id =
            { namespace_name = _namespace_name
              namespace_prefix = "ArcGISRuntime" }
        let _prefix (local_name: string) =
            Prefixed_Name(_prefix_id, local_name.low_lined) |> PrefixedName
...
module interraster =
    module Catalog =
        ...
        module MapServices =
            let _namespace_name =
                $"https://interraster.leoncountyfl.gov/interraster/rest/services/MapServices/"
            let _prefix_id =
                { namespace_name = _namespace_name
                  namespace_prefix = "interraster" }
...
type System.Type with
    member this.GroupedProperties =
        ...
        |> Array.collect (fun (PropertyType, Properties) ->
            Properties
            |> Array.map (fun Property -> sprintf "%s : %s" Property.Name PropertyType.FullName)
            |> Array.insertAt 0 "\n")
    ...
    member this.SystemPropertyTypes =
        ...
        |> Array.filter (fun PropertyTypeFullName ->
            PropertyTypeFullName.StartsWith("System")
            && not (PropertyTypeFullName.StartsWith("System.Collections")))
...
let id_predicates = set [ "id"; "service_layer_id" ]
...
type Field_Type =
    ...
    member this.as_string: string =
        Convert.ToString(this.boxed_value, CultureInfo.InvariantCulture)

    member this.as_int16: int16 =
        Convert.ToInt16(this.boxed_value, CultureInfo.InvariantCulture)

    member this.as_double: double =
        Convert.ToDouble(this.boxed_value, CultureInfo.InvariantCulture)

    member this.as_date_time: DateTime =
        Convert.ToDateTime(this.boxed_value, CultureInfo.InvariantCulture)

    member this.as_int32: int =
        Convert.ToInt32(this.boxed_value, CultureInfo.InvariantCulture)

    member this.as_object: obj = this.boxed_value
...
let rec inline predicateObjectList (value: 'Type) =
    ...
    |> Array.choose (fun Property ->
        ...
        | :? System.String as value when
            not (String.IsNullOrWhiteSpace(value))
            && id_predicates.Contains(predicate_local_name)
            ->
            Some(key_predicate ->- value .*^ xsd.ID)
...
type ArcGISMapImageLayer with
    member this.directory =
        Directory.CreateDirectory $@"D:\Artifact\Company\Esri\{this.Name.low_lined}"
    member this._prefix_id =
        { namespace_name = $"https://interraster.leoncountyfl.gov/interraster/rest/services/MapServices/{this.Name.low_lined}/"
          namespace_prefix = "interraster" }
    member this._prefix(local_name: string) =
        Prefixed_Name(this._prefix_id, local_name.low_lined) |> PrefixedName
...
type CodedValue with
    ...
    member this.named_individual domain_name =
        MapServer._prefix $"{domain_name}.{this.Name}"
    member this.predicateObjectList = predicateObjectList this
    member this.predicates = predicates this
...
type ArcGISServices.MapServiceCapabilities with
    member this.owl_class =
        Esri.ArcGISRuntime._prefix typeof<ArcGISServices.MapServiceCapabilities>.FullName
    member this.predicateObjectList = predicateObjectList this
    member this.predicates = predicates this
...
type Geometry.SpatialReference with
    member this.owl_class =
        Esri.ArcGISRuntime._prefix typeof<Geometry.SpatialReference>.FullName
    member this.predicateObjectList = predicateObjectList this
    member this.predicates = predicates this
...
type ArcGISServices.ArcGISFeatureLayerInfo with
    member this.owl_class =
        Esri.ArcGISRuntime._prefix typeof<ArcGISServices.ArcGISFeatureLayerInfo>.FullName
    member this.predicateObjectList = predicateObjectList this
    member this.predicates = predicates this
...
type ArcGISMapImageSublayer with
    ...
    member this.directory =
        ...
        | Some parent_name ->
            Path.Combine(MapServer.directory.FullName, "esriGeometry" + this.MapServiceSublayerInfo.GeometryType.ToString(), parent_name.low_lined, this.Name.low_lined)
            |> Directory.CreateDirectory
        | None ->
            Path.Combine(MapServer.directory.FullName, "esriGeometry" + this.MapServiceSublayerInfo.GeometryType.ToString(), this.Name.low_lined)
            |> Directory.CreateDirectory

    member this.named_individual = MapServer._prefix this.Name
    member this.owl_class =
        Esri.ArcGISRuntime._prefix typeof<ArcGISMapImageSublayer>.FullName
...
let layer_by_id =
    layers |> Array.Parallel.map (fun layer -> layer.Id, layer) |> Map.ofSeq
...
let fields =
    tables |> Array.Parallel.collect (fun table -> table.Fields |> Seq.toArray)
let domain_by_field =
    fields
...
let field_by_name =
    fields |> Array.Parallel.map (fun field -> field.Name, field) |> Map.ofSeq
let field_types =
    fields
...
let domain_individuals =
    domains |> Array.map (fun domain -> MapServer._prefix domain.Name)
...
let features =
    ...
    |> Array.Parallel.collect (fun (table) ->
        ...
        table.QueryFeaturesAsync(feature_query, QueryFeatureFields.LoadAll)
        |> Async.AwaitTask
        |> Async.RunSynchronously
        |> Seq.toArray
...
type Feature with
    ...
    member this.maybe_subtype =
        ...
        with err ->
            failwithf "%s %O failed with error %s " this.named_individual.lexical_form this err.Message

    member this.layer =
        layer_by_id[(this.FeatureTable :?> ServiceFeatureTable).ServiceLayerId]
    ...
    member this.predicateAttributeList =
        ...
        |> List.map (fun (field_name, attribute_value) ->
            ...
            | Some(domain, coded_value) -> key_predicate ->- MapServer._prefix $"{domain.Name}.{coded_value.Name}"
            | None ->
    ...
    member this.directory =
        Path.Combine(this.layer.directory.FullName, this.id)
        |> Directory.CreateDirectory
    ...
    member this.maybe_x =
        ...
        | Geometry.GeometryType.Point ->
            ...
            if Double.IsNormal x then Some x else None
    ...
    member this.maybe_y =
        ...
        | Geometry.GeometryType.Point ->
            ...
            if Double.IsNormal y then Some y else None
        | _ -> None
    member this.maybe_z =
        match this.Geometry.GeometryType with
        | Geometry.GeometryType.Point ->
            let z = (this.Geometry :?> Geometry.MapPoint).Z
            if Double.IsNormal z then Some z else this.ZVALUE
    ...
    member this.ACCEPT_DATE =
        this.field ACCEPT_DATE |> Option.map (fun value -> value.as_string)
    ...
    member this.ATTACHEDTOID =
        this.field ATTACHEDTOID |> Option.map (fun value -> value.as_string)
    member this.AccuracyCode'ACCURACYCODE =
        this.field ACCURACYCODE |> Option.map (fun value -> value.as_string)
    member this.Accuracy_Code'ACCURACYCODE =
        this.field ACCURACYCODE |> Option.map (fun value -> value.as_string)
    member this.AncillaryRole'ANCILLARYROLE =
        this.field ANCILLARYROLE |> Option.map (fun value -> value.as_int16)
    member this.AttributeComplete'ATTRIBUTECOMPLETE =
        this.field ATTRIBUTECOMPLETE |> Option.map (fun value -> value.as_int16)
    ...
    member this.Company'FIELDCREW =
        this.field FIELDCREW |> Option.map (fun value -> value.as_string)
    member this.CreationDate'CREATIONDATE =
        this.field CREATIONDATE |> Option.map (fun value -> value.as_date_time)
    member this.Creator'CREATOR =
        this.field CREATOR |> Option.map (fun value -> value.as_string)
    member this.Crew'CREW = this.field CREW |> Option.map (fun value -> value.as_string)
    member this.DATECREATED =
        this.field DATECREATED |> Option.map (fun value -> value.as_date_time)
    member this.DATEMODIFIED =
        this.field DATEMODIFIED |> Option.map (fun value -> value.as_date_time)
    member this.DIGITALPICTUREID =
        this.field DIGITALPICTUREID |> Option.map (fun value -> value.as_string)
    member this.DIGITALPICTUREID_2'DIGITALPICTUREID_S =
        this.field DIGITALPICTUREID_S |> Option.map (fun value -> value.as_string)
    member this.DIGITALPICTUREID_3'DIGITALPICTUREID_E =
        this.field DIGITALPICTUREID_E |> Option.map (fun value -> value.as_string)
    member this.DIGITALPICTUREID_4'DIGITALPICTUREID_W =
        this.field DIGITALPICTUREID_W |> Option.map (fun value -> value.as_string)
    member this.DOWNSTREAMDEPTH =
        this.field DOWNSTREAMDEPTH |> Option.map (fun value -> value.as_double)
    member this.DOWNSTREAMELEVATION =
        this.field DOWNSTREAMELEVATION |> Option.map (fun value -> value.as_double)
    member this.DOWNSTREAM_DEPTH =
        this.field DOWNSTREAM_DEPTH |> Option.map (fun value -> value.as_double)
    member this.DOWNSTREAM_ELEV =
        this.field DOWNSTREAM_ELEV |> Option.map (fun value -> value.as_double)
    member this.DRAINAGEAREA =
        this.field DRAINAGEAREA |> Option.map (fun value -> value.as_double)
    member this.DRAINAGEBASIN =
        this.field DRAINAGEBASIN |> Option.map (fun value -> value.as_string)
    member this.DRAINAGE_AREA =
        this.field DRAINAGE_AREA |> Option.map (fun value -> value.as_double)
    member this.DRAWINGID =
        this.field DRAWINGID |> Option.map (fun value -> value.as_string)
    member this.DamageID'DAMAGEID =
        this.field DAMAGEID |> Option.map (fun value -> value.as_string)
    member this.DamageType'DAMAGETYPE =
        this.field DAMAGETYPE |> Option.map (fun value -> value.as_string)
    member this.Datafile'DATAFILE =
        this.field DATAFILE |> Option.map (fun value -> value.as_string)
    member this.Date'DATE_ =
        this.field DATE_ |> Option.map (fun value -> value.as_date_time)
    member this.Date_Created'DATECREATED =
        this.field DATECREATED |> Option.map (fun value -> value.as_date_time)
    member this.Date_Modified'DATEMODIFIED =
        this.field DATEMODIFIED |> Option.map (fun value -> value.as_date_time)
    member this.Diameter'DIAMETER =
        this.field DIAMETER |> Option.map (fun value -> value.as_int16)
    member this.Drawing_ID'DRAWINGID =
        this.field DRAWINGID |> Option.map (fun value -> value.as_string)
    ...
    member this.Easting'EASTING =
        this.field EASTING |> Option.map (fun value -> value.as_double)
    member this.EditDate'EDITDATE =
        this.field EDITDATE |> Option.map (fun value -> value.as_date_time)
    member this.Editor'EDITOR =
        this.field EDITOR |> Option.map (fun value -> value.as_string)
    member this.Enabled'ENABLED =
        this.field ENABLED |> Option.map (fun value -> value.as_int16)
    member this.FACILITYID =
        this.field FACILITYID |> Option.map (fun value -> value.as_string)
    member this.FIELDCOMMENTS =
        this.field FIELDCOMMENTS |> Option.map (fun value -> value.as_string)
    member this.FILTERTYPE =
        this.field FILTERTYPE |> Option.map (fun value -> value.as_string)
    member this.FieldComments'FIELDCOMMENTS =
        this.field FIELDCOMMENTS |> Option.map (fun value -> value.as_string)
    member this.FieldComments'FIELD_COMMENTS_ =
        this.field FIELD_COMMENTS_ |> Option.map (fun value -> value.as_string)
    member this.FieldComplete'FIELDCOMPLETE =
        this.field FIELDCOMPLETE |> Option.map (fun value -> value.as_int16)
    member this.File_Name'FILE_NAME_1 =
        this.field FILE_NAME_1 |> Option.map (fun value -> value.as_string)
    member this.Filename'FILE_NAME =
        this.field FILE_NAME |> Option.map (fun value -> value.as_string)
    member this.Filter'FILTER =
        this.field FILTER |> Option.map (fun value -> value.as_int16)
    member this.FilterLocation'FILTERLOCATION =
        this.field FILTERLOCATION |> Option.map (fun value -> value.as_string)
    member this.Final_Name'FINAL_NAME =
        this.field FINAL_NAME |> Option.map (fun value -> value.as_string)
    member this.Folder'FOLDER =
        this.field FOLDER |> Option.map (fun value -> value.as_string)
    member this.GLOBALID = this.field GLOBALID |> Option.map (fun value -> value.as_string)
    member this.GlobalID'GLOBALID =
        this.field GLOBALID |> Option.map (fun value -> value.as_string)
    member this.HEIGHT = this.field HEIGHT |> Option.map (fun value -> value.as_double)
    member this.HYPERLINK_AR =
        this.field HYPERLINK_AR |> Option.map (fun value -> value.as_string)
    member this.Hotlink'HOTLINK =
        this.field HOTLINK |> Option.map (fun value -> value.as_string)
    member this.Hyperlink'HYPERLINK =
        this.field HYPERLINK |> Option.map (fun value -> value.as_string)
    member this.IMAGERYYEAR =
        this.field IMAGERYYEAR |> Option.map (fun value -> value.as_string)
    member this.INVENTORYDATE =
        this.field INVENTORYDATE |> Option.map (fun value -> value.as_date_time)
    member this.INVERTELEV =
        this.field INVERTELEV |> Option.map (fun value -> value.as_double)
    member this.INVERT_ELEV =
        this.field INVERT_ELEV |> Option.map (fun value -> value.as_double)
    member this.INVERT_ELEV29 =
        this.field INVERT_ELEV29 |> Option.map (fun value -> value.as_double)
    member this.ISINFALL = this.field ISINFALL |> Option.map (fun value -> value.as_int32)
    member this.Inspdate'INSPDATE =
        this.field INSPDATE |> Option.map (fun value -> value.as_date_time)
    member this.Inventoried_by'INVENTORIED_BY =
        this.field INVENTORIED_BY |> Option.map (fun value -> value.as_string)
    member this.InventoryType'INVENTORYTYPE =
        this.field INVENTORYTYPE |> Option.map (fun value -> value.as_string)
    member this.Inventory_Type'INVENTORYTYPE =
        this.field INVENTORYTYPE |> Option.map (fun value -> value.as_string)
    member this.IsInfall'ISINFALL =
        this.field ISINFALL |> Option.map (fun value -> value.as_int16)
    ...
    member this.LegacyID'LEGACY_ID =
        this.field LEGACY_ID |> Option.map (fun value -> value.as_string)
    member this.Lifecycle'LIFECYCLE =
        this.field LIFECYCLE |> Option.map (fun value -> value.as_string)
    member this.Location'LOC_DESC =
        this.field LOC_DESC |> Option.map (fun value -> value.as_string)
    member this.Location_Date'INVENTORYDATE =
        this.field INVENTORYDATE |> Option.map (fun value -> value.as_date_time)
    member this.MILES = this.field MILES |> Option.map (fun value -> value.as_double)
    member this.MaintBy'MAINTBY =
        this.field MAINTBY |> Option.map (fun value -> value.as_string)
    member this.Material'MATERIAL =
        this.field MATERIAL |> Option.map (fun value -> value.as_string)
    member this.Media_Code'MEDIA_CODE =
        this.field MEDIA_CODE |> Option.map (fun value -> value.as_string)
    member this.NEEDSATTENTION =
        this.field NEEDSATTENTION |> Option.map (fun value -> value.as_int16)
    ...
    member this.NUM_BARRELS =
        this.field NUM_BARRELS |> Option.map (fun value -> value.as_int32)
    member this.NeedsAttention'NEEDSATTENTION =
        this.field NEEDSATTENTION |> Option.map (fun value -> value.as_int16)
    member this.Northing'NORTHING =
        this.field NORTHING |> Option.map (fun value -> value.as_double)
    member this.Notes'DESCRIPTION =
        this.field DESCRIPTION |> Option.map (fun value -> value.as_string)
    ...
    member this.OBSTRUCTION =
        this.field OBSTRUCTION |> Option.map (fun value -> value.as_string)
    member this.ORIGINALSOURCE =
        this.field ORIGINALSOURCE |> Option.map (fun value -> value.as_string)
    member this.OUTFALLID =
        this.field OUTFALLID |> Option.map (fun value -> value.as_string)
    member this.OUTFALLTYPE =
        this.field OUTFALLTYPE |> Option.map (fun value -> value.as_string)
    member this.Original_Source'ORIGINALSOURCE =
        this.field ORIGINALSOURCE |> Option.map (fun value -> value.as_string)
    ...
    member this.PERCT_GRD =
        this.field PERCT_GRD |> Option.map (fun value -> value.as_double)
    ...
    member this.PRFRESOLUTION =
        this.field PRFRESOLUTION |> Option.map (fun value -> value.as_string)
    member this.PRFResolution'PRFRESOLUTION =
        this.field PRFRESOLUTION |> Option.map (fun value -> value.as_string)
    member this.PhotoID'PHOTOID =
        this.field PHOTOID |> Option.map (fun value -> value.as_string)
    member this.PhotoNum'PHOTO_NUM =
        this.field PHOTO_NUM |> Option.map (fun value -> value.as_int16)
    member this.Photo_Folder'PHOTO_FOLDER =
        this.field PHOTO_FOLDER |> Option.map (fun value -> value.as_string)
    member this.PipeShape'PIPESHAPE =
        this.field PIPESHAPE |> Option.map (fun value -> value.as_string)
    member this.PondID'PONDID =
        this.field PONDID |> Option.map (fun value -> value.as_string)
    member this.PondID'POND_ID =
        this.field POND_ID |> Option.map (fun value -> value.as_string)
    member this.PondType'PONDTYPE =
        this.field PONDTYPE |> Option.map (fun value -> value.as_string)
    member this.Pond_Number'HYD_ID =
        this.field HYD_ID |> Option.map (fun value -> value.as_string)
    member this.Raw_Name'RAW_NAME =
        this.field RAW_NAME |> Option.map (fun value -> value.as_string)
    member this.RelatedFeature'RELATEDFEATURE =
        this.field RELATEDFEATURE |> Option.map (fun value -> value.as_string)
    member this.Rotation_Az'ROTATION_AZ =
        this.field ROTATION_AZ |> Option.map (fun value -> value.as_int16)
    member this.SHAPE_Area =
        this.field SHAPE_Area |> Option.map (fun value -> value.as_double)
    member this.SHAPE_Length =
        this.field SHAPE_Length |> Option.map (fun value -> value.as_double)
    member this.SLOT_ELEV =
        this.field SLOT_ELEV |> Option.map (fun value -> value.as_double)
    member this.SOURCEYEAR =
        this.field SOURCEYEAR |> Option.map (fun value -> value.as_string)
    member this.STORAGE_FULL_PATH_ADDR =
        this.field STORAGE_FULL_PATH_ADDR |> Option.map (fun value -> value.as_string)
    member this.STORAGE_STATIC_PATH_ADDR =
        this.field STORAGE_STATIC_PATH_ADDR |> Option.map (fun value -> value.as_string)
    member this.STRCT_DEPTH =
        this.field STRCT_DEPTH |> Option.map (fun value -> value.as_double)
    member this.STRUCTUREID =
        this.field STRUCTUREID |> Option.map (fun value -> value.as_string)
    member this.STRUCTURETYPE =
        this.field STRUCTURETYPE |> Option.map (fun value -> value.as_string)
    member this.SUBTYPEFIELD =
        this.field SUBTYPEFIELD |> Option.map (fun value -> value.as_int32)
    member this.Shape_Area =
        this.field Shape_Area |> Option.map (fun value -> value.as_double)
    member this.Shape_Length =
        this.field Shape_Length |> Option.map (fun value -> value.as_double)
    member this.Source'SOURCE =
        this.field SOURCE |> Option.map (fun value -> value.as_string)
    member this.Storage_Path_Address'STORAGE_PATH_ADDR =
        this.field STORAGE_PATH_ADDR |> Option.map (fun value -> value.as_string)
    member this.Struct_ID'STRUCTURE_ID =
        this.field STRUCTURE_ID |> Option.map (fun value -> value.as_string)
    member this.StructureID'STRUCTUREID =
        this.field STRUCTUREID |> Option.map (fun value -> value.as_string)
    member this.SubtypeField'SUBTYPEFIELD =
        this.field SUBTYPEFIELD |> Option.map (fun value -> value.as_int32)
    member this.SurfaceType'SURFACETYPE =
        this.field SURFACETYPE |> Option.map (fun value -> value.as_string)
    member this.UNDERDRAINS =
        this.field UNDERDRAINS |> Option.map (fun value -> value.as_double)
    ...
    member this.UPDATESOURCE =
        this.field UPDATESOURCE |> Option.map (fun value -> value.as_string)
    member this.UPSTREAMDEPTH =
        this.field UPSTREAMDEPTH |> Option.map (fun value -> value.as_double)
    member this.UPSTREAMELEVATION =
        this.field UPSTREAMELEVATION |> Option.map (fun value -> value.as_double)
    member this.UPSTREAMSTUCTUREID =
        this.field UPSTREAMSTUCTUREID |> Option.map (fun value -> value.as_string)
    member this.UPSTREAM_DEPTH =
        this.field UPSTREAM_DEPTH |> Option.map (fun value -> value.as_double)
    member this.UPSTREAM_ELEV =
        this.field UPSTREAM_ELEV |> Option.map (fun value -> value.as_double)
    member this.Update__Source'UPDATESOURCE =
        this.field UPDATESOURCE |> Option.map (fun value -> value.as_string)
    member this.View'VIEW_ = this.field VIEW_ |> Option.map (fun value -> value.as_string)
    member this.WATERBODYNAME =
        this.field WATERBODYNAME |> Option.map (fun value -> value.as_string)
    member this.WBID = this.field WBID |> Option.map (fun value -> value.as_string)
    member this.WHOMODIFIED =
        this.field WHOMODIFIED |> Option.map (fun value -> value.as_string)
    member this.WIDTH = this.field WIDTH |> Option.map (fun value -> value.as_double)
    member this.WhoCreated'WHOCREATED =
        this.field WHOCREATED |> Option.map (fun value -> value.as_string)
    member this.Who_Created'WHOCREATED =
        this.field WHOCREATED |> Option.map (fun value -> value.as_string)
    member this.Who_Modified'WHOMODIFIED =
        this.field WHOMODIFIED |> Option.map (fun value -> value.as_string)
    ...
    member this.ZValue'ZVALUE =
        this.field ZVALUE |> Option.map (fun value -> value.as_double)
    member this.``Inflow\Infiltration'INFILTRATION`` =
        this.field INFILTRATION |> Option.map (fun value -> value.as_int16)
...
module ExcelRows =
    ...
    let private optionTypeDefinition = typedefof<option<_>>
    ...
    let private cellValue (declaredType: System.Type) (value: obj) : obj =
        ...
        if isOptionType declaredType then
            ...
            if isNull value then
                ...
                let unionCase, fields = FSharpValue.GetUnionFields(value, declaredType)
    ...
    let fromRecords<'Record> (records: 'Record list) : DataRow list =

        let recordType = typeof<'Record>

        if not (FSharpType.IsRecord recordType) then
            invalidArg (nameof records) $"{recordType.FullName} is not an F# record type."

        let fields = FSharpType.GetRecordFields(recordType)

        let dataTable = new DataTable(recordType.Name)

        // Preserve the record's declared field order and underlying types.
        fields
        |> Array.iter (fun field -> dataTable.Columns.Add(field.Name, columnType field.PropertyType) |> ignore)
        ...
        |> List.iter (fun record ->
            ...
            |> Array.iteri (fun columnIndex field ->
                ...
                let originalValue = field.GetValue(box record)

                row.[columnIndex] <- cellValue field.PropertyType originalValue)

            dataTable.Rows.Add(row))

        dataTable.Rows |> Seq.cast<DataRow> |> Seq.toList

let xlsx_file = new FileInfo @"D:\Persistence\XLSX\LCG\MapServer.xlsx"
let tableItems records =
    records |> ExcelRows.fromRecords |> TableItems

[

  Worksheet Conduit.name
  FsExcel.Table [ TableName Conduit.name; tableItems Conduit.features ]
  Worksheet Culvert_Cross_Drain.name
  FsExcel.Table [ TableName Culvert_Cross_Drain.name; tableItems Culvert_Cross_Drain.features ]
  Worksheet Debris_Trap.name
  FsExcel.Table [ TableName Debris_Trap.name; tableItems Debris_Trap.features ]
  Worksheet Ditch.name
  FsExcel.Table [ TableName Ditch.name; tableItems Ditch.features ]
  Worksheet End_Point.name
  FsExcel.Table [ TableName End_Point.name; tableItems End_Point.features ]
  Worksheet Inlet.name
  FsExcel.Table [ TableName Inlet.name; tableItems Inlet.features ]
  Worksheet Junction_Fixed.name
  FsExcel.Table [ TableName Junction_Fixed.name; tableItems Junction_Fixed.features ]
  Worksheet Outfall.name
  FsExcel.Table [ TableName Outfall.name; tableItems Outfall.features ]
  ...
  FsExcel.Table [ TableName Stormwater_Pond.name; tableItems Stormwater_Pond.features ]
...
type StormServiceLineAsset =
    ...
    static member from_culvert_cross_drain(culvert_cross_drain: Culvert_Cross_Drain_Feature) =
...
type StormManholeAsset =
    ...
    static member from_junction_fixed(junction_fixed: Junction_Fixed_Feature) =
...
type StormMiscellaneousAsset =
    ...
    static member from_stormwater_pond(stormwater_pond: Stormwater_Pond_Feature) =
...
type StormLeveeAsset =
    ...
    static member from_stormwater_pond_discharge(stormwater_pond_discharge: Stormwater_Pond_Discharge_Feature) =
...
and Feature_Class =
    ...
    static member formula(layer: ArcGISMapImageSublayer) =
        let subtype_formulas =
            ...
            |> Seq.map (fun Subtype ->
                !>layer.named_individual --- Esri.ArcGISRuntime._prefix "subtype"
                --> MapServer._prefix $"{layer.Name}.{Subtype.Name}")
            |> Seq.toList
        ...
        let attribute_formulas =
            ...
            |> Seq.toList
        !|[ Esri.ArcGISRuntime._prefix "Field"; layer.feature_class; layer.owl_class ]
        --- a
        --> owl.Class
...
and Feature_Entity =
    ...
    static member formula(feature: Feature) =
        !>feature.owl_class --- a --> owl.Class -!> feature.layer.named_individual
        ...
        -*| [ if feature.owl_class <> feature.feature_class then
                  !>feature.owl_class --- rdfs.subClassOf -->/ feature.feature_class
                  -~| [ a ->- owl.Class ]
                  --- rdfs.subClassOf
                  -->/ feature.owl_superclass
                  --- a
                  --> owl.Class
              else
                  !>feature.feature_class -~| [ a ->- owl.Class ] --- rdfs.subClassOf
                  -->/ feature.owl_superclass
                  --- a
                  --> owl.Class
        ...
        -*| [ if feature.OUTFALLID.IsSome then
                  ...
                  -->/ MapServer._prefix feature.OUTFALLID.Value
                  -~| [ a ->- owl.NamedIndividual ]
                  --- a
                  -->/ MapServer._prefix "Outfall"
                  --- a
                  --> owl.Class ]
    ...
    { Formula.Empty with
        triples =
            ...
            |> Array.map (fun domain ->
                let coded_value_individuals =
                    domain.CodedValues
                    |> Seq.map (fun coded_value -> coded_value.named_individual domain.Name)
                    |> Seq.toList
...
let save_gzipped_turtle (file_path: string) (graph: IGraph) =
    use file_stream =
        new FileStream(file_path, FileMode.Create, FileAccess.Write, FileShare.None)

    use gzip_stream = new GZipStream(file_stream, CompressionLevel.Optimal)

    use text_writer = new StreamWriter(gzip_stream, UTF8Encoding(false))

    let turtle_writer = CompressingTurtleWriter()
...
domain_formula
|> write_draft feature_dataset.map_server.directory.FullName "domains"
coded_value_formula
|> write_draft feature_dataset.map_server.directory.FullName "coded_values"
feature_dataset.map_server_formula
|> write_draft feature_dataset.map_server.directory.FullName feature_dataset.map_server.Name.low_lined
...
|> Array.Parallel.iter (fun feature_class ->
    feature_class.feature_class_formula
    |> write_draft feature_class.layer.directory.FullName feature_class.layer.Name.low_lined
    feature_class.features
    |> Array.Parallel.iter (fun stormwater_feature ->
        stormwater_feature.feature_formula
        |> write_draft stormwater_feature.feature.directory.FullName stormwater_feature.feature.id.low_lined))
...
module infor =
    ...
    let _prefix_id =
        { namespace_name = _namespace_name
          namespace_prefix = "infor" }

    let _prefix (local_name: string) =
        Prefixed_Name(_prefix_id, local_name.low_lined.Replace("\\", "-"))
        |> PrefixedName
    let _element_class (local_name: string) =
        Prefixed_Name(_prefix_id, local_name |> Converters.SplitCamelCase |> Converters.ToTitleCase |> _.low_lined)
        |> PrefixedName
    let _attribute_property (local_name: string) =
        Prefixed_Name(_prefix_id, local_name |> Converters.SplitCamelCase |> Converters.ToSnakeCase |> _.low_lined)
        |> PrefixedName
    let _element_property (local_name: string) =
        Prefixed_Name(_prefix_id, local_name |> Converters.SplitCamelCase |> Converters.ToSnakeCase |> _.low_lined)
        |> PrefixedName
    ...
    let cardinalityUsedByApplication =
        _attribute_property "cardinalityUsedByApplication"
    ...
    { Formula.Empty with
        triples =
            ...
            |> Seq.collect (fun SystemLicense ->
                ...
                let formula =
                    !>owl_class --- a --> owl.Class -!> system_license
                    -~|> [ a ->- owl_class
                           infor.name ->= SystemLicense.Name
                           if
                               SystemLicense.Description.IsSome
                               && not (String.IsNullOrWhiteSpace SystemLicense.Description.Value)
                           then
    ...
    { Formula.Empty with
        triples =
            ...
            |> Seq.collect (fun DomainColumn ->
                ...
                let formula =
                    !>owl_class --- a --> owl.Class -!> domain_column
                    -~|> [ a ->- owl_class
                           infor.name ->= DomainColumn.Name
                           infor.databaseName ->= DomainColumn.DatabaseName
                           if
                               DomainColumn.DefaultValue.IsSome
                               && not (String.IsNullOrWhiteSpace DomainColumn.DefaultValue.Value)
                           then
...
let product_family_schemas =
    ...
    |> Array.Parallel.choose (fun ProductFamily ->
        InforProdSql.operations.Schemas
        |> PSeq.tryFind (fun Schema ->
            let fully_qualified_name =
                InforProdSql.operations.tbox.DisplayServices.GetElementName(Schema, ElementNameStyle.FullyQualifiedName)
...
let product_family_formulas =
    ...
    |> Array.Parallel.map (fun ProductFamily ->
        ...
        let tables =
            ...
            |> Array.Parallel.map (fun table -> infor._prefix $"Hansen.{ProductFamily.Name}.{table.Name}")
            |> Array.toList
        let database_tables =
            ProductFamily.Tables
            |> Array.Parallel.map (fun table -> InforProdSql._prefix $"{schema_name}.{table.DatabaseName}")
            |> Array.toList
        let formula =
            !|[ InforProdSql.Schema; owl_class ] --- a --> owl.Class -!> product_family
            -~| [ a ->- owl_class
            ...
            -->/ database_schema
            -~|> [ a ->- InforProdSql.Schema; infor.databaseName ->= schema_name ]

        formula |> write_draft directory.path ProductFamily.Name
        formula)

let target_product_families =
    set [ "AssetManagement"; "AssetManagement.Storm"; "AssetManagement.UsageArea" ]
...
let table_formulas =
    ...
    |> Array.Parallel.collect (fun ProductFamily ->
        ...
        |> Array.Parallel.map (fun Table ->
            ...
            let columns =
                Table.Columns
                |> Array.Parallel.map (fun Column -> infor._prefix $"Hansen.{ProductFamily.Name}.{Table.Name}.{Column.Name}")
                |> Array.toList
            let database_columns =
                Table.Columns
                |> Array.Parallel.map (fun Column -> InforProdSql._prefix $"{schema_name}.{Table.DatabaseName}.{Column.DatabaseName}")
                |> Array.toList
            let primary_keys =
                match Table.PrimaryKeyConstraint with
                | Some primaryKeyConstraint ->
                    primaryKeyConstraint.LocalColumnReferences
                    |> Array.map (fun LocalColumnReference -> infor._prefix $"Hansen.{LocalColumnReference.Name}")
                    |> Array.toList
                | None -> []
            ...
            let referencing_formulas =
                ...
                |> Array.collect (fun ReferencingConstraint ->
                    ...
                    |> Array.map (fun (LocalColumnReference, ForeignColumnReference) ->
                        ...
                        !>local_column --- infor.value_referenced_by -->/ foreign_column
                        --- infor.value_references
                        --> local_column
            ...
            let foreign_key_formulas =
                ...
                |> Array.collect (fun ForeignKeyConstraint ->
                    ...
                    |> Array.map (fun (LocalColumnReference, ForeignColumnReference) ->
                        let local_column = infor._prefix $"Hansen.{LocalColumnReference.Name}"
                        let foreign_column = infor._prefix $"Hansen.{ForeignColumnReference.Name}"
                        !>local_column --- infor.value_references -->/ foreign_column
                        --- infor.value_referenced_by
                        --> local_column
            ...
            let enumeration_formulas =
                ...
                |> Array.map (fun EnumerationCheckConstraint ->
                    !>(infor._prefix $"Hansen.{EnumerationCheckConstraint.LocalColumnReference}")
                    --- infor.enumeration
                    --> (infor._prefix $"Enumeration.{EnumerationCheckConstraint.EnumerationName}"))
                |> Array.toList
            let unique_formulas =
                Table.UniqueConstraints
                |> Array.collect (fun UniqueConstraint ->
                    UniqueConstraint.LocalColumnReferences
                    |> Array.map (fun LocalColumnReference ->
                        !>(infor._prefix $"Hansen.{LocalColumnReference.Name}") --- a
                        --> infor.UniqueColumn))
                |> Array.toList
            let maybe_table_key = table_key_from_name Table.Name
            let formula =
                !|[ InforProdSql.Table; owl_class ] --- a --> owl.Class -!> database_table --- a
                --> InforProdSql.Table
                ...
                -~|> [ a ->- owl_class
                       ...
                       if
                           Table.Text.Description.IsSome
                           && not (String.IsNullOrWhiteSpace Table.Text.Description.Value)
                       then
                           infor.description ->= Table.Text.Description.Value
                       if
                           Table.Text.Remarks.IsSome
                           && not (String.IsNullOrWhiteSpace Table.Text.Remarks.Value)
                       then
    ...
    |> Array.Parallel.collect (fun ProductFamily ->
        ...
        |> Array.Parallel.collect (fun Table ->
            ...
            |> Array.Parallel.map (fun Column ->
                ...
                let directory =
                    Folder.ProductFamily ./ ProductFamily.Name ./ Table.Name ./ Column.Name
                ...
                let database_column =
                    InforProdSql._prefix $"{schema_name}.{Table.DatabaseName}.{Column.DatabaseName}"
                ...
                let formula =
                    ...
                    -!> table
                    -~| [ infor.databaseColumn ->- database_column ]
                    --- infor.column
                    -->/ column
                    -~|> [ a ->- owl_class
                           ...
                           if
                               Column.DefaultValue.IsSome
                               && not (String.IsNullOrWhiteSpace Column.DefaultValue.Value)
                           then
                           ...
                           if
                               ...
                               && not (String.IsNullOrWhiteSpace Column.Text.Description.Value)
                           then
                               infor.description ->= Column.Text.Description.Value
                           if
                               Column.Text.Remarks.IsSome
                               && not (String.IsNullOrWhiteSpace Column.Text.Remarks.Value)
                           then
                               infor.remarks ->= Column.Text.Remarks.Value
                           if
                               Column.Text.DisplayDescription.IsSome
                               && not (String.IsNullOrWhiteSpace Column.Text.DisplayDescription.Value)
                           then
                               infor.displayDescription ->= Column.Text.DisplayDescription.Value
                           if
                               Column.Text.DisplayTitle.IsSome
                               && not (String.IsNullOrWhiteSpace Column.Text.DisplayTitle.Value)
                           then
                               infor.displayTitle ->= Column.Text.DisplayTitle.Value
                           if
                               Column.Text.DisplayTitleLong.IsSome
                               && not (String.IsNullOrWhiteSpace Column.Text.DisplayTitleLong.Value)
                           then
        ...
        triples =
            ...
            |> Seq.collect (fun (esri_layer_name, infor_table_name) ->
                ...
                let filtered_names =
                    ...
                    |> Array.filter (fun (layer_name, field_name, productFamily_name, table_name, column_name) ->
                        layer_name = esri_layer_name
                        && $"{productFamily_name}.{table_name}" = infor_table_name)
                let formula =

                    !>h8importtool.Maps --- h8importtool.map -->/ map
                    -~|> [ a ->- h8importtool.Map
                    ...
                    -*| [ for layer_name, field_name, productFamily_name, table_name, column_name in filtered_names do
                              let map_column_name =
                                  $"{layer_name}.{field_name}_to_Hansen.{productFamily_name}.{table_name}.{column_name}"
                              let map_column = h8importtool._prefix map_column_name
                              let source_field = MapServer._prefix $"{layer_name}.{field_name}"
                              let target_column =
                                  infor._prefix $"Hansen.{productFamily_name}.{table_name}.{column_name}"
                              let column_type =
                                  moniker_to_target_column_type productFamily_name table_name column_name
...
!|[ h8importtool.Mapping; h8importtool.MapColumn; h8importtool.Map ] --- a
--> owl.Class
...
--- rdfs.subPropertyOf
--> h8importtool.MapTableInformation
-!> h8importtool.Maps
...
-*| [ h8import_formula ]
|> write_draft Folder.H8ImportTool.path "H8ImportTool"
...
fsi.AddPrinter<ArcGISFeature>(fun feature ->
    (feature.attributes
     |> Map.toArray
     |> Array.map (fun attribute -> sprintf "%O" attribute)
     |> String.concat "\n")
    + "\n\n")
...
|> Map.toArray
|> Array.iter (fun attribute -> printfn "%O" attribute)
random_conduit.attributes
|> Map.toArray
|> Array.choose (fun (attribute_name, _) ->

    layer'field'productFamily'table'column_names
    |> Array.tryFind (fun (layer_name, field_name, productFamily_name, table_name, column_name) -> layer_name = "Conduit" && field_name = attribute_name))
