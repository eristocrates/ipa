...
type Converters with
    ...
    static member To_Pascal_Snake_Case(original_string: string) =
        original_string |> Converters.ToSnakeCase |> Converters.ToTitleCase
...
module Folder =
    let WebServices =
        DirectoryInfo @"D:\Surface\Company\Infor\Download_Center\Product\Operations_and_Regulations\Release\Infor_Public_Sector_2025_04_01\IPS_Web_Services_2025_04_01\ApplicationFiles\WebServices"
    let Generated =
        DirectoryInfo @"C:\Repositories\appsdb\IPS_Sites\integration\InforModel\Generated"
...
module Document =
    module InforProdSql =
        module Dacpac =
            ...
            [<Literal>]
            let xsd_path =
                @"C:\Repositories\eristocrates\ipa\Source-code\Host-environment\Common-Language-Runtime\FSharp\Interactive\SqlDatabase\Metadata\dapac\dacpac.xsd"
...
module HansenDataDistribution =
    ...
    let xsd_navigator = XPathNavigator.Load(xsd_file)
    let xsd =
        XmlProvider<xsd_path>.Load(xsd_path).Element.ComplexType.Sequence.Element.ComplexType.Sequence


let infer_xsds (xml_path: string) =
    let output_directory = Path.GetDirectoryName(Path.GetFullPath(xml_path))

    let base_name = Path.GetFileNameWithoutExtension(xml_path)
    ...
    let schemas =
        ...
        xsd_inferrer.InferSchema(reader).Schemas() |> Seq.cast<XmlSchema> |> Seq.toArray
    ...
    |> Array.mapi (fun index schema ->
        let xsd_path =
            if schemas.Length = 1 then
                ...
                Path.Combine(output_directory, $"{base_name}.{index + 1}.xsd")
...
let xsd_files =
    ...
    |> Array.Parallel.map (fun xsd_file_path -> new FileInfo(xsd_file_path))
...
module XMLSchema =
    ...
    let prefix_from_context (context_element: XElement) =
        context_element.GetPrefixOfNamespace xnamespace
    ...
    let simple_type_from_qname (qname: XmlQualifiedName) =
        XmlSchemaType.GetBuiltInSimpleType qname
    let fsharp_type_from_simple_type (simple_type: XmlSchemaSimpleType) =
        simple_type.Datatype.ValueType.fsharp_name
    let fsharp_type_from_curie (curie: string) (context_element: XElement) =
        context_element
        |> qname_from_curie curie
        |> simple_type_from_qname
        |> fsharp_type_from_simple_type
let combined_schema =


    XElement(XMLSchema.xnamespace + "schema", XAttribute(XNamespace.Xmlns + "s", XMLSchema.xnamespace), XAttribute("elementFormDefault", "qualified"), XAttribute("targetNamespace", "http://hansen.com/"), business_object_elements |> Array.map XElement)
...
module InforProdSql =
    module Data =
        ...
        type Provider = SqlDataProvider<IndividualsAmount=1000, UseOptionTypes=Common.NullableColumnType.OPTION, CaseSensitivityChange=Common.CaseSensitivityChange.ORIGINAL, SsdtPath=Prod.dapac, ConnectionString=Prod.connectionString>
        let operations = Provider.GetDataContext()
(*
    module Dac =
        let extract_dacpac () =
            let services = new Dac.DacServices(Prod.connectionString)
...
dacpac.xml.Model.Elements
|> Array.Parallel.map (fun element -> element.Type)
|> Array.distinct
|> Array.sort
|> Array.iter (fun sql_type -> printfn "%s" sql_type)
                ...
                for element in HansenDataDistribution.xsd.Elements do
                    ...
                    Ast.Record(type_name) {
                        for attribute in element.ComplexType.Attributes do
                            ...
                            let field_type_name =
                                HansenDataDistribution.xsd.XElement
                                |> XMLSchema.fsharp_type_from_curie test_attribute.Type
