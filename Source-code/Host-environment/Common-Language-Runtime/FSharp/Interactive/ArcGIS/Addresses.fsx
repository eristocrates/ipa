...
let AccelaXAPO =
    ArcGISMapImageLayer(new Uri "https://tlcaccela.leoncountyfl.gov/accela/rest/services/AccelaXAPO/MapServer/")
...
type ArcGISMapImageLayer with
    member this.sublayers =
        ...
        |> Seq.toArray
    member this.sublayerById =
        this.sublayers |> Array.map (fun layer -> layer.Id, layer) |> Map.ofArray
...
type Feature with
    ...
    member this.hasAttribute(fieldName: string) =
        ...
        | Some _ -> true
        | None -> false
    member this.ACCELAID =
        this.attributes
        |> Array.tryPick (fun (key, value) -> if key = "ACCELAID" then Some value else None)
    member this.CW_ASSET =
        this.attributes
        |> Array.tryPick (fun (key, value) -> if key = "CW_ASSET" then Some value else None)
    member this.HOUSENUM =
        this.attributes
        |> Array.tryPick (fun (key, value) -> if key = "HOUSENUM" then Some value else None)
    member this.LATITUDE =
        this.attributes
        |> Array.tryPick (fun (key, value) -> if key = "LATITUDE" then Some value else None)
    member this.LONGITUDE =
        this.attributes
        |> Array.tryPick (fun (key, value) -> if key = "LONGITUDE" then Some value else None)
    member this.NAME =
        this.attributes
    ...
    member this.POSTAL_TOWN =
        this.attributes
        |> Array.tryPick (fun (key, value) -> if key = "POSTAL_TOWN" then Some value else None)
    member this.PREFIX =
        this.attributes
        |> Array.tryPick (fun (key, value) -> if key = "PREFIX" then Some value else None)
    member this.STATE =
        this.attributes
        |> Array.tryPick (fun (key, value) -> if key = "STATE" then Some value else None)
    member this.SUFFIX =
        this.attributes
        |> Array.tryPick (fun (key, value) -> if key = "SUFFIX" then Some value else None)
    member this.TYPE =
        this.attributes
        |> Array.tryPick (fun (key, value) -> if key = "TYPE" then Some value else None)
    member this.UNITID =
        this.attributes
        |> Array.tryPick (fun (key, value) -> if key = "UNITID" then Some value else None)
    member this.UNITTYPE =
        this.attributes
        |> Array.tryPick (fun (key, value) -> if key = "UNITTYPE" then Some value else None)
    member this.WHOLE_ADDRESS =
        this.attributes
        |> Array.tryPick (fun (key, value) -> if key = "WHOLE_ADDRESS" then Some value else None)
    member this.ZIPCD =
        this.attributes
        |> Array.tryPick (fun (key, value) -> if key = "ZIPCD" then Some value else None)
...
let addressesMatch (layerAddress: AddressAssetRow) (databaseAddress: AddressAssetRow) =

    addressMatcher.Compare(FieldKind.USStreetAddress, layerAddress.asStreetAddress, databaseAddress.asStreetAddress).State = MatchState.Match
...
type MatchByStreetNumber() =
    ...
    override _.MatchType = AddressMatchType.ByStreetNumber

    override _.Conversion =
        Func<AddressAssetRow, string>(fun address -> address.StreetNumber)

    override _.FilterMatch =
        Func<AddressAssetRow, bool>(fun address -> not (String.IsNullOrWhiteSpace address.StreetNumber))

let addressDefinitions: ISetMatchDefinition<AddressAssetRow, AddressMatchType>[] =
    [| MatchByStreetNumber() :> ISetMatchDefinition<AddressAssetRow, AddressMatchType> |]

let databaseAddressMatcher =
    EntitySetMatcher<AddressAssetRow, AddressMatchType>(databaseAddresses, addressDefinitions)
...
let findCandidates (layerAddress: AddressAssetRow) : AddressAssetRow array =
    let requirements = [| AddressMatchType.ByStreetNumber |]

    let requirementSpan = ReadOnlySpan<AddressMatchType>(requirements)

    let matches = databaseAddressMatcher.FindMatches(layerAddress, requirementSpan)
...
let findMatches (layerAddress: AddressAssetRow) =
    ...
    |> Array.choose (fun databaseAddress ->
        let result =
            addressMatcher.Compare(FieldKind.USStreetAddress, layerAddress.asStreetAddress, databaseAddress.asStreetAddress)
...
let layerAddresses =
    AddressFeatureClass.features |> Array.map AddressAssetRow.fromFeature
...
let addressLinkage =
    ...
    |> Array.map (fun layerAddress ->
        let matches = findMatches layerAddress
...
let alreadyExists (layerAddress: AddressAssetRow) =
    let candidates = findCandidates layerAddress

    candidates |> Array.exists (addressesMatch layerAddress)



let matchedLayerAddresses = layerAddresses |> Array.filter alreadyExists
...
let testAddresses =
    databaseAddresses
    |> Array.filter (fun databaseAddress -> databaseAddress.StreetNumber = matchedLayerAddresses[0].StreetNumber)

testAddresses.Length

let layerAddress, matches = addressLinkage.[0]

layerAddress
matches
let databaseAddress, matchResult = matches.[0]
...
let missingLayerAddresses =
    addressLinkage
    |> Array.choose (fun (layerAddress, matches) -> if Array.isEmpty matches then Some layerAddress else None)

let existingLayerAddresses =
    addressLinkage
    |> Array.choose (fun (layerAddress, matches) -> if Array.isEmpty matches then None else Some layerAddress)
...
let addressAssetRows =
    sampleFeatures |> Array.map AddressAssetRow.fromFeature |> Array.toList
let addressFeatureRows =
    sampleFeatures |> Array.map AddressFeatureRow.fromFeature |> Array.toList

let dateDirectoryName = DateTime.Now.ToString("yyyy-MM-dd")

let dateDirectory =
    Directory.CreateDirectory $@"D:\Persistence\XLSX\LCG\{dateDirectoryName}"
let addressImportFile =
    Path.Combine(dateDirectory.FullName, "Addresses.xlsx") |> FileInfo
...
let MappingsXmlFile =
    Path.Combine(dateDirectory.FullName, "Mappings.xml") |> FileInfo
let sourceFile =
    FileInfo @"D:\www\update\Integration\TLCAccela\AccelaXAPO\Addresses.xlsx"
...
let MappingsXDocument =
    MappingsXmlContent |> ElementBuilder.writeToString |> XDocument.Parse
...
|> Array.iter (fun element ->
    if element.Name.Namespace = XNamespace.None then
        element.Name <-
            XNamespace.op_Implicit ("http://www.infor.com/Hansen8/2011/08/Maps.xsd")
...
let mapnames =
    assetFeatureIdentities
    |> List.map (fun assetFeatureIdentity -> sprintf "%s_to_%s" (assetFeatureIdentity.featureClass.ReplaceWhitespace("_")) (assetFeatureIdentity.businessObject.ReplaceWhitespace("_")))
    |> String.concat ","

let h8importPs1File =
    Path.Combine(dateDirectory.FullName, "h8import.ps1") |> FileInfo
