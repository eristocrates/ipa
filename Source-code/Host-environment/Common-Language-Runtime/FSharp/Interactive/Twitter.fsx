#load @"C:\Secrets\TwitterSecrets.fsx"
#r "nuget: FsHttp"
open TwitterSecrets
...
module private OAuth1 =
    ...
    let normalizeBaseUrl (uri: Uri) =
        ...
        let includePort =
            not ((scheme = "http" && uri.Port = 80) || (scheme = "https" && uri.Port = 443))

        let authority =
            if includePort then $"{host}:{uri.Port}" else host
    ...
    let buildAuthorizationHeader
        ...
        let allParams =
            queryParams @ extraParams @ oauthParams
  ...
  module api = 
    module GET = 
        ...
        let user_by_username
            ...
            let authHeader =
                OAuth1.buildAuthorizationHeader credentials "GET" fullUrl []
        ...
        let community_members_by_id (id:string) = 
            ...
            let baseUrl = $"https://api.x.com/2/communities/{id}/members"
        ...
        let community_members_by_url(url:string) = 
          ...
          community_members_by_id(url.Replace("https://x.com/i/communities/","").Replace("/members",""))


let response = twitter.api.GET.community_members_by_url "https://x.com/i/communities/1695187660526981214" |> Response.toText

File.WriteAllText(Path.Combine(__SOURCE_DIRECTORY__,"getCommunityMembersByIdSample.json"), response)
