...
module c4o =
    ...
    /// </summary>
    let BibliographicInformationSource = _prefixId.prefix "BibliographicInformationSource"
    ...
    /// <summary>
    ///   <para>dc11:description : An in-text reference pointer is a textual device denoting a single bibliographic reference that is embedded in the text of a document within the context of a particular sentence.</para>
    ...
    /// <summary>
    ///   <para>dc11:description : An in-text reference pointer list can contains only in-text reference pointer list items and it always pertains to one or more bibliographic references.</para>
    ...
    ///   <para>dc11:description : An in-text reference pointer list item can contain only a single in-text reference pointer.
    ///
    ...
    /// </summary>
    let InTextReferencePointerListItem = _prefixId.prefix "InTextReferencePointerListItem"

    /// <summary>
    ///   <para>rdfs:comment : An in-text reference pointer list that pertains to exactly one bibliographic reference.</para>
    ///   <para>dc11:description : A single reference pointer list pertains only to one bibliographic reference.
    ///
    ...
    ///   <para>dc11:description : Any FRBR expression that constitutes the range of this property can have at most one particular literal textual content specified through the property c4o:hasContent.</para>
