...
module fabio =
    ...
    ///   <para>dc11:description : An analog item is an exemplar of an analog manifestation only and it is always stored in a storage medium suitable for analog objects, such as paper, vinyl discs and films.</para>
    ...
    /// </summary>
    let ApplicationProgrammingInterface = _prefixId.prefix "ApplicationProgrammingInterface"
    ...
    ///   <para>dc11:description : A digital item is an exemplar of a digital manifestation only and it is always stored in a storage medium suitable for digital objects, such as CDs, DVDs, HDs and the Web.</para>
    ...
    ///   <para>dc11:description : An excerpt is more general than a quotation, and is generally used to indicate a re-published extract from a book, instruction manual, film, radio programme, etc, that need not be what someone said.
    ///
    /// For example:
    ///      Oxford    01865
    ///      Oxshott   01372
    ...
    ///   <para>dc11:description : A fabio:Expression can only have part or be part of another fabio:Expression.  Moreover, it can be a representation only of a fabio:Work, and  it can be embodied only in fabio:Manifestation(s).</para>
    ...
    let ``FRBR%20diagram%20with%20new%20Fabio%20verbs'_period_'png`` = _prefixId.prefix "FRBR%20diagram%20with%20new%20Fabio%20verbs.png"
    ...
    /// <summary>
    ///   <para>dc11:description : A fabio:Item can only have part or be part of another fabio:Item. Moreover, it can be an exemplar only of a fabio:Manifestation.</para>
    ...
    ///   <para>dc11:description : A fabio:Manifestation can only have part or be part of another fabio:Manifestation.  Moreover, it can be an embodiment only of a fabio:Expression and it can be exemplified only by fabio:Item(s).</para>
    ...
    /// <summary>
    ///   <para>dc11:description : A term dictionary is a collection of subject terms.</para>
    ...
    ///   <para>dc11:description : A fabio:Work can only have part or be part of another fabio:Work. Moreover, it can be realized only by fabio:Expression(s).</para>
    ///   <a href="http://purl.org/spar/fabio/Work">fabio:Work</a>
    /// </summary>
    let Work = _prefixId.prefix "Work"
    /// <summary>
    ///   <para>rdfs:label : work collection</para>
    ///   <para>dc11:creator : A collection of works.</para>
    ...
    /// </summary>
    let hasElectronicArticleIdentifier = _prefixId.prefix "hasElectronicArticleIdentifier"
    ...
    /// </summary>
    let hasNLMJournalTitleAbbreviation = _prefixId.prefix "hasNLMJournalTitleAbbreviation"
    ...
    /// </summary>
    let hasNationalLibraryOfMedicineJournalId = _prefixId.prefix "hasNationalLibraryOfMedicineJournalId"
