...
module voaf =
    ...
    ///   <para>rdfs:comment : Classe utilisée pour stocker l'information d'occurences d'un vocabulaire dans un jeu de donnéesrdfs:comment : Class used to store the number of occurences of a vocabulary in a particular dataset</para>
    ///   <para>rdfs:label : Occurrences dans un jeu de donnéesrdfs:label : Dataset occurrences</para>
    ...
    /// <summary>
    ///   <para>rdfs:comment : The number of classes defined in the vocabulary namespace. Classes imported from other namespaces are not taken into account.rdfs:comment : Le nombre de classes définies dans l'espace de noms du Vocabulaire. Les classes importées d'autres vocabulaires ne sont pas comptabilisées.</para>
    ...
    ///   <para>rdfs:comment : Déprécié depuis la v2.4rdfs:comment : Deprecated since v2.4</para>
    ...
    /// <summary>
    ///   <para>vs:term_status : deprecated^^xsd:string</para>
    ///   <para>rdfs:comment : Déprécié depuis la v1.1rdfs:comment : Deprecated since v1.1</para>
    ///   <para>rdfs:label : exemple de jeu de donnéesrdfs:label : example dataset</para>
    ...
    /// <summary>
    ///   <para>vs:term_status : stable^^xsd:string</para>
    ///   <para>rdfs:label : étendrdfs:label : extends</para>
    ///   <para>dcterms:description : PREFIX rdfs:&lt;http://www.w3.org/2000/01/rdf-schema#&gt;
    /// 		PREFIX owl:&lt;http://www.w3.org/2002/07/owl#&gt;
    /// 		PREFIX voaf:&lt;http://purl.org/vocommons/voaf#&gt;
    ///
    ...
    /// 			FILTER(?vocab1!=?vocab2)
    /// 		}^^xsd:string</para>
    ///   <para>rdfs:comment : Indique que le vocabulaire sujet déclare des équivalences de classes ou de propriétés avec le vocabulaire objetrdfs:comment : Indicates that the subject vocabulary declares some equivalent classes or properties with the object vocabulary.</para>
    ///   <para>vs:term_status : testing^^xsd:string</para>
    ...
    /// <summary>
    ///   <para>vs:term_status : testing^^xsd:string</para>
    ///   <para>rdfs:comment : jeu de données dans lequel un vocabulaire apparaitrdfs:comment : dataset in which a vocabulary occurred</para>
    ...
    /// 			FILTER(?vocab1!=?vocab2)
    /// 		}^^xsd:string</para>
    ///   <para>rdfs:label : vocabulaire de métadonnéesrdfs:label : metadata vocabulary</para>
    ///   <para>rdfs:comment : Indique que le vocabulaire sujet utilise le vocabulaire objet pour décrire ses métadonnées, au niveau global du vocabulaire ou au niveau des élémentsrdfs:comment : Indicates that the subject vocabulary uses the object vocabulary in metadata at global vocabulary level or at element level</para>
    ///   <para>vs:term_status : stable^^xsd:string</para>
    ...
    /// <summary>
    ///   <para>vs:term_status : testing^^xsd:string</para>
    ///   <para>rdfs:comment : nombre d'occurrences d'un vocabulaire dans un datasetrdfs:comment : Number of occurrences of a vocabulary in a dataset</para>
    ...
    /// <summary>
    ///   <para>rdfs:comment : The number of properties defined in the vocabulary namespace. Properties imported from other namespaces are not taken into account.rdfs:comment : Le nombre de propriétés définies dans l'espace de noms du Vocabulaire. Les propriétés importées d'autres vocabulaires ne sont pas comptabilisées.</para>
    ///   <para>vs:term_status : stable^^xsd:string</para>
    ///   <para>dcterms:description : PREFIX rdf:&lt;http://www.w3.org/1999/02/22-rdf-syntax-ns#&gt;
    /// 		PREFIX rdfs:&lt;http://www.w3.org/2000/01/rdf-schema#&gt;
    /// 		PREFIX owl:&lt;http://www.w3.org/2002/07/owl#&gt;
    ...
    /// 			}GROUP BY ?vocab
    /// 		}^^xsd:string</para>
    ///   <para>rdfs:label : number of propertiesrdfs:label : nombre de propriétés</para>
    ...
    /// <summary>
    ///   <para>rdfs:label : réutiliserdfs:label : relies on</para>
    ///   <para>rdfs:comment : Indique que le vocabulaire sujet utilise ou étend au moins une classe ou propriété du vocabulaire objetrdfs:comment : Indicates that the subject vocabulary uses or extends some class or property of the object vocabulary</para>
    ///   <para>vs:term_status : stable^^xsd:string</para>
    ...
    /// <summary>
    ///   <para>vs:term_status : testing^^xsd:string</para>
    ///   <para>rdfs:comment : Nombre distinct de jeux de données reutilisant une resource.rdfs:comment : Distinct number of datasets reusing a resource.</para>
    ///   <para>rdfs:label : réutilisé par des jeux de donnéesrdfs:label : reused by datasets</para>
    ...
    /// <summary>
    ///   <para>vs:term_status : testing^^xsd:string</para>
    ///   <para>rdfs:comment : Nombre distinct de vocabulaires reutilisant une resource.rdfs:comment : Distinct number of vocabularies reusing a resource.</para>
    ///   <para>rdfs:label : réutilisé par des vocabulairesrdfs:label : reused by vocabularies</para>
    ...
    ///   <para>vs:term_status : testing^^xsd:string</para>
    ///   <para>rdfs:comment : Used to assert that two vocabularies are similar in scope and objectives, independently of the fact that they otherwise refer to each other.rdfs:comment : Indique que deux vocabulaires sont similaires dans leur domaine et/ou leurs objectifs, sans être nécessairement reliés par des références.</para>
    ...
    ///   <para>rdfs:label : utilisation dans un jeu de donnéesrdfs:label : usage in dataset</para>
