...
module ``cmns-loc`` =
    let _prefixId = PrefixId.fromNamespaceLabel "https://www.omg.org/spec/Commons/Locations/" "cmns-loc"
    ...
    ///   <para>omg-ann:adaptedFrom : FpML Business Center and related codes, see http://www.fpml.org/coding-scheme/business-center-7-14.xml^^xsd:string</para>
    ...
    ///   <para>omg-ann:explanatoryNote : In the US, the US Census Bureau and other federal agencies may group some number of other subdivisions for statistical analysis and reporting purposes, ranging from multiple states to minor civil divisions.^^xsd:string</para>
    ...
    ///   <para>omg-ann:explanatoryNote : The capital administrative region of some federations includes those for which the capital is situated other than within one of the states or provinces which make up the country as a whole.^^xsd:string</para>
    ...
    ///   <para>omg-ann:explanatoryNote : This type of entity, variously referred to as a state, province or canton, has a level of self government including its own legal system and court jurisdiction, but cedes a level of autonomy to the federation of which it forms a part.^^xsd:string</para>
    ///   <para>omg-ann:synonym : canton^^xsd:stringomg-ann:synonym : province^^xsd:stringomg-ann:synonym : state^^xsd:string</para>
    ...
    ///   <para>omg-ann:explanatoryNote : Geodetic coordinates are crucial to any technology or technique based on spatial location, including geodesy, navigation, surveying, geographic information systems, remote sensing, and cartography.^^xsd:string</para>
    ...
    /// </summary>
    let GeographicRegionIdentificationScheme = _prefixId.prefix "GeographicRegionIdentificationScheme"
    ...
    ///   <para>omg-ann:explanatoryNote : A municipality is a general-purpose administrative subdivision, as opposed to a special-purpose district.^^xsd:string</para>
    ...
    ///   <para>omg-ann:adaptedFrom : https://en.wikipedia.org/wiki/City^^xsd:anyURIomg-ann:adaptedFrom : https://www.gleif.org/en/about-lei/common-data-file-format/current-versions/level-1-data-lei-cdf-3-1-format^^xsd:anyURI</para>
    ...
    ///   <para>omg-ann:usageNote : This property should be used in cases where a formal individual for the business center or municipality is not available. Note that Geonames could be used as a source in cases where an individual is desired. Use the property cmns-loc;hasMunicipality in cases where an individual is available. Also note that with respect to an address, this property may stand in for any village, town, or city of any size.^^xsd:string</para>
    ...
    ///   <para>omg-ann:adaptedFrom : https://www.gleif.org/en/about-lei/common-data-file-format/current-versions/level-1-data-lei-cdf-3-1-format^^xsd:anyURI</para>
    ...
    ///   <para>omg-ann:usageNote : Note that certain greater metropolitan areas span multiple counties or states (e.g., the greater Washington, D.C. area, which includes parts of Maryland and Virginia, and divided/disputed cities such as Jerusalem), thus hasMunicipality is a subproperty of hasRegion rather than hasSubdivision.^^xsd:string</para>
    ...
    ///   <para>omg-ann:usageNote : This property is intended for use when using RDF entailment or other inferencing to query for only the primary subdivisions of a geopolitical entity and eliminate references to smaller subdivisions, such as counties, from being returned in results, if present in the data set.^^xsd:string</para>
