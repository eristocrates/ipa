...
let rootDirectory = @"D:\https\com\github\eristocrates\ipa"
...
let enumerateFilesRecursive (rootDir: string) : string array =
    ...
    Directory.EnumerateFiles(rootDir, "*", opts) |> Seq.toArray

let tryFileSize (path: string) =
    try
        let file_info = FileInfo(path)

        if file_info.Exists then Some file_info.Length else None
    with _ ->
        None
...
let generated_text =
    if largeFiles.Length < 1 then
        ...
        largeFiles |> String.concat "\n"

let file_text =
    [|
        authored_text.trimmed
        dotnet_gitignore.trimmed
        generated_header.trimmed
        generated_text
    |]
    |> String.concat "\n"
