...
and PrefixId = {
    ...
    member this.namespaceAbsoluteDirectoryPath =
        let terminalName =
            match this.namespaceName.ToCharArray() |> Array.last with
            | '/' -> '/'.tryHtmlEntity.Value
            | '#' -> '#'.tryHtmlEntity.Value
            | _ -> String.Empty
        let namespaceResource = ResolvedResource.fromUri this.namespaceUri
        namespaceResource.asAbsoluteDirectoryPath.WithSuffix(terminalName)
    ...
    static member inline autotyped<'ValueType>(value: 'ValueType) =
        ...
        let datatypedLiteral =
            ...
            | :? Boolean as value -> {
                ...
                datatypeIri = PrefixId.xsd.prefix "boolean" |> PrefixedIri
              }
            | :? (Byte array) as value -> {
                lexicalForm = Convert.ToBase64String(value)
                datatypeIri = PrefixId.xsd.prefix "base64Binary" |> PrefixedIri
              }
            | :? Byte as value -> {
                lexicalForm = invariantString
                datatypeIri = PrefixId.xsd.prefix "unsignedByte" |> PrefixedIri
              }
            | :? DateOnly as value -> {
                lexicalForm = value.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture)
                datatypeIri = PrefixId.xsd.prefix "date" |> PrefixedIri
              }
            | :? DateTime as value -> {
                lexicalForm = value.ToString("o", CultureInfo.InvariantCulture)
                datatypeIri = PrefixId.xsd.prefix "dateTime" |> PrefixedIri
              }
            | :? DateTimeOffset as value -> {
                lexicalForm = value.ToString("o", CultureInfo.InvariantCulture)
                datatypeIri = PrefixId.xsd.prefix "dateTimeStamp" |> PrefixedIri
              }
            | :? Decimal as value -> {
                lexicalForm = invariantString
                datatypeIri = PrefixId.xsd.prefix "decimal" |> PrefixedIri
              }
            | :? Double as value -> {
                lexicalForm = value.ToString("R", CultureInfo.InvariantCulture)
                datatypeIri = PrefixId.xsd.prefix "double" |> PrefixedIri
              }
            | :? Int16 as value -> {
                lexicalForm = invariantString
                datatypeIri = PrefixId.xsd.prefix "short" |> PrefixedIri
              }
            | :? Int32 as value -> {
                lexicalForm = invariantString
                datatypeIri = PrefixId.xsd.prefix "int" |> PrefixedIri
              }
            | :? Int64 as value -> {
                lexicalForm = invariantString
                datatypeIri = PrefixId.xsd.prefix "long" |> PrefixedIri
              }
            | :? SByte as value -> {
                lexicalForm = invariantString
                datatypeIri = PrefixId.xsd.prefix "byte" |> PrefixedIri
              }
            | :? Single as value -> {
                lexicalForm = value.ToString("R", CultureInfo.InvariantCulture)
                datatypeIri = PrefixId.xsd.prefix "float" |> PrefixedIri
              }
            | :? TimeOnly as value -> {
                lexicalForm = value.ToString("HH:mm:ss.fffffff", CultureInfo.InvariantCulture)
                datatypeIri = PrefixId.xsd.prefix "time" |> PrefixedIri
              }
            | :? TimeSpan as value -> {
                lexicalForm = Xml.XmlConvert.ToString(value)
                datatypeIri = PrefixId.xsd.prefix "duration" |> PrefixedIri
              }
            | :? UInt16 as value -> {
                lexicalForm = invariantString
                datatypeIri = PrefixId.xsd.prefix "unsignedShort" |> PrefixedIri
              }
            | :? UInt32 as value -> {
                lexicalForm = invariantString
                datatypeIri = PrefixId.xsd.prefix "unsignedInt" |> PrefixedIri
              }
            | :? UInt64 as value -> {
                lexicalForm = invariantString
                datatypeIri = PrefixId.xsd.prefix "unsignedLong" |> PrefixedIri
              }
            | :? Uri as value -> {
                lexicalForm = value.OriginalString
                datatypeIri = PrefixId.xsd.prefix "anyURI" |> PrefixedIri
              }
            | :? DomUrl as value -> {
            ...
            | :? XmlQualifiedName as value -> {
                lexicalForm = value.ToString()
                datatypeIri = PrefixId.xsd.prefix "QName" |> PrefixedIri
              }
            | :? Guid as value -> {
                lexicalForm = value.ToString()
                datatypeIri = PrefixId.xsd.prefix "ID" |> PrefixedIri
              }
            | :? String as value -> {
                lexicalForm = value
                datatypeIri = PrefixId.xsd.prefix "string" |> PrefixedIri
              }
            | null -> {
                lexicalForm = "true"
                datatypeIri = PrefixId.xsi.prefix "nil" |> PrefixedIri
              }
            | value -> {
                lexicalForm = invariantString
                datatypeIri = PrefixId.xdt.prefix "anyAtomicType" |> PrefixedIri
              }
...
and [<CustomEquality; CustomComparison>] RdfVariable = {
    uuid: Guid
    identifier: string
    mutable bindingCell: FSharp.Data.Adaptive.cval<Point option>
} with

    static member fromVariableNode(variableNode: VariableNode) = {
        uuid = Guid.NewGuid()
        identifier = variableNode.VariableName
        bindingCell = FSharp.Data.Adaptive.cval (None: Point option)
    }
    ...
    member this.binding: FSharp.Data.Adaptive.aval<Point option> = this.bindingCell :> FSharp.Data.Adaptive.aval<Point option>
