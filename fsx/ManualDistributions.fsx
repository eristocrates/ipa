...
let graphDistributions =
    [|
        ...
        "http://www.w3.org/2002/12/cal/ical#", "https://www.w3.org/2002/12/cal/ical.n3"
    ...
    |]
    |> Array.map (fun (namespaceName, namespaceDistribution) ->
        try
            namespaceName, Uri namespaceDistribution |> ResolvedResource.fromUri

        with err ->
            failwithf "%s, %s failed with %s" namespaceName namespaceDistribution err.Message)
...
let datasetDistributions =
    [|
        ...
        [|
            ...
            "http://qudt.org/3.1.10/schema/qudt"
    ...
    |]
    |> Array.map (fun (namespaceName, namespaceDistributions) ->
        namespaceName,
        namespaceDistributions
        |> Array.map (fun namespaceDistribution -> Uri namespaceDistribution |> ResolvedResource.fromUri))
